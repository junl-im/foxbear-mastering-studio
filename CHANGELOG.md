...
## v1.4.8 - Dock spectrum cleanup + detail-only FFT

- Finished the Dock FFT removal by deleting the leftover `FoxBearSpectrumVisualizer.renderMini()` API and the internal mini canvas registry.
- Runtime health no longer requires the removed `renderMini` API, so the Dock cleanup cannot create a false missing-global warning.
- Kept the detail-page `FoxBearSpectrumVisualizer.renderPanel()` path, static FFT evidence, live analyser tap support, and diagnostics.
- Tightened `hasRenderableCanvas()` so live FFT only connects when the detail spectrum canvas is actually mounted.
- Kept v1.4.7 PC/PWA floating settings gear alignment and Dock cleanup rules.
- Added `qa/v148_dock_spectrum_cleanup_smoke.js` and `qa/BROWSER_BACK_QA_MATRIX_1.4.8.md`.
- Bumped package/build/cache key to `1.4.8-dock-spectrum-cleanup`.
...
- Stage8 async/mobile Dock polish.
- Stage9 Dock waveform CSS split.
- Stage9.1 cumulative overwrite manifest.
- Stage10 download service split through Stage14 runtime recovery.
- Stage16 mobile settings/version release through Stage23 playback orchestration.
- Stage24 settings overlay cleanup through Stage28 waveform-control-view.js / unmanaged waveform audit.
- v1.4.1 Spectrum Visualizer + Exit Guard, v1.4.2 crossfade/waveform zoom, v1.4.5 external analyser taps.


## v1.4.8 - Stability polish: FFT lifecycle + Back confirm debounce
...
- Added document visibility recovery for live FFT: returning from a hidden/backgrounded browser or PWA can restart the live loop or reactivate the current audio.
- Added hidden-tab frame throttling so FFT rendering does not burn a full animation loop while the app is backgrounded.
...
- Added `qa/v146_stability_polish_smoke.js` and refreshed `qa/BROWSER_BACK_QA_MATRIX_1.4.8.md`.
- Bumped package/build/cache key to `1.4.8-dock-spectrum-cleanup`.
...
- Added `qa/BROWSER_BACK_QA_MATRIX_1.4.2.md` for Kakao/Chrome/Safari/PWA back-refresh expectations. v1.4.8 carries this forward as `qa/BROWSER_BACK_QA_MATRIX_1.4.8.md`.
- Added `qa/v142_crossfade_zoom_spectrum_smoke.js`.
- Bumped asset/cache key to `1.4.2-crossfade-zoom-spectrum`.

---

## v1.4.1 - Spectrum visualizer + browser exit guard

- Promoted patch versioning from the long `1.4.0-stageXX` line to normal patch releases, starting with `1.4.1`.
- Added `src/ui/spectrum-visualizer.js` and `assets/css/spectrum-visualizer.css` to show an AI-facing FFT spectrum canvas in the analysis detail view.
- The spectrum view uses the existing 24-band `analysis.spectrumProfile` immediately, then switches to realtime Web Audio `AnalyserNode` data when a registered preview/player audio element is playing.
- Registered playback-linked audio elements with the spectrum visualizer so Dock/detail/A-B players can drive the live graph without adding standalone hidden players.
- Added a browser navigation guard: refresh/close uses the native `beforeunload` confirmation, while back/PWA hardware back uses an in-app confirm prompt and keeps the user on the workspace when cancelled.
- Added service-worker precache, runtime-health coverage, SRI coverage, and `qa/v141_spectrum_exit_guard_smoke.js`.
- Bumped asset/cache key to `1.4.1-spectrum-exit-guard`.

---

## Stage28 - Waveform control view extraction + unmanaged waveform audit

- Added `src/ui/waveform-control-view.js` as the shared owner for waveform DOM bar creation.
- Dock integrated waveform, A/B inline waveform, waveform compare popup rows, and detail waveform rows now delegate bar rendering to the managed view helper.
- Kept `FoxBearWaveformControlService` as the math/control layer for pointer mapping, playhead mapping, seek, peak percent, and stamping.
...
- Bumped asset/cache key to `1.4.8-dock-spectrum-cleanup`.

---

## v1.4.0 Stage27 - Common waveform control service and next-chat handoff

- Added `src/audio/waveform-control-service.js` as the shared waveform control layer for Dock, A/B, comparison, mastering-settings preview, and inline preview surfaces.
- Centralized pointer-to-percent mapping, audio-to-visual playhead mapping, live playhead updates, progress bar state, local audio seek, strongest-peak detection, managed waveform stamping, and future reusable bar rendering.
- Kept existing app function names as compatibility wrappers while delegating core behavior to `FoxBearWaveformControlService`, reducing the chance that each panel drifts into its own seek/playhead rules again.
...
- Bumped runtime asset queries and service worker cache to `1.4.8-dock-spectrum-cleanup`.
...
## v1.4.8 - Crossfade transitions + waveform zoom + Dock mini spectrum

- Added short 96ms fade-out/fade-in transitions for Dock source changes and A/B switching to reduce click/pop artifacts during comparisons.
- Added detail waveform zoom controls, double-tap zoom, and pinch zoom while keeping re-rendered bars behind the managed waveform view gateway.
- Added Dock mini FFT spectrum through `FoxBearSpectrumVisualizer.renderMini()` and the new `#bottomPreviewSpectrum` host.
- Added `qa/BROWSER_BACK_QA_MATRIX_1.4.8.md` for Kakao/Chrome/Safari/PWA back-refresh expectations.
- Added `qa/v142_crossfade_zoom_spectrum_smoke.js`.
- Bumped asset/cache key to `1.4.8-dock-spectrum-cleanup`.
...
- Bumped asset cache key to `1.4.8-dock-spectrum-cleanup` and service worker cache to `foxbear-shell-v1.4.8-dock-spectrum-cleanup`.
