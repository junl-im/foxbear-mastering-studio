...
- Added `FoxBearPerformanceDiagnostics.getSummary()` to flag common lag causes such as multiple audible audio elements, too many canvas nodes, hidden spectrum loops, runtime-health warnings, and long tasks.
- Added `serializeSnapshot()`, `copySnapshotToClipboard()`, and `clearHistory()` for faster support handoff from the diagnostics panel.
...
- `renderMini` was removed; runtime health does not require `renderMini`.
- detail-only FFT remains the current policy.
- Stability polish for FFT lifecycle and Back confirm debounce remains active.
- Stage27 waveform-control-service and Stage28 waveform-control-view.js unmanaged waveform audit remain active.
