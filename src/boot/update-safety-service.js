...
(function attachFoxBearUpdateSafety(global) {
    ...
    const PATCH_ID = BUILD_INFO.assetVersion ? `v${BUILD_INFO.assetVersion}` : 'v1.6.82-overlay-focus-ancestor-hardening';
    const EXPECTED_BOOT_KEY = BUILD_INFO.bootRevision || 'boot-sri-v1682';
    ...
    function getRecoveryPlan(risk) {
        ...
        if (active.level === 'critical') {
            return Object.freeze([
                ...
                '직접 확인 시 app.js, performance-diagnostics.js, runtime-health.js의 h=boot-sri-v1682와 SRI가 일치해야 합니다.'
            ]);
        }
