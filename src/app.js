...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.13';
...
async function registerFoxBearServiceWorker() {
    ...
    try {
        // stage13 compatibility anchor: navigator.serviceWorker.register('./sw.js?v=1.5.13-handoff-package-integrity')
        const registration = await navigator.serviceWorker.register('./sw.js?v=1.5.13-handoff-package-integrity&h=sw-v1513');
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.13-handoff-package-integrity', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.13-handoff-package-integrity',
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.13-handoff-package-integrity', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.13-handoff-package-integrity', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.13-handoff-package-integrity',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.13-handoff-package-integrity', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.13',
