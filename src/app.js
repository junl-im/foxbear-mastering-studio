// FoxBear AI Mastering Studio Pro v1.5.23 - app slim-down orchestration bridge
'use strict';
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.23';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.5.23-e2e-preview-readiness'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1523'}`;
...
async function registerFoxBearServiceWorker() {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.5.23-e2e-preview-readiness') · navigator.serviceWorker.register('./sw.js?v=1.5.23-e2e-preview-readiness&h=sw-v1523')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.23-e2e-preview-readiness', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.23-e2e-preview-readiness',
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.23-e2e-preview-readiness', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.23-e2e-preview-readiness', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.23-e2e-preview-readiness',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.23-e2e-preview-readiness', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.23',
