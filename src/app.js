// FoxBear AI Mastering Studio Pro v1.6.88 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.88';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.88-mobile-header-grid-ownership-recovery'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1688'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.6.88-mobile-header-grid-ownership-recovery';
...
async function registerFoxBearServiceWorker(options = {}) {
    // compatibility anchors: navigator.serviceWorker.register(resolveFoxBearScriptUrl(SERVICE_WORKER_URL)) · navigator.serviceWorker.register('./sw.js?v=1.6.88-mobile-header-grid-ownership-recovery') · navigator.serviceWorker.register('./sw.js?v=1.6.88-mobile-header-grid-ownership-recovery&h=sw-v1688')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.88-mobile-header-grid-ownership-recovery', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.88-mobile-header-grid-ownership-recovery', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.88-mobile-header-grid-ownership-recovery', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.88-mobile-header-grid-ownership-recovery', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.88-mobile-header-grid-ownership-recovery',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.88-mobile-header-grid-ownership-recovery', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.88-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.88-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.88',
