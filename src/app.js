// FoxBear AI Mastering Studio Pro v1.6.101 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.101';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.101-admin-lazyload-repo-cleanup'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v16101'}`;
const ADMIN_INCIDENT_MONITOR_SCRIPT_URL = `src/ui/admin-incident-monitor-view.js?v=${FoxBearBuildInfo.assetVersion || '1.6.101-admin-lazyload-repo-cleanup'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.6.101-admin-lazyload-repo-cleanup';
...
let adminIncidentMonitorAssetsPromise = null;
let adminStatsReturnFocus = null;
function appendLazyScript(src, integrity) {
    const existing = document.querySelector('script[data-foxbear-lazy="admin-incident-monitor"]');
    if (window.FoxBearAdminIncidentMonitorView?.create) return Promise.resolve(existing || null);
    if (existing) return new Promise((resolve, reject) => {
        existing.addEventListener('load', () => resolve(existing), { once: true });
        existing.addEventListener('error', () => reject(new Error('관리자 오류 관리 모듈을 불러오지 못했습니다.')), { once: true });
    });
    return new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.async = true;
        script.src = resolveFoxBearScriptUrl(src);
        script.dataset.foxbearLazy = 'admin-incident-monitor';
        if (integrity) script.integrity = integrity;
        script.crossOrigin = 'anonymous';
        script.addEventListener('load', () => resolve(script), { once: true });
        script.addEventListener('error', () => reject(new Error('관리자 오류 관리 모듈을 불러오지 못했습니다.')), { once: true });
        document.head.appendChild(script);
    });
}
async function ensureAdminIncidentMonitorAssets() {
    if (window.FoxBearAdminIncidentMonitorView?.create) return true;
    if (adminIncidentMonitorAssetsPromise) return adminIncidentMonitorAssetsPromise;
    const lazy = FoxBearBuildInfo.lazyAssets?.adminIncidentMonitor || {};
    adminIncidentMonitorAssetsPromise = appendLazyScript(ADMIN_INCIDENT_MONITOR_SCRIPT_URL, lazy.scriptIntegrity || '').then(() => {
        if (!window.FoxBearAdminIncidentMonitorView?.create) throw new Error('관리자 오류 관리 모듈 초기화에 실패했습니다.');
        return true;
    }).catch(error => {
        adminIncidentMonitorAssetsPromise = null;
        throw error;
    });
    return adminIncidentMonitorAssetsPromise;
}
...
function getPwaRuntimeBridge() {
    ...
    pwaRuntimeBridge = window.FoxBearPwaRuntimeBridge?.createBridge?.({
        ...
        updateMobileUi: updateMobileNativeUi,
...
async function registerFoxBearServiceWorker(options = {}) {
    // compatibility anchors: navigator.serviceWorker.register(resolveFoxBearScriptUrl(SERVICE_WORKER_URL)) · navigator.serviceWorker.register('./sw.js?v=1.6.101-admin-lazyload-repo-cleanup') · navigator.serviceWorker.register('./sw.js?v=1.6.101-admin-lazyload-repo-cleanup&h=sw-v16101')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.101-admin-lazyload-repo-cleanup', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.101-admin-lazyload-repo-cleanup', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.101-admin-lazyload-repo-cleanup', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.101-admin-lazyload-repo-cleanup', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.101-admin-lazyload-repo-cleanup',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.101-admin-lazyload-repo-cleanup', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.101-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.101-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.101',
