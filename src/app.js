...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.12';
...
async function registerFoxBearServiceWorker() {
    ...
    try {
        // stage13 compatibility anchor: navigator.serviceWorker.register('./sw.js?v=1.5.12-ci-runtime-readiness')
        const registration = await navigator.serviceWorker.register('./sw.js?v=1.5.12-ci-runtime-readiness&h=sw-v1512');
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.12-ci-runtime-readiness', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.12-ci-runtime-readiness',
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.12-ci-runtime-readiness', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.12-ci-runtime-readiness', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.12-ci-runtime-readiness',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.12-ci-runtime-readiness', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.12',
