// FoxBear AI Mastering Studio Pro v1.5.26 - app slim-down orchestration bridge
'use strict';
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.26';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.5.26-engraved-command-header'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1526'}`;
...
function updateAdminStatsTriggerVisibility() {
    ...
    el.adminStatsTrigger.textContent = visible ? '관리자 통계' : '모바일 · PC 호환';
}
...
async function registerFoxBearServiceWorker() {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.5.26-engraved-command-header') · navigator.serviceWorker.register('./sw.js?v=1.5.26-engraved-command-header&h=sw-v1526')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.26-engraved-command-header', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.26-engraved-command-header',
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.26-engraved-command-header', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.26-engraved-command-header', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.26-engraved-command-header',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.26-engraved-command-header', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.26',
