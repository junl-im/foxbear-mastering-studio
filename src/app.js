// FoxBear AI Mastering Studio Pro v1.6.99 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.99';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.99-header-role-focus-integrity'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1699'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.6.99-header-role-focus-integrity';
...
async function registerFoxBearServiceWorker(options = {}) {
    // compatibility anchors: navigator.serviceWorker.register(resolveFoxBearScriptUrl(SERVICE_WORKER_URL)) · navigator.serviceWorker.register('./sw.js?v=1.6.99-header-role-focus-integrity') · navigator.serviceWorker.register('./sw.js?v=1.6.99-header-role-focus-integrity&h=sw-v1699')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.99-header-role-focus-integrity', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.6.99-header-role-focus-integrity',
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.99-header-role-focus-integrity', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.99-header-role-focus-integrity', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.99-header-role-focus-integrity', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.99-header-role-focus-integrity',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.99-header-role-focus-integrity', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.99-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.99-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.99',
