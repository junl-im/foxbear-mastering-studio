// FoxBear AI Mastering Studio Pro v1.6.45 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.45';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.45-windows-release-gate-spark-hosting-no-app-check'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1645-spark-hosting'}`;
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.6.45-windows-release-gate-spark-hosting-no-app-check') · navigator.serviceWorker.register('./sw.js?v=1.6.45-windows-release-gate-spark-hosting-no-app-check&h=sw-v1645-spark-hosting')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.45-windows-release-gate-spark-hosting-no-app-check', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.45-windows-release-gate-spark-hosting-no-app-check', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.45-windows-release-gate-spark-hosting-no-app-check', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.45-windows-release-gate-spark-hosting-no-app-check', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.45-windows-release-gate-spark-hosting-no-app-check',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.45-windows-release-gate-spark-hosting-no-app-check', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.45-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.45-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.45',
