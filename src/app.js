// FoxBear AI Mastering Studio Pro v1.6.4 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.4';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.4-incident-callable-csp-recovery'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1604'}`;
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.6.4-incident-callable-csp-recovery') · navigator.serviceWorker.register('./sw.js?v=1.6.4-incident-callable-csp-recovery&h=sw-v1604')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.4-incident-callable-csp-recovery', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.4-incident-callable-csp-recovery', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.4-incident-callable-csp-recovery', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.4-incident-callable-csp-recovery', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.4-incident-callable-csp-recovery',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.4-incident-callable-csp-recovery', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.4-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.4-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.4',
