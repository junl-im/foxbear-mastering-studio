// FoxBear AI Mastering Studio Pro v1.6.105 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.105';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.105-header-dock-css-ownership-hardening'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v16105'}`;
const ADMIN_INCIDENT_MONITOR_SCRIPT_URL = `src/ui/admin-incident-monitor-view.js?v=${FoxBearBuildInfo.assetVersion || '1.6.105-header-dock-css-ownership-hardening'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.6.105-header-dock-css-ownership-hardening';
...
async function ensureAdminIncidentMonitorAssets() {
    ...
    await loader.load({
        src: ADMIN_INCIDENT_MONITOR_SCRIPT_URL,
...
async function registerFoxBearServiceWorker(options = {}) {
    // compatibility anchors: navigator.serviceWorker.register(resolveFoxBearScriptUrl(SERVICE_WORKER_URL)) · navigator.serviceWorker.register('./sw.js?v=1.6.105-header-dock-css-ownership-hardening') · navigator.serviceWorker.register('./sw.js?v=1.6.105-header-dock-css-ownership-hardening&h=sw-v16105')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.105-header-dock-css-ownership-hardening', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.6.105-header-dock-css-ownership-hardening',
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.105-header-dock-css-ownership-hardening', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.105-header-dock-css-ownership-hardening', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.105-header-dock-css-ownership-hardening', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.105-header-dock-css-ownership-hardening',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.105-header-dock-css-ownership-hardening', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.105-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.105-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.105',
