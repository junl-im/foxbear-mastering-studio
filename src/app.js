// FoxBear AI Mastering Studio Pro v1.7.1 - app slim-down orchestration bridge
...
const {
    ...
    createWaveformOverviewAsync = async (before, after, bins) => createWaveformOverview(before, after, bins),
    removeDcOffsetAudioBufferCooperative = async buffer => removeDcOffsetAudioBuffer(buffer),
    sanitizeAudioBufferCooperative = async (buffer, label) => sanitizeAudioBuffer(buffer, label),
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.7.1';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.7.1-reference-match-2-phase1'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v171'}`;
const ADMIN_INCIDENT_MONITOR_SCRIPT_URL = `src/ui/admin-incident-monitor-view.js?v=${FoxBearBuildInfo.assetVersion || '1.7.1-reference-match-2-phase1'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.7.1-reference-match-2-phase1';
...
const MASTERING_PROGRESS_VISIBLE_STEP = 1, MASTERING_PROGRESS_FULL_RENDER_STEP = 5, MASTERING_PROGRESS_HEARTBEAT_MS = 320;
...
function createAdaptiveMasteringDecisionForTrack(track, requestedSettings) {
    ...
    try {
        const referenceTarget = state.referenceProfile?.status === 'ready' ? state.referenceProfile.target : null;
    ...
    } catch (error) { console.warn('[FoxBear] adaptive mastering decision fallback', error); track.adaptiveDecisionInfo = Object.freeze({ version: '1.7.1-adaptive-decision-phase1', mode: 'fallback', reason: getErrorMessage(error, 'adaptive decision failed') }); return requestedSettings; }
}
...
let playbackSourceRecoveryController = null;
function getPlaybackSourceRecoveryController() { if (playbackSourceRecoveryController) return playbackSourceRecoveryController; const factory = window.FoxBearPlaybackSourceRecoveryService?.createController; if (!factory) return null; playbackSourceRecoveryController = factory({ document, getTrackById: trackId => state.tracks.find(track => String(track?.id || '') === String(trackId || '')) || null, getSelectedTrack, getTransitionService: getPlaybackTransitionService, createObjectURL: blob => URL.createObjectURL(blob), revokeObjectURL: url => URL.revokeObjectURL(url), onTrackUrlReplaced: (_track, mode, nextUrl, previousUrl, audio) => { const root = el.bottomPreviewPlayer; if (root?.dataset?.previewKey && previousUrl && root.dataset.previewKey.includes(previousUrl)) root.dataset.previewKey = root.dataset.previewKey.replace(previousUrl, nextUrl); if (audio?.dataset) audio.dataset.sourceRecoveryMode = mode; } }); return playbackSourceRecoveryController; }
function retirePlaybackObjectUrl(track, url) { return getPlaybackSourceRecoveryController()?.retireObjectUrl?.(track, url) || false; }
function configurePreviewAudioElement(audio) { return getPlaybackTransitionService()?.configureAudioElement?.(audio) || audio; }
function rememberAudioTargetVolume(audio) { return getPlaybackTransitionService()?.rememberTargetVolume?.(audio) ?? 1; } function isAudioPlaybackIntended(audio) { const service = getPlaybackTransitionService(); return typeof service?.isPlaybackIntended === 'function' ? service.isPlaybackIntended(audio) : Boolean(audio && !audio.paused && !audio.ended); }
...
function isOpaqueExternalScriptErrorEvent(event) {
    const message = String(event?.message || event?.error?.message || '').trim();
    if (!/^Script error\.?$/i.test(message) || event?.error) return false;
    const filename = String(event?.filename || '').trim();
    if (!filename) return true;
    try { return new URL(filename, window.location.href).origin !== window.location.origin; }
    catch (error) { return true; }
}
function handleGlobalWindowError(event) {
    if (isOpaqueExternalScriptErrorEvent(event)) {
        console.info('FoxBear ignored an opaque cross-origin script error without actionable stack data.', {
            source: String(event?.filename || 'cross-origin'),
            line: Number(event?.lineno || 0),
            column: Number(event?.colno || 0)
        });
        try {
            window.dispatchEvent(new CustomEvent('foxbear:runtime-external-warning', {
                detail: { code: 'OPAQUE_EXTERNAL_SCRIPT_ERROR', source: String(event?.filename || '') }
            }));
        } catch (dispatchError) {}
        return false;
    }
    reportBootOrImportError(event?.error || event?.message, '앱 실행 오류');
    return true;
}
...
function registerPlaybackLinkedAudio(audio, meta = {}) { if (!audio || !FoxBearPlaybackLinkService || typeof FoxBearPlaybackLinkService.registerAudio !== 'function') return null; return FoxBearPlaybackLinkService.registerAudio(audio, meta); }
function unregisterPlaybackLinkedAudio(audio, reason = 'dispose') { if (!audio) return false; getPlaybackSourceRecoveryController()?.releaseAudio?.(audio); let removed = cancelAudioPlaybackRequest(audio, reason); try { removed = FoxBearPlaybackLinkService?.unregisterAudio?.(audio, reason) || removed; } catch (error) { console.warn('Playback link cleanup failed:', error); } return removed; }
...
function init() {
    ...
    runInitStep('작업 방식 선택', initUiModeExperience, { critical: true });
...
function initUiModeExperience() {
    const service = window.FoxBearUiModeService;
    if (!service?.createController) throw new Error('작업 방식 선택 서비스를 불러오지 못했습니다.');
    if (uiModeController) return uiModeController.getSnapshot?.() || null;
    uiModeController = service.createController({
        document,
        onModeChange: mode => {
            state.uiMode = mode;
        },
        onLayoutChange: () => {
            requestAnimationFrame(() => {
                ...
                scheduleBottomPreviewIntegrityCheck('ui-mode-layout-change');
                window.dispatchEvent(new CustomEvent('foxbear:ui-mode-layout-change', { detail: { mode: state.uiMode || '' } }));
            });
        },
...
function cacheElements() {
    const ids = [
        'fileDrop', 'folderDrop', 'fileInput', 'folderInput', 'importStatus', 'uiModeChooser', 'uiModeChooserPanel', 'uiModeChooserClose', 'uiModeAiBtn', 'uiModeExpertBtn', 'uiModeSwitchBtn', 'uiModeSwitchLabel', 'featureDock', 'featureCount', 'featureOpenBtn', 'featureDialog', 'featureDialogClose', 'featureDialogList',
        ...
        'adminAccessDialog', 'adminAccessClose', 'adminAccessForm', 'adminAccessSubmit', 'adminAccessCancel', 'adminAccessStatus', 'adminAccessIdentity', 'adminAccessEmail', 'adminAccessUid', 'adminAccessUidCopy',
...
function sendRealtimePreviewToDock(track, audio) {
    ...
    state.bottomPreviewTransport = {
        ...
        playing: typeof audio._foxbearDesiredPlaying === 'boolean' ? audio._foxbearDesiredPlaying && !audio.ended : !audio.paused && !audio.ended,
        translationMode: state.previewTranslationMode || 'studio',
        capturedAt: Date.now()
    };
    state.bottomPreviewMode = 'original';
    state.bottomPreviewTrackId = track.id;
    renderBottomPreviewDock({ autoPlay: isAudioPlaybackIntended(audio), keepPlaying: isAudioPlaybackIntended(audio) });
...
function updateProcessingHud() {
    ...
    const visibleProgress = running.status === 'done' ? 100 : Math.max(1, quantizeProgressStep(progress, 1));
    ...
    if (el.processingHudText) el.processingHudText.textContent = `${running.name} · ${running.report || '처리 중'} · ${visibleProgress}% 진행 중`;
...
const ACTION_HELP_TEXTS = {
    ...
    zipBtn: '완료된 결과물을 반드시 한 개의 ZIP 파일로 묶어 다운로드합니다. 곡별 저장으로 자동 전환하지 않습니다.',
...
function getAdminAccessController() {
    if (!adminAccessController && window.FoxBearAdminAccessController?.create) adminAccessController = window.FoxBearAdminAccessController.create({ state, elements: el, toggleSettings: toggleMobileNativePanel, openMonitor: openAdminStatsDialog, closeMonitor: closeAdminStatsDialog, updateTrigger: updateAdminStatsTriggerVisibility, updateUi: updateMobileNativeUi, showToast, setFallbackModalState: hardSetModalState });
...
function bindAdminStatsEvents() {
    ...
    if (el.adminStatsTrigger) el.adminStatsTrigger.addEventListener('click', handleAdminStatsTriggerTap);
...
function updateAdminStatsTriggerVisibility() {
    ...
    el.adminStatsTrigger.hidden = !visible;
    el.adminStatsTrigger.setAttribute('aria-hidden', visible ? 'false' : 'true');
    el.adminStatsTrigger.disabled = !visible;
    if (visible) ensureAdminIncidentMonitorStyle().catch(error => console.warn('Admin monitor style preload skipped:', error));
}
function openAdminStatsDialog() {
    if (!el.adminStatsDialog) return;
    adminStatsReturnFocus = document.activeElement;
    const access = getAdminAccessController();
    if (!access?.hasActiveSession?.()) {
        showToast('Google 관리자 인증이 필요합니다. 다시 로그인해주세요.');
        openAdminAccessDialog({ returnFocus: el.mobileNativeQuickToggle || document.activeElement });
        return;
    }
    access.updateSessionUi?.();
...
function closeAdminStatsDialog() {
    ...
    const returnFocus = adminStatsReturnFocus;
    adminStatsReturnFocus = null;
    if (returnFocus && document.body.contains(returnFocus) && !returnFocus.hidden) {
        setTimeout(() => { try { returnFocus.focus({ preventScroll: true }); } catch (_) {} }, 0);
    }
}
...
function refreshFirebaseAdminAccess(bridge = window.FoxBearFirebase) {
    ...
    target.getAdminProfile()
        .then(profile => {
            ...
            state.firebaseAdminEmail = profile?.email || '';
            state.firebaseAdminDisplayName = profile?.displayName || '';
            state.firebaseAdminProvider = profile?.providerId || profile?.authMethod || '';
            state.adminSessionExpiresAt = profile?.expiresAt || '';
            state.firebaseAdminChecked = true;
            state.adminStatsRemoteError = state.firebaseIsAdmin ? '' : `현재 UID(${state.firebaseUserId || '확인 중'})는 활성 관리자 세션이 아닙니다.`;
            getAdminAccessController()?.scheduleSessionCheck?.();
            updateAdminStatsTriggerVisibility();
            updateMobileNativeUi();
        })
        .catch(error => {
...
let adminStatsReturnFocus = null;
function ensureAdminIncidentMonitorStyle() { const lazy = FoxBearBuildInfo.lazyAssets?.adminIncidentMonitor || {}; return window.FoxBearAdminIncidentLoaderService?.loadStyle?.({ styleSrc: lazy.stylePath ? `${lazy.stylePath}?v=${FoxBearBuildInfo.assetVersion}` : '', styleIntegrity: lazy.styleIntegrity || '' }) || Promise.resolve({ reused: true, ready: false }); }
async function ensureAdminIncidentMonitorAssets() {
    if (window.FoxBearAdminIncidentMonitorView?.create) return true;
    const lazy = FoxBearBuildInfo.lazyAssets?.adminIncidentMonitor || {};
    const loader = window.FoxBearAdminIncidentLoaderService;
    if (!loader || typeof loader.load !== 'function') throw new Error('관리자 오류 관리 로더를 사용할 수 없습니다.');
    await loader.load({
        src: ADMIN_INCIDENT_MONITOR_SCRIPT_URL, styleSrc: lazy.stylePath ? `${lazy.stylePath}?v=${FoxBearBuildInfo.assetVersion}` : '', styleIntegrity: lazy.styleIntegrity || '',
        integrity: lazy.scriptIntegrity || '',
        resolveScriptUrl: resolveFoxBearScriptUrl,
        isReady: () => Boolean(window.FoxBearAdminIncidentMonitorView?.create)
    });
    if (!window.FoxBearAdminIncidentMonitorView?.create) throw new Error('관리자 오류 관리 모듈 초기화에 실패했습니다.');
    return true;
}
...
async function renderAdminIncidentsDialog(forceRemote) {
    if (el.adminIncidentsNotice && !window.FoxBearAdminIncidentMonitorView?.create) el.adminIncidentsNotice.textContent = '오류 관리 모듈을 안전하게 불러오는 중입니다.';
    try {
        await ensureAdminIncidentMonitorAssets();
    } catch (error) {
        console.error('Admin incident monitor lazy load failed:', error);
        if (el.adminIncidentsNotice) el.adminIncidentsNotice.textContent = '오류 관리 모듈을 불러오지 못했습니다. 네트워크 상태를 확인한 뒤 다시 시도해주세요.';
        return;
    }
    const controller = getAdminIncidentMonitorViewController();
    if (!controller) {
        if (el.adminIncidentsNotice) el.adminIncidentsNotice.textContent = '오류 관리 화면 모듈 초기화에 실패했습니다.';
        return;
    }
...
function updateMobileNativeUi() {
    ...
    if (el.mobileNativePanel) {
        ...
        setMobileNativeActionState('admin-monitor', state.adminUnlockBusy ? '확인 중' : (getAdminAccessController()?.getSessionLabel?.() || (state.firebaseIsAdmin ? '열기' : (state.adminAccessChecking ? '확인' : '인증'))), state.firebaseIsAdmin);
        const adminMonitorButton = el.mobileNativePanel.querySelector('[data-native-action="admin-monitor"]');
        if (adminMonitorButton) adminMonitorButton.disabled = Boolean(state.adminUnlockBusy || state.adminLogoutBusy);
...
function getDockAudioTrackId(audio) { return String(audio?.dataset?.trackId || ''); }
function syncMediaSessionForDock(audioOverride = null) {
    ...
    const mode = ownerId === String(state.bottomPreviewTrackId || '') ? (state.bottomPreviewMode || 'original') : (audio.dataset?.waveformMode || 'original');
    ...
    try {
        ...
        navigator.mediaSession.playbackState = (typeof audio._foxbearDesiredPlaying === 'boolean' ? audio._foxbearDesiredPlaying && !audio.ended : !audio.paused && !audio.ended) ? 'playing' : 'paused';
        navigator.mediaSession.setActionHandler('play', () => playBottomPreviewAudio());
        navigator.mediaSession.setActionHandler('pause', () => { const current = getBottomPreviewAudio(); const service = typeof getPlaybackTransitionService === 'function' ? getPlaybackTransitionService() : null; if (service?.pauseWithFadeOut) service.pauseWithFadeOut(current, { reason: 'media-session-pause' }); else { if (current) current._foxbearDesiredPlaying = false; try { current?.pause?.(); } catch (error) {} } });
...
}let playbackLifecycleRecoveryController = null;
function getPlaybackLifecycleRecoveryController() {
    if (playbackLifecycleRecoveryController) return playbackLifecycleRecoveryController;
    const factory = window.FoxBearPlaybackLifecycleRecoveryService?.createController; if (!factory) return null;
    playbackLifecycleRecoveryController = factory({ document, getState: () => state, getMobileState: ensureMobileNativeState, getSelectedTrack, getActiveAudio: getBottomPreviewAudio, getPlayerRoot: () => el.bottomPreviewPlayer, captureTransport: captureBottomPreviewTransport, renderDock: renderBottomPreviewDock, scheduleLayout: scheduleBottomPreviewLayoutSync, syncWakeLock: syncWakeLockForCurrentActivity, syncMediaSession: syncMediaSessionForDock, syncPlayButton: syncBottomPreviewExternalPlayButton, unregisterAudio: unregisterPlaybackLinkedAudio, showToast, getTransitionService: getPlaybackTransitionService });
    return playbackLifecycleRecoveryController;
}
function handleMobileVisibilityChange() { return getPlaybackLifecycleRecoveryController()?.handleVisibilityChange?.() || false; } // compatibility: reason: 'visibility-hidden', ttlMs: 12 * 60 * 60 * 1000 · mobile.lastDockRestoreAt
function handleMobilePageShow() { return getPlaybackLifecycleRecoveryController()?.handlePageShow?.() || false; }
function restoreDockTransportAfterReturn(forceNotice = false) { return getPlaybackLifecycleRecoveryController()?.restore?.(forceNotice) || false; }
let pwaRuntimeBridge = null;
function getPwaRuntimeBridge() {
    if (pwaRuntimeBridge) return pwaRuntimeBridge;
    pwaRuntimeBridge = window.FoxBearPwaRuntimeBridge?.createBridge?.({
        navigator,
        location,
        serviceWorkerUrl: SERVICE_WORKER_URL,
        resolveScriptUrl: resolveFoxBearScriptUrl,
        ensureMobileState: ensureMobileNativeState,
        updateMobileUi: updateMobileNativeUi, isWarmCacheSafe: () => !state.busy && !isAnyExportActive() && !state.masterPreviewRenderingTrackId && !state.tracks.some(track => ['analyzing', 'processing'].includes(track.status)),
        recoveryService: window.FoxBearServiceWorkerRecoveryService,
        updateService: window.FoxBearServiceWorkerUpdateService,
        shareService: window.FoxBearPwaShareTargetService
    }) || null;
    return pwaRuntimeBridge;
}
async function registerFoxBearServiceWorker(options = {}) {
    // compatibility anchors: navigator.serviceWorker.register(resolveFoxBearScriptUrl(SERVICE_WORKER_URL)) · navigator.serviceWorker.register('./sw.js?v=1.7.1-reference-match-2-phase1') · navigator.serviceWorker.register('./sw.js?v=1.7.1-reference-match-2-phase1&h=sw-v171')
    return getPwaRuntimeBridge()?.registerServiceWorker?.(options);
}
async function processPwaShareTargetLaunch() {
    return getPwaRuntimeBridge()?.processShareLaunch?.({ validateAudioFile, handleFiles, showToast });
}
...
function createDifferencePreviewPlayer(track, options = {}) {
    ...
    const playBoth = async () => {
        ...
        try {
            ...
            cancelAudioPlaybackRequest(originalAudio, 'difference-preview-failed'); cancelAudioPlaybackRequest(compareAudio, 'difference-preview-failed');
            showToast('차이 듣기 자동 재생이 차단되었습니다. 재생 버튼을 다시 눌러주세요.');
            setPlaying(false);
        }
    };
    const pauseBoth = () => {
        cancelAudioPlaybackRequest(originalAudio, 'difference-preview-pause'); cancelAudioPlaybackRequest(compareAudio, 'difference-preview-pause');
        setPlaying(false);
    };
    toggle.addEventListener('click', () => {
        if (!isAudioPlaybackIntended(compareAudio)) playBoth();
        else pauseBoth();
    });
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.7.1-reference-match-2-phase1', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.7.1-reference-match-2-phase1', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.7.1-reference-match-2-phase1', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.7.1-reference-match-2-phase1', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.7.1-reference-match-2-phase1',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.7.1-reference-match-2-phase1', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
async function preflightImportPlanDecodedMemory(plan) {
    ...
    return service.run(plan, { lowMemoryMaxDecodedPcmBytes: FoxBearRuntimeConfig.LOW_MEMORY_MAX_DECODED_PCM_BYTES, lowMemoryMaxDecodePeakBytes: FoxBearRuntimeConfig.LOW_MEMORY_MAX_DECODE_PEAK_BYTES, standardMaxDecodedPcmBytes: FoxBearRuntimeConfig.STANDARD_MAX_DECODED_PCM_BYTES, standardMaxDecodePeakBytes: FoxBearRuntimeConfig.STANDARD_MAX_DECODE_PEAK_BYTES, lowMemoryMaxUnknownProbeFileBytes: FoxBearRuntimeConfig.LOW_MEMORY_MAX_UNKNOWN_PROBE_FILE_BYTES, standardMaxUnknownProbeFileBytes: FoxBearRuntimeConfig.STANDARD_MAX_UNKNOWN_PROBE_FILE_BYTES, metadataTimeoutMs: FoxBearRuntimeConfig.IMPORT_METADATA_PROBE_TIMEOUT_MS, concurrency: FoxBearRuntimeConfig.IMPORT_METADATA_PROBE_CONCURRENCY, largeBatchThreshold: SAFE_LARGE_IMPORT_BATCH_THRESHOLD, onStatus: updateImportStatus, onProbeError: (error, file) => console.warn('Import memory preflight probe failed:', file?.name, error) });
}
...
async function decodeAudio(file, task = null) {
    ...
    if (task && typeof task.throwIfCancelled === 'function') task.throwIfCancelled();
    const importPolicy = getImportMemoryPolicy();
    const lowMemory = Boolean(importPolicy?.lowMemory);
    return await service.decodeAudioFile(file, {
        latencyHint: 'playback',
        metadataTimeoutMs: 4500,
        signal: task?.signal || null,
        maxDecodedPcmBytes: lowMemory ? FoxBearRuntimeConfig.LOW_MEMORY_MAX_DECODED_PCM_BYTES : FoxBearRuntimeConfig.STANDARD_MAX_DECODED_PCM_BYTES,
        maxDecodePeakBytes: lowMemory ? FoxBearRuntimeConfig.LOW_MEMORY_MAX_DECODE_PEAK_BYTES : FoxBearRuntimeConfig.STANDARD_MAX_DECODE_PEAK_BYTES
    });
}
...
async function analyzeBufferAsync(buffer, task = null) {
    ...
    const channels = Math.max(1, Math.min(buffer.numberOfChannels || 1, 2)), channelBuffers = [];
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.7.1-bulk-pause-skip-reorder-summary-fallback',
...
function quantizeProgressStep(value, step = MASTERING_PROGRESS_VISIBLE_STEP) {
    const n = clamp(Number(value || 0), 0, 100), unit = Math.max(1, Number(step || 1));
    if (n >= 100) return 100;
    return clamp(Math.floor(n / unit) * unit, 0, 99);
}
function refreshMasteringProgressUi(track, reason = 'mastering-progress', options = {}) {
    if (!track) return; if (track.bulkMasteringBatchId || track.bulkImportBatchId) updateBulkImportHud(); updateProcessingHud();
    const whole = Math.floor(clamp(Number(track.progress || 0), 0, 100)), renderStep = whole >= 100 ? 100 : Math.floor(whole / MASTERING_PROGRESS_FULL_RENDER_STEP) * MASTERING_PROGRESS_FULL_RENDER_STEP;
    if ((options.forceRender || whole >= 100 || whole % MASTERING_PROGRESS_FULL_RENDER_STEP === 0) && (options.forceRender || track._lastMasteringFullRenderStep !== renderStep)) { track._lastMasteringFullRenderStep = renderStep; scheduleRenderAll(reason, { keepDetailAudio: true, delayMs: Number.isFinite(Number(options.renderDelayMs)) ? Number(options.renderDelayMs) : SAFE_MASTERING_PROGRESS_RENDER_DELAY_MS, immediate: Boolean(options.immediateRender || whole >= 100) }); }
}
function applyMappedMasteringProgress(track, masteringJobId, rangeStart, rangeEnd, progress, fallbackStage) {
    if (!track || track.masteringJobId !== masteringJobId) return; const workerPercent = clamp(Number(progress?.percent || 0), 0, 100);
    track.progress = Math.max(Number(track.progress || 0), rangeStart + workerPercent * ((rangeEnd - rangeStart) / 100)); track.report = `${progress?.stage || fallbackStage} ${Math.round(workerPercent)}%${progress?.detail ? ` · ${progress.detail}` : ''}`; refreshMasteringProgressUi(track, 'mastering-worker-progress', { renderDelayMs: 160 });
}
function startMasteringProgressHeartbeat(track, capProgress, report = '', options = {}) {
    const cap = Math.min(99, Math.max(0, Math.floor(Number(capProgress || 0)))); if (!track || cap <= Math.floor(Number(track.progress || 0))) return () => {};
    const timer = setInterval(() => { if (!track || track.status !== 'processing' || Math.floor(Number(track.progress || 0)) >= cap) return; track.progress = Math.min(cap, Math.floor(Number(track.progress || 0)) + 1); if (report) track.report = report; refreshMasteringProgressUi(track, options.reason || 'mastering-heartbeat', { renderDelayMs: 180 }); }, Math.max(180, Number(options.intervalMs || MASTERING_PROGRESS_HEARTBEAT_MS)));
    return () => clearInterval(timer);
}
async function setMasteringProgress(track, targetProgress, report = '', options = {}) {
    if (!track) return; const target = clamp(Number(targetProgress || 0), 0, 100); track.progress = target >= 100 ? 100 : Math.max(Number(track.progress || 0), target); if (report) track.report = report;
    refreshMasteringProgressUi(track, 'mastering-progress', { ...options, forceRender: true }); if (!options.noYield) await yieldToBrowser();
}
...
async function runQualityGateRecoveryAttempt(track, context = {}) {
    ...
    try {
        ...
        await sanitizeAudioBufferCooperative(retryMasteredBuffer, 'quality-recovery-master-chain', { yieldFn: yieldToBrowser, throwIfCancelled: () => context.assertMasteringJobActive('quality-recovery-master-sanitize') });
        ...
        await sanitizeAudioBufferCooperative(retryFinalBuffer, 'quality-recovery-finalizer', { yieldFn: yieldToBrowser, throwIfCancelled: () => context.assertMasteringJobActive('quality-recovery-finalizer-sanitize') });
...
async function masterTrack(track, calledFromBatch = false, options = {}) {
    ...
    try {
        ...
        const stopDecodeHeartbeat = startMasteringProgressHeartbeat(track, 9, '오디오 디코딩 및 PCM 준비 중', { reason: 'master-decode-heartbeat', intervalMs: 360 });
        try { currentSourceBuffer = await decodeAudio(track.file, masteringTask); } finally { stopDecodeHeartbeat(); }
        ...
        if (!track.analysis) {
            ...
            const stopAnalysisHeartbeat = startMasteringProgressHeartbeat(track, 24, '마스터링용 긴급 분석 신호를 계산 중', { reason: 'master-analysis-heartbeat' });
            let analysis; try { analysis = await analyzeBufferAsync(currentSourceBuffer, masteringTask); } finally { stopAnalysisHeartbeat(); }
        ...
        track.dcInfo = await removeDcOffsetAudioBufferCooperative(currentSourceBuffer, { yieldFn: yieldToBrowser, throwIfCancelled: () => assertMasteringJobActive('dc-offset'), onProgress: progress => applyMappedMasteringProgress(track, masteringJobId, 30, 34, progress, 'DC offset 정리') });
        await sanitizeAudioBufferCooperative(currentSourceBuffer, 'pre-pitch-cleanup', { yieldFn: yieldToBrowser, throwIfCancelled: () => assertMasteringJobActive('pre-pitch-sanitize'), onProgress: progress => applyMappedMasteringProgress(track, masteringJobId, 34, 39, progress, 'PCM 안전 점검') });
        ...
        preparedBuffer = await preparePitchSpeedBuffer(currentSourceBuffer, track.transform, { signal: masteringTask.signal, jobId: `${masteringJobId}:pitch`, onProgress: progress => { if (!masteringTask.signal?.aborted) applyMappedMasteringProgress(track, masteringJobId, 40, 54, progress, '피치/BPM 변환'); } });
        ...
        let finalization = await finalizeMasterBufferAsync(masteredBuffer, {
            ...
            jobId: `${masteringJobId}:finalizer`, onProgress: progress => { if (!masteringAbortController?.signal?.aborted) applyMappedMasteringProgress(track, masteringJobId, 90, 94, progress, '파이널라이저'); }
        });
        assertMasteringJobActive('finalizer');
        finalBuffer = finalization.buffer;
        await sanitizeAudioBufferCooperative(finalBuffer, 'finalizer', { yieldFn: yieldToBrowser, throwIfCancelled: () => assertMasteringJobActive('finalizer-sanitize'), onProgress: progress => applyMappedMasteringProgress(track, masteringJobId, 94, 95, progress, '최종 PCM 안전 점검') });
        masteredBuffer = null;
        track.waveformOverview = await createWaveformOverviewAsync(preparedBuffer, finalBuffer, track.memoryGovernorInfo?.compactWaveform ? Math.max(36, Math.round(WAVEFORM_OVERVIEW_BINS / 2)) : WAVEFORM_OVERVIEW_BINS, { yieldFn: yieldToBrowser, budgetMs: 10, throwIfCancelled: () => assertMasteringJobActive('waveform-overview'), onProgress: progress => applyMappedMasteringProgress(track, masteringJobId, 95, 95, progress, 'A/B 파형 요약') }); if (state.autoHighlightAB && !track.memoryGovernorInfo?.compactWaveform) track.abHighlightStartSec = estimateABHighlightStartFromPair(preparedBuffer, finalBuffer, track.analysis); else if (track.memoryGovernorInfo?.compactWaveform && Number.isFinite(Number(track.analysis?.abHighlightStartSec))) track.abHighlightStartSec = Number(track.analysis.abHighlightStartSec);
        ...
        let encoded = await encodeMasterOutputAsync(finalBuffer, requestedOutputFormat, {
            signal: masteringAbortController?.signal || null,
            jobId: `${masteringJobId}:encode`, onProgress: progress => { if (!masteringAbortController?.signal?.aborted) applyMappedMasteringProgress(track, masteringJobId, 95, 99, progress, getOutputFormatLabel(requestedOutputFormat)); }
        });
        ...
        if (previousMasteredUrl && previousMasteredUrl !== nextMasteredUrl && !retirePlaybackObjectUrl(track, previousMasteredUrl)) URL.revokeObjectURL(previousMasteredUrl);
        track.status = 'done';
        ...
        setNativeBadge(getCompletedUndownloadedCount());
        stabilizeBottomPreviewDockAfterMastering(track, { preserveOriginalPlayback: preserveOriginalDockPlayback });
        foxBearHaptic('complete');
...
async function downloadZip() { if (isExportQueueActive()) { showToast('곡별 순차 저장을 먼저 취소하거나 완료해 주세요.'); return Object.freeze({ ok: false, conflictingExport: true }); }
    ...
    return zipService.start({
        ...
        downloadBlob,
        fileName: `FoxBear mastered ${timestampForFile()}.zip`,
        singleArchiveRequired: true,
        automaticIndividualFallback: false,
...
function invalidateMasteredOutput(track, report, autoRefresh = false) {
    ...
    if (track.masteredUrl && !retirePlaybackObjectUrl(track, track.masteredUrl)) {
        try { URL.revokeObjectURL(track.masteredUrl); } catch (error) {}
    }
    try { getDownloadService()?.clearDownloadVariantCache?.(track.outBlob); } catch (error) {}
    track.outBlob = null;
    track.outName = '';
...
function releaseTrackResourcesSafely(track) {
    ...
    if (track) { try { getDownloadService()?.clearDownloadVariantCache?.(track.outBlob); } catch (error) {} track.masterPreviewAbortController = null; track.masterPreviewJobId = ''; track.masteredBuffer = null; track.masterPreviewBlob = null; track.masterPreviewInfo = null; track.outBlob = null; }
    return null;
}
function clearQueue() {
...
function makeReferenceTargetFromAnalysis(analysis) {
    ...
    return {
        ...
        crest: Number(analysis.crest ?? 5),
        lowMonoScore: clamp(Number(analysis.lowMonoScore ?? 92), 0, 100),
        spatialExcessRisk: clamp01(Number(analysis.spatialExcessRisk ?? 0)),
        metallicHint: clamp01(Number(analysis.metallicHint ?? 0.40)),
        loudnessIntegrated: Number(analysis.loudnessIntegrated ?? analysis.loudnessHint ?? -18),
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.7.1-kakao-adaptive-memory-governor', selected, recent });
}
...
let bottomPreviewDockIntegrityController = null; function getBottomPreviewDockIntegrityController() { if (!bottomPreviewDockIntegrityController) bottomPreviewDockIntegrityController = window.FoxBearBottomPreviewDockIntegrityService?.createController?.({ state, document, getSelectedTrack, applyTrackToControls, getDock: () => el.bottomPreviewDock, getPlayer: () => el.bottomPreviewPlayer, renderDock: options => renderBottomPreviewDock(options), hideDock: () => { clearBottomPreviewPlayer(); el.bottomPreviewDock?.classList.remove('show'); el.bottomPreviewDock?.setAttribute('aria-hidden', 'true'); document.body?.classList.remove('bottom-preview-active'); syncBottomPreviewFloatingOffset(); syncMediaSessionForDock(null); updateMobileNativeUi(); state.bottomPreviewMode = 'original'; state.bottomPreviewTrackId = null; state.bottomPreviewAutoplayTrackId = null; syncBottomPreviewExternalPlayButton(null, false); } }) || null; return bottomPreviewDockIntegrityController; } function repairActiveTrackSelectionForRender(reason = 'render') { return getBottomPreviewDockIntegrityController()?.repairSelection(reason) || getSelectedTrack(); } function scheduleBottomPreviewIntegrityCheck(reason = 'scheduled') { return getBottomPreviewDockIntegrityController()?.schedule(reason) || 0; }
function renderAll(options = {}) {
    repairActiveTrackSelectionForRender('render-all');
    renderStats();
    renderButtons();
...
function renderButtons() {
    ...
    el.zipBtn.textContent = isZipExportActive() ? 'ZIP 생성 중...' : 'ZIP 다운로드 · 1개 파일';
    if (el.individualExportBtn) { el.individualExportBtn.disabled = !hasCompleted || state.busy || isAnyExportActive(); el.individualExportBtn.textContent = isExportQueueActive() ? '순차 저장 진행 중...' : '곡별 순차 저장 · 별도'; }
...
function renderExportFileNameSummary() { const workflow = window.FoxBearFileNameWorkflowService, policy = window.FoxBearFileNamePolicyService; if (!workflow?.renderExportFileNameSummary || !policy) { if (el.exportNameSummary) el.exportNameSummary.textContent = state.tracks.length ? '파일명 미리보기 준비 중' : ''; return false; } return workflow.renderExportFileNameSummary({ node: el.exportNameSummary, tracks: state.tracks, buildFileName: buildMasteredFileName, fileNamePolicy: policy, exportGuardService: getExportGuardService() }); }
...
function renderDetail(options = {}) { const view = window.FoxBearDetailView; if (!view?.renderDetail) { if (el.trackDetail) el.trackDetail.textContent = ''; return false; } return view.renderDetail(options, getDetailViewDeps()); }
...
function setupPreviewTranslationAudio(audio, options = {}) {
    ...
    try {
        const controller = window.FoxBearPreviewTranslationService?.attach?.(audio, {
            ...
            createContext: () => FoxBearAudioContexts.create({ purpose: 'preview-translation', ownerId: `preview-translation:${options.trackId || audio.dataset.trackId || 'track'}:${Date.now()}`, latencyHint: 'interactive' })
        });
...
let postMasterPlaybackRecoveryController = null;
function getPostMasterPlaybackRecoveryController() {
    if (!postMasterPlaybackRecoveryController && window.FoxBearPostMasterPlaybackRecoveryService?.createController) postMasterPlaybackRecoveryController = window.FoxBearPostMasterPlaybackRecoveryService.createController({ getState: () => state, getPlayerRoot: () => el.bottomPreviewPlayer, getActiveAudio: getBottomPreviewAudio, renderDock: renderBottomPreviewDock, syncExternalPlayButton: syncBottomPreviewExternalPlayButton, playWithFadeIn: playAudioWithFadeIn, requestAnimationFrame });
    return postMasterPlaybackRecoveryController;
}
function warmBottomPreviewAudio(audio) { return getPostMasterPlaybackRecoveryController()?.warmAudio(audio) || false; }
function stabilizeBottomPreviewDockAfterMastering(track = getSelectedTrack(), options = {}) { return getPostMasterPlaybackRecoveryController()?.stabilizeAfterMastering(track, options) || false; }
...
function getBottomPreviewPlayerOwner(audio = getBottomPreviewAudio()) { return getPostMasterPlaybackRecoveryController()?.getPlayerOwner(audio) || null; }
function showBottomPreviewPlaybackFailure(error = null) { if (error) console.warn('FoxBear Dock playback start failed:', error); showToast('재생 준비가 지연됐습니다. Dock 재생 버튼을 한 번 더 눌러주세요.'); }
function startBottomPreviewPlayback(audio = getBottomPreviewAudio(), options = {}) { return getPostMasterPlaybackRecoveryController()?.startPlayback(audio, options) || Promise.resolve(false); }
function toggleBottomPreviewExternalPlayback(event = null) {
    event?.preventDefault?.(); event?.stopPropagation?.();
    const audio = getBottomPreviewAudio();
    if (!audio || !isAudioPlaybackIntended(audio)) playBottomPreviewAudio({ userGesture: true }).then(started => { if (!started) showBottomPreviewPlaybackFailure(); }).catch(showBottomPreviewPlaybackFailure);
...
function createDockIntegratedWaveformPlayer(track, options = {}) {
    ...
    if (mode === 'mastered' || mode === 'masterPreview') audio.preload = 'auto';
    ...
    toggle.addEventListener('click', event => {
        event.preventDefault(); event.stopPropagation();
        if (!isAudioPlaybackIntended(audio)) startBottomPreviewPlayback(audio, { userGesture: true }).then(started => { if (!started) showBottomPreviewPlaybackFailure(); }).catch(showBottomPreviewPlaybackFailure);
        else pauseAudioWithFadeOut(audio);
    });
    let sourceRecoveryPromise = null;
    const recoverPlaybackSource = reason => {
        if (sourceRecoveryPromise || audio._foxbearSourceRecoveryDisposed || audio.isConnected === false) return sourceRecoveryPromise || Promise.resolve(false);
        const transportPlaying = Boolean(state.bottomPreviewTransport && String(state.bottomPreviewTransport.trackId || '') === String(track?.id || '') && state.bottomPreviewTransport.playing); wrap.classList.add('playback-source-recovering');
        sourceRecoveryPromise = Promise.resolve(getPlaybackSourceRecoveryController()?.repairAudio?.(audio, { track, mode, reason, shouldResume: isAudioPlaybackIntended(audio) || transportPlaying, readyTimeoutMs: 2600 })).then(result => { const recovered = Boolean(result?.recovered); wrap.classList.toggle('playback-source-error', !recovered); setPlaying(Boolean(recovered && result.resumed && !audio.paused)); if (recovered) sync(); else syncBottomPreviewExternalPlayButton(audio, false); return recovered; }).catch(error => { console.warn('FoxBear playback source recovery failed:', error); wrap.classList.add('playback-source-error'); setPlaying(false); return false; }).finally(() => { wrap.classList.remove('playback-source-recovering'); sourceRecoveryPromise = null; });
        return sourceRecoveryPromise;
    };
    audio.addEventListener('loadedmetadata', () => { getPlaybackSourceRecoveryController()?.markHealthy?.(audio); wrap.classList.remove('playback-source-error'); applyBottomPreviewStart(audio, Number(options.startSec || 0)); sync(); });
    audio.addEventListener('canplay', () => { getPlaybackSourceRecoveryController()?.markHealthy?.(audio); wrap.classList.remove('playback-source-error'); });
    audio.addEventListener('emptied', () => { if ((audio.getAttribute('src') || audio.src) && !audio._foxbearSourceRecoveryPending && isAudioPlaybackIntended(audio)) recoverPlaybackSource('dock-source-emptied'); });
    audio.addEventListener('error', () => { wrap.classList.add('playback-source-error'); recoverPlaybackSource('dock-source-error'); });
    audio.addEventListener('play', () => {
        ...
        getPlaybackTransitionService()?.reconcileAudibleVolume?.(audio, 'dock-play-event');
        setPlaying(true);
        onDockAudioTransportEvent(audio);
        sync();
    });
    audio.addEventListener('pause', () => {
        getPlaybackLifecycleRecoveryController()?.handleUnexpectedPause?.(audio, 'dock-visible-native-pause'); setPlaying(false);
        onDockAudioTransportEvent(audio);
        sync();
    });
    audio.addEventListener('ended', () => {
    ...
    wrap._foxbearPlay = (playOptions = {}) => isAudioPlaybackIntended(audio) ? Promise.resolve(true) : startBottomPreviewPlayback(audio, { readyTimeoutMs: 2200, ...playOptions });
    wrap._foxbearPause = () => {
...
function renderBottomPreviewDock(options = {}) {
    ...
    const track = getSelectedTrack() || repairActiveTrackSelectionForRender('dock-render');
    if (!track) {
        clearBottomPreviewPlayer();
    ...
    const key = `${track.id}|${mode}|${src || ''}|${state.abLoopMode && !useMasterPreview ? 'loop' : 'free'}|dock-clean|wave:${waveformSignature}`; el.bottomPreviewPlayer.dataset.trackId = String(track.id);
    ...
    const canCrossfadeDock = Boolean(options.userGesture && transitionOldAudio && isAudioPlaybackIntended(transitionOldAudio) && src);
    ...
    if (shouldAutoPlay) {
        ...
        if (pendingCrossfade) {
            const runCrossfade = () => {
                let settled = false;
                const settleCrossfade = (completed, settleOptions = {}) => {
                    if (settled) return Boolean(completed);
                    settled = true; pendingCrossfade.oldPlayer?.remove?.(); el.bottomPreviewPlayer?.classList.remove('is-crossfading');
                    if (pendingCrossfade.nextPlayer) delete pendingCrossfade.nextPlayer._foxbearCrossfadeOldAudio;
                    const activeAudio = getBottomPreviewAudio(), nextIsActive = activeAudio === pendingCrossfade.nextAudio && pendingCrossfade.nextAudio?.isConnected !== false;
                    syncBottomPreviewExternalPlayButton(nextIsActive ? pendingCrossfade.nextAudio : activeAudio, Boolean(completed && nextIsActive && !pendingCrossfade.nextAudio.paused));
                    return settleOptions.retry && nextIsActive ? playBottomPreviewAudio({ userGesture: Boolean(settleOptions.userGesture), silent: true }) : Boolean(completed);
                };
                Promise.resolve(crossfadeAudioPair(pendingCrossfade.oldAudio, pendingCrossfade.nextAudio, {
                    userGesture: Boolean(options.userGesture),
                    onComplete: () => settleCrossfade(true)
                })).then(completed => {
                    if (!completed) settleCrossfade(false);
                }).catch(error => {
                    console.warn('FoxBear Dock crossfade failed:', error);
                    settleCrossfade(false, { retry: true, userGesture: Boolean(options.userGesture) });
                });
            };
    ...
    syncMediaSessionForDock();
    updateMobileNativeUi();
    if (!options.skipIntegritySchedule) scheduleBottomPreviewIntegrityCheck(options.integrityRepair ? 'render-repair' : 'render');
}
function scheduleBottomPreviewLayoutSync() {
    ...
    state.bottomPreviewLayoutRaf = requestAnimationFrame(() => {
        ...
        syncBottomPreviewFloatingOffset();
        scheduleBottomPreviewIntegrityCheck('layout-sync');
    });
}
...
function clearBottomPreviewPlayer() {
    ...
    el.bottomPreviewPlayer.querySelectorAll('audio').forEach(audio => {
        cancelAudioPlaybackRequest(audio, 'bottom-preview-clear');
    ...
    delete el.bottomPreviewPlayer.dataset.previewKey; delete el.bottomPreviewPlayer.dataset.trackId; delete el.bottomPreviewPlayer.dataset.previewMode;
}
function playBottomPreviewAudio(options = {}) {
    let audio = getBottomPreviewAudio();
    if (!audio || audio.isConnected === false) {
        renderBottomPreviewDock({ keepPlaying: true });
        audio = getBottomPreviewAudio();
    }
    if (!audio) return Promise.resolve(false);
    const player = getBottomPreviewPlayerOwner(audio);
    const request = player && typeof player._foxbearPlay === 'function'
        ? player._foxbearPlay(options)
        : startBottomPreviewPlayback(audio, options);
    return Promise.resolve(request).then(Boolean).catch(error => {
        if (!options.silent) console.warn('FoxBear bottom preview playback failed:', error);
        return false;
    });
}
...
function createPreviewPlayer(src, gainDb = 0, knownDurationSec = 0, loopCompare = false, loopStartHint = NaN, options = {}) {
    ...
    toggle.addEventListener('click', () => {
        if (!isAudioPlaybackIntended(audio)) playAudioWithFadeIn(audio).catch(() => showToast('브라우저가 재생을 차단했습니다. 다시 눌러주세요.'));
        else pauseAudioWithFadeOut(audio);
    });
...
function bindExclusivePreview(audio, options = {}) {
    ...
    document.querySelectorAll('.custom-player audio, .ab-switch-deck audio, .difference-preview-player audio, audio[data-preview-system]').forEach(other => {
        if (other !== audio && !allowed.has(other)) cancelAudioPlaybackRequest(other, 'legacy-exclusive-preview');
    });
}
function createABSwitchPlayer(track) {
    ...
    const switchTo = (next, options = {}) => {
        ...
        const wasPlaying = isAudioPlaybackIntended(oldAudio);
    ...
    play.addEventListener('click', () => {
        const audio = activeAudio();
        if (!isAudioPlaybackIntended(audio)) {
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.7.1',
...
function safeBaseName(fileName) {
    return getFileNamePolicyService().sanitizeTitle(fileName || 'track', {
        fallback: 'track',
