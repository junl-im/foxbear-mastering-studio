// FoxBear AI Mastering Studio Pro v1.6.65 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.65';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.65-firestore-write-fencing'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1665-write-fencing'}`;
...
const SHARED_DSP_PROFILE_VERSION = 'v1.6.65-firestore-write-fencing';
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.6.65-firestore-write-fencing') · navigator.serviceWorker.register('./sw.js?v=1.6.65-firestore-write-fencing&h=sw-v1665-write-fencing')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.65-firestore-write-fencing', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.6.65-firestore-write-fencing',
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.65-firestore-write-fencing', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.65-firestore-write-fencing', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.65-firestore-write-fencing', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.65-firestore-write-fencing',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.65-firestore-write-fencing', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.65-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.65-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.65',
