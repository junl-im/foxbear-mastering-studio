// FoxBear AI Mastering Studio Pro v1.5.24 - app slim-down orchestration bridge
'use strict';
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.24';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.5.24-e2e-responsive-preview-control'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1524'}`;
...
async function registerFoxBearServiceWorker() {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.5.24-e2e-responsive-preview-control') · navigator.serviceWorker.register('./sw.js?v=1.5.24-e2e-responsive-preview-control&h=sw-v1524')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.24-e2e-responsive-preview-control', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.24-e2e-responsive-preview-control',
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.24-e2e-responsive-preview-control', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.24-e2e-responsive-preview-control', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.24-e2e-responsive-preview-control',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.24-e2e-responsive-preview-control', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.24',
