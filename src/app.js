// FoxBear AI Mastering Studio Pro v1.5.92 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.92';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.5.92-python-bytecode-ci-hygiene-node24-cache'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1592'}`;
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.5.92-python-bytecode-ci-hygiene-node24-cache') · navigator.serviceWorker.register('./sw.js?v=1.5.92-python-bytecode-ci-hygiene-node24-cache&h=sw-v1592')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.92-python-bytecode-ci-hygiene-node24-cache', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.92-python-bytecode-ci-hygiene-node24-cache',
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.5.92-python-bytecode-ci-hygiene-node24-cache', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.92-python-bytecode-ci-hygiene-node24-cache', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.92-python-bytecode-ci-hygiene-node24-cache', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.92-python-bytecode-ci-hygiene-node24-cache',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.92-python-bytecode-ci-hygiene-node24-cache', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.5.92-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.5.92-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.92',
