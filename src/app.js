// FoxBear AI Mastering Studio Pro v1.5.93 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.5.93';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.5.93-external-engine-transfer-admin-export-openai-readiness'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1593'}`;
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.5.93-external-engine-transfer-admin-export-openai-readiness') · navigator.serviceWorker.register('./sw.js?v=1.5.93-external-engine-transfer-admin-export-openai-readiness&h=sw-v1593')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.5.93-external-engine-transfer-admin-export-openai-readiness', total: 0, pending: 0, active: 0, fallback: true });
}
...
function getMasteringQueueSnapshot() {
    ...
    return Object.freeze({
        version: '1.5.93-external-engine-transfer-admin-export-openai-readiness',
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.5.93-external-engine-transfer-admin-export-openai-readiness', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.5.93-external-engine-transfer-admin-export-openai-readiness', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.5.93-external-engine-transfer-admin-export-openai-readiness', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.5.93-external-engine-transfer-admin-export-openai-readiness',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.5.93-external-engine-transfer-admin-export-openai-readiness', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.5.93-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.5.93-kakao-adaptive-memory-governor', selected, recent });
}
...
async function renderMasterPreviewForTrack(track, options = {}) {
    ...
    const previewJob = previewJobs.create(track, { label: 'master-preview', supersedeReason: 'master-preview-restarted' });
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.5.93',
