// FoxBear AI Mastering Studio Pro v1.6.0 - app slim-down orchestration bridge
...
const FoxBearBuildInfo = window.FoxBearBuildInfo || {}; const APP_VERSION = 'Pro v1.6.0';
...
const SERVICE_WORKER_URL = `./sw.js?v=${FoxBearBuildInfo.assetVersion || '1.6.0-incident-mail-pipeline-health'}&h=${FoxBearBuildInfo.serviceWorkerRevision || 'sw-v1600'}`;
...
async function registerFoxBearServiceWorker(options = {}) {
    ...
    try {
        // compatibility anchors: navigator.serviceWorker.register('./sw.js?v=1.6.0-incident-mail-pipeline-health') · navigator.serviceWorker.register('./sw.js?v=1.6.0-incident-mail-pipeline-health&h=sw-v1600')
...
function getBulkImportHudSnapshot() {
    const view = getBulkImportHudView();
    return view && typeof view.getSnapshot === 'function' ? view.getSnapshot() : Object.freeze({ version: '1.6.0-incident-mail-pipeline-health', total: 0, pending: 0, active: 0, fallback: true });
}
...
window.FoxBearMasteringDiagnostics = Object.freeze({ version: '1.6.0-incident-mail-pipeline-health', getSnapshot: getMasteringPerformanceSnapshot });
...
function getMemoryGuardSnapshot() {
    const service = getMemoryGuardService();
    if (!service || typeof service.getSnapshot !== 'function') return Object.freeze({ version: 'v1.6.0-incident-mail-pipeline-health', unavailable: true, trackCount: state.tracks.length });
...
function diagnoseCompletedMasteringMemory(reason = 'manual-diagnostic') {
    const service = getMemoryGuardService();
    if (!service || typeof service.diagnoseCompletedBatch !== 'function') return Object.freeze({ version: 'v1.6.0-incident-mail-pipeline-health', unavailable: true });
...
window.FoxBearMemoryGuard = Object.freeze({
    version: 'v1.6.0-incident-mail-pipeline-health',
...
window.FoxBearExportGuard = Object.freeze({ version: 'v1.6.0-incident-mail-pipeline-health', getReadiness: () => getExportGuardService()?.getExportReadiness?.(state.tracks, { memorySnapshot: getMemoryGuardSnapshot() }) || null, getDiagnostics: () => getExportGuardService()?.getDiagnostics?.() || [] });
...
function getMasteringBatchRunner() {
    ...
    if (service && typeof service.createMasteringBatchRunner === 'function') {
        ...
        masteringBatchRunner = Object.freeze({
            version: '1.6.0-bulk-pause-skip-reorder-summary-fallback',
...
function getMasteringPerformanceSnapshot() {
    ...
    return Object.freeze({ version: '1.6.0-kakao-adaptive-memory-governor', selected, recent });
}
...
function createExportReport(track) {
    return {
        app: 'FoxBear AI Mastering Studio Pro v1.6.0',
