...
assert(html.includes('data-build="1.5.32"'), 'index build should be 1.5.32');
assert(config.includes("ASSET_VERSION = '1.5.32-kakao-external-browser-local-flow'"), 'asset key should be v1.5.32 bulk import guard');
assert(sw.includes("foxbear-shell-v1.5.32-kakao-external-browser-local-flow"), 'service worker cache should be v1.5.32 bulk import guard');
