...
assert(decode.includes("SERVICE_VERSION = '1.6.86-header-order-mobile-overflow-browser-gate-recovery'"), 'decode service should be bumped');
...
assert(index.includes('src/audio/audio-decode-service.js?v=1.6.86-header-order-mobile-overflow-browser-gate-recovery'), 'index should load versioned decode service');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.6.86-header-order-mobile-overflow-browser-gate-recovery'), 'service worker should precache decode service');
assert(pkg.qaChecks.includes('node qa/v1423_audio_decode_memory_guard_smoke.js'), 'package QA should include v1.6.86 smoke');
assert(matrix.includes('Audio Decode Memory Guard'), 'matrix should document v1.6.86 audio decode guard scope');
