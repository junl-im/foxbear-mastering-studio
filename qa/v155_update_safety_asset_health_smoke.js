...
const BOOT_KEY = ['h=boot-sri-v1551','h=boot-sri-v156','h=boot-sri-v155'].find(key => index.includes(key));
const UPDATE_KEY = ['h=update-safety-v1551','h=update-safety-v156','h=update-safety-v155'].find(key => index.includes(key));
assert(index.includes(`src/boot/runtime-health.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'runtime health boot key missing from index');
assert(index.includes(`src/boot/performance-diagnostics.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'performance diagnostics boot key missing from index');
assert(index.includes(`src/app.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'app boot key missing from index');
assert(index.includes(`src/boot/update-safety-service.js?v=1.5.51-ci-runtime-contract-hardening&${UPDATE_KEY}`), 'update safety service missing from index');
assert(sw.includes(`./src/boot/update-safety-service.js?v=1.5.51-ci-runtime-contract-hardening&${UPDATE_KEY}`), 'update safety service missing from service worker precache');
assert(sw.includes(`./src/boot/runtime-health.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'runtime health boot key missing from service worker');
assert(sw.includes(`./src/boot/performance-diagnostics.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'performance diagnostics boot key missing from service worker');
assert(sw.includes(`./src/app.js?v=1.5.51-ci-runtime-contract-hardening&${BOOT_KEY}`), 'app boot key missing from service worker');
assert(sw.includes("foxbear-shell-v1.5.5-update-safety") || sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") || sw.includes("foxbear-shell-v1.5.51-ci-runtime-contract-hardening"), 'service worker cache generation should include v1.5.5+ generation');
...
assert(['boot-sri-v155','boot-sri-v156','boot-sri-v1551'].some(key => updateSafety.includes(key)) && updateSafety.includes('foxbear:update-safety-risk'), 'update safety should classify boot key risk and dispatch risk events');
...
assert(['sw-v155','sw-v156','sw-v1551'].some(key => app.includes(`./sw.js?v=1.5.51-ci-runtime-contract-hardening&h=${key}`)), 'app should register service worker with v1.5.5+ cache-bust key');
