...
const BOOT_KEY = index.includes('h=boot-sri-v156') ? 'h=boot-sri-v156' : 'h=boot-sri-v155';
const UPDATE_KEY = index.includes('h=update-safety-v156') ? 'h=update-safety-v156' : 'h=update-safety-v155';
assert(index.includes(`src/boot/runtime-health.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'runtime health boot key missing from index');
assert(index.includes(`src/boot/performance-diagnostics.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'performance diagnostics boot key missing from index');
assert(index.includes(`src/app.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'app boot key missing from index');
assert(index.includes(`src/boot/update-safety-service.js?v=1.4.26-wake-lock-state-sync&${UPDATE_KEY}`), 'update safety service missing from index');
assert(sw.includes(`./src/boot/update-safety-service.js?v=1.4.26-wake-lock-state-sync&${UPDATE_KEY}`), 'update safety service missing from service worker precache');
assert(sw.includes(`./src/boot/runtime-health.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'runtime health boot key missing from service worker');
assert(sw.includes(`./src/boot/performance-diagnostics.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'performance diagnostics boot key missing from service worker');
assert(sw.includes(`./src/app.js?v=1.4.26-wake-lock-state-sync&${BOOT_KEY}`), 'app boot key missing from service worker');
assert(sw.includes("foxbear-shell-v1.5.5-update-safety") || sw.includes("foxbear-shell-v1.5.6-export-progress-recovery"), 'service worker cache generation should include v1.5.5+ generation');
...
assert((updateSafety.includes('boot-sri-v155') || updateSafety.includes('boot-sri-v156')) && updateSafety.includes('foxbear:update-safety-risk'), 'update safety should classify boot key risk and dispatch risk events');
...
assert(app.includes("./sw.js?v=1.4.26-wake-lock-state-sync&h=sw-v155") || app.includes("./sw.js?v=1.4.26-wake-lock-state-sync&h=sw-v156"), 'app should register service worker with v1.5.5+ cache-bust key');
...
assert(qaReport.includes('176/176 PASS') || qaReport.includes('178/178 PASS'), 'QA report missing 176/176+ PASS target');
