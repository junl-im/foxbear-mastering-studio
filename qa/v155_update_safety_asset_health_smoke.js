...
assert(index.includes(`src/boot/runtime-health.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'runtime health boot key missing from index');
assert(index.includes(`src/boot/performance-diagnostics.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'performance diagnostics boot key missing from index');
assert(index.includes(`src/app.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'app boot key missing from index');
assert(index.includes(`src/boot/update-safety-service.js?v=1.6.20-incident-background-sync-network-decay&${UPDATE_KEY}`), 'update safety service missing from index');
assert(sw.includes(`./src/boot/update-safety-service.js?v=1.6.20-incident-background-sync-network-decay&${UPDATE_KEY}`), 'update safety service missing from service worker precache');
assert(sw.includes(`./src/boot/runtime-health.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'runtime health boot key missing from service worker');
assert(sw.includes(`./src/boot/performance-diagnostics.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'performance diagnostics boot key missing from service worker');
assert(sw.includes(`./src/app.js?v=1.6.20-incident-background-sync-network-decay&${BOOT_KEY}`), 'app boot key missing from service worker');
assert(sw.includes("foxbear-shell-v1.5.5-update-safety") || sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") || sw.includes("foxbear-shell-v1.6.20-incident-background-sync-network-decay"), 'service worker cache generation should include v1.5.5+ generation');
...
assert(['sw-v155','sw-v156','sw-v1618'].some(key => app.includes(`./sw.js?v=1.6.20-incident-background-sync-network-decay&h=${key}`)), 'app should register service worker with v1.5.5+ cache-bust key');
