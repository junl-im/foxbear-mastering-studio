...
assert(index.includes('src/boot/service-worker-recovery-service.js?v=1.6.88-mobile-header-grid-ownership-recovery'), 'service worker recovery module is not loaded');
assert(sw.includes('./src/boot/service-worker-recovery-service.js?v=1.6.88-mobile-header-grid-ownership-recovery'), 'service worker recovery module is not cached');
...
async function exerciseServiceWorkerGenerationIsolation() {
  ...
  const cachesApi={
    ...
    async keys(){return ['foxbear-shell-v1.5.30-inapp-playback-recovery','foxbear-shell-v1.6.88-mobile-header-grid-ownership-recovery'];},
