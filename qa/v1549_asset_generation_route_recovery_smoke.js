...
assert(sw.includes('const REQUIRED_INSTALL_ASSETS = CORE_ASSETS.filter'), 'service worker minimum recovery install graph is not derived from core assets');
assert(sw.includes('const WARM_ASSETS = CORE_ASSETS.filter') && sw.includes('!REQUIRED_INSTALL_ASSET_SET.has(asset)'), 'post-activation warm graph is missing');
...
assert(index.includes('src/boot/service-worker-recovery-service.js?v=1.6.109-mastering-cooperative-pcm-responsiveness'), 'service worker recovery module is not loaded');
assert(sw.includes('./src/boot/service-worker-recovery-service.js?v=1.6.109-mastering-cooperative-pcm-responsiveness'), 'service worker recovery module is not cached');
...
async function exerciseServiceWorkerGenerationIsolation() {
  ...
  const cachesApi={
    ...
    async keys(){return ['foxbear-shell-v1.5.30-inapp-playback-recovery','foxbear-shell-v1.6.109-mastering-cooperative-pcm-responsiveness'];},
  ...
  vm.runInContext(`${sw}\n;globalThis.__v1549={REQUIRED_INSTALL_ASSETS,WARM_ASSETS,networkFirstNoFallbackOnIntegrityAssets,networkFirstNavigation};`,context,{filename:'sw.js'});
  ...
  bootRefs.forEach(path=>assert(api.REQUIRED_INSTALL_ASSETS.includes(`./${path}`) || api.WARM_ASSETS.includes(`./${path}`),`boot asset missing from install/warm core graph: ${path}`));
