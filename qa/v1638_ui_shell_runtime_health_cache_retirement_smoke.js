...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'latest rollback shell must remain available');
