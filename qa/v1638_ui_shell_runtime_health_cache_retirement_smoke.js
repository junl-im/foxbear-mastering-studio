...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.7.2-expert-workspace-default-entry'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.7.2-expert-workspace-default-entry'), 'latest rollback shell must remain available');
