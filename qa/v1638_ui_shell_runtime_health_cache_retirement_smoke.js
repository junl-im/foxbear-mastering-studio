...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.44-google-auth-gapi-module-trusted-types-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.44-google-auth-gapi-module-trusted-types-recovery'), 'latest rollback shell must remain available');
