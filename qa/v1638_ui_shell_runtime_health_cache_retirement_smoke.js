...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'), 'latest rollback shell must remain available');
