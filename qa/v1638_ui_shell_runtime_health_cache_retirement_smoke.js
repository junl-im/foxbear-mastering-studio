...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.78-release-generation-assist-url-playback-invalidation'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.78-release-generation-assist-url-playback-invalidation'), 'latest rollback shell must remain available');
