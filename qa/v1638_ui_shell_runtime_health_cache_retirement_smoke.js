...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.98-spectrum-retirement-mobile-header-integrity'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.98-spectrum-retirement-mobile-header-integrity'), 'latest rollback shell must remain available');
