...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.101-admin-lazyload-repo-cleanup'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.101-admin-lazyload-repo-cleanup'), 'latest rollback shell must remain available');
