...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.87-mobile-header-device-label-overlap-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.87-mobile-header-device-label-overlap-recovery'), 'latest rollback shell must remain available');
