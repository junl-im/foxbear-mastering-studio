...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.102-admin-lazyload-sw-hygiene-hardening'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.102-admin-lazyload-sw-hygiene-hardening'), 'latest rollback shell must remain available');
