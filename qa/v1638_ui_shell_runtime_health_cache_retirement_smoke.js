...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.89-mobile-header-flex-ownership-browser-gate-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.89-mobile-header-flex-ownership-browser-gate-recovery'), 'latest rollback shell must remain available');
