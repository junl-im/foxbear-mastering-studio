...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.91-runtime-health-hidden-geometry-contract-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.91-runtime-health-hidden-geometry-contract-recovery'), 'latest rollback shell must remain available');
