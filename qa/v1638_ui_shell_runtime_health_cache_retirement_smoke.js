...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.47-external-host-admin-auth-opaque-error-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.47-external-host-admin-auth-opaque-error-recovery'), 'latest rollback shell must remain available');
