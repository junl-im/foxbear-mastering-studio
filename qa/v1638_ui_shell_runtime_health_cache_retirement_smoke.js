...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.86-header-order-mobile-overflow-browser-gate-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.86-header-order-mobile-overflow-browser-gate-recovery'), 'latest rollback shell must remain available');
