...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.72-ci-safe-hygiene-self-repair'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.72-ci-safe-hygiene-self-repair'), 'latest rollback shell must remain available');
