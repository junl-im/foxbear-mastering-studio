...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.93-mobile-dock-visibility-integrity-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.93-mobile-dock-visibility-integrity-recovery'), 'latest rollback shell must remain available');
