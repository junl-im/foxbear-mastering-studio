...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.104-boot-emergency-visit-privacy-hardening'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.104-boot-emergency-visit-privacy-hardening'), 'latest rollback shell must remain available');
