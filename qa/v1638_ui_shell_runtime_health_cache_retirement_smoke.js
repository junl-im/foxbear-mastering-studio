...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.7.4-reload-reentry-mode-chooser'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.7.4-reload-reentry-mode-chooser'), 'latest rollback shell must remain available');
