...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.88-mobile-header-grid-ownership-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.88-mobile-header-grid-ownership-recovery'), 'latest rollback shell must remain available');
