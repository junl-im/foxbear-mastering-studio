...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.40-ui-shell-retry-replacement-settlement'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.40-ui-shell-retry-replacement-settlement'), 'latest rollback shell must remain available');
