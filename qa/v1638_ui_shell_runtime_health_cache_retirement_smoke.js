...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.92-spectrum-panel-mount-lifecycle-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.92-spectrum-panel-mount-lifecycle-recovery'), 'latest rollback shell must remain available');
