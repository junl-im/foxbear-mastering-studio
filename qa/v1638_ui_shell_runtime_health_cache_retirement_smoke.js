...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.76-download-viewport-runtime-fault-diagnostics'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.76-download-viewport-runtime-fault-diagnostics'), 'latest rollback shell must remain available');
