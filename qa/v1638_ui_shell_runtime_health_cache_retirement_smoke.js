...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.70-share-retry-policy-drift-ci-efficiency'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.70-share-retry-policy-drift-ci-efficiency'), 'latest rollback shell must remain available');
