...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.103-ci-hygiene-mail-routing-hardening'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.103-ci-hygiene-mail-routing-hardening'), 'latest rollback shell must remain available');
