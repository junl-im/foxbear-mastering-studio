...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.43-google-auth-trusted-types-csp-recovery'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.43-google-auth-trusted-types-csp-recovery'), 'latest rollback shell must remain available');
