...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.39-ui-shell-partial-script-probe-isolation'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.39-ui-shell-partial-script-probe-isolation'), 'latest rollback shell must remain available');
