...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.100-sw-stereo-ci-cleanup-hardening'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.100-sw-stereo-ci-cleanup-hardening'), 'latest rollback shell must remain available');
