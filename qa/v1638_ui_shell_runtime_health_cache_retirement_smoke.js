...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.7.1-reference-match-2-phase1'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.7.1-reference-match-2-phase1'), 'latest rollback shell must remain available');
