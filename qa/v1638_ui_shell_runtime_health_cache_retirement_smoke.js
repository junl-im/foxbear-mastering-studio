...
(async () => {
  ...
  const cacheNames = new Set([
    ...
    'foxbear-shell-v1.6.113-incident-finalizer-p1-hardening'
  ]);
  const deleted = [];
  const client = {
    id: 'client-current',
  ...
  assert(!deleted.includes('foxbear-shell-v1.6.113-incident-finalizer-p1-hardening'), 'latest rollback shell must remain available');
