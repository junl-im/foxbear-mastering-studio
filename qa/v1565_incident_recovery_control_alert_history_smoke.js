...
assert.strictEqual(meta.assetVersion, '1.5.66-incident-operations-action-guard-deploy-verification');
...
]) assert(functionsSource.includes(token), `v1.5.66 function contract missing ${token}`);
...
]) assert(firebaseSource.includes(token), `v1.5.66 Firebase bridge contract missing ${token}`);
...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.5.66 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.5.66 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.5.66 admin monitor missing ${token}`);
