...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.6.63 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.6.63 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.6.63 admin monitor missing ${token}`);
