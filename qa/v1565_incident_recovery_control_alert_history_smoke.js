...
assert.strictEqual(meta.assetVersion, '1.5.67-incident-admin-audit-webhook-failover-index-health');
...
]) assert(functionsSource.includes(token), `v1.5.67 function contract missing ${token}`);
...
]) assert(firebaseSource.includes(token), `v1.5.67 Firebase bridge contract missing ${token}`);
...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.5.67 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.5.67 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.5.67 admin monitor missing ${token}`);
