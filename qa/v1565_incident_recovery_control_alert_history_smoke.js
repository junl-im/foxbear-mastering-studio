...
assert.strictEqual(pkg.version, '1.5.68');
assert.strictEqual(meta.assetVersion, '1.5.68-mail-delivery-verification-sender-subject-rules');
...
]) assert(functionsSource.includes(token), `v1.5.68 function contract missing ${token}`);
...
]) assert(firebaseSource.includes(token), `v1.5.68 Firebase bridge contract missing ${token}`);
...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.5.68 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.5.68 app element cache missing ${token}`);
}

for (const token of [
  "makeSummaryCard('보조 경보'",
  "makeSummaryCard('자동 복구'",
  "makeSummaryCard('24시간 추세'",
  'requestBatchRecovery',
  "requestIncidentBatchRecovery(mode)",
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.5.68 admin monitor missing ${token}`);
