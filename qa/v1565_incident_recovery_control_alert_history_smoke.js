...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.6.13 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.6.13 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.6.13 admin monitor missing ${token}`);
