...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.7.2 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.7.2 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.7.2 admin monitor missing ${token}`);
