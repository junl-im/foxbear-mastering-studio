...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.6.100 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.6.100 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.6.100 admin monitor missing ${token}`);
