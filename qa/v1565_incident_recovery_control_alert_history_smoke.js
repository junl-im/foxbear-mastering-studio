...
assert.strictEqual(meta.assetVersion, '1.5.65-incident-recovery-control-alert-history');
...
]) assert(functionsSource.includes(token), `v1.5.65 function contract missing ${token}`);
...
]) assert(firebaseSource.includes(token), `v1.5.65 Firebase bridge contract missing ${token}`);
...
for (const token of [
  ...
  assert(indexSource.includes(`id="${token}"`), `v1.5.65 HTML missing ${token}`);
  assert(appSource.includes(`'${token}'`), `v1.5.65 app element cache missing ${token}`);
}
...
for (const token of [
  ...
  'getIncidentBatchRecoveryRequest',
  'summarizeHistory'
]) assert(monitorSource.includes(token), `v1.5.65 admin monitor missing ${token}`);
