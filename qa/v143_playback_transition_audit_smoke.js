...
must(sw.includes(`foxbear-shell-v${version}`) || sw.includes(`foxbear-shell-${version}`) || sw.includes(`foxbear-shell-v1.6.73-csp-memory-admission-runtime-config`), 'service worker cache should use v1.6.73 key');
must(runtime.includes('FoxBearPlaybackTransitionService.crossfadePair'), 'runtime health should require transition service');

must(service.includes('SERVICE_VERSION = \'1.6.73-csp-memory-admission-runtime-config\''), 'transition service version missing');
...
must(pkg.qaChecks.includes('node qa/v143_playback_transition_audit_smoke.js'), 'v1.6.73 smoke should be in qaChecks');
