...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.12 playback transition audit smoke: ${message}`);
    process.exit(1);
  }
};
...
must(sw.includes(`foxbear-shell-v${version}`) || sw.includes(`foxbear-shell-${version}`) || sw.includes(`foxbear-shell-v1.4.12-download-diagnostics`), 'service worker cache should use v1.4.12 key');
must(runtime.includes('FoxBearPlaybackTransitionService.crossfadePair'), 'runtime health should require transition service');

must(service.includes('SERVICE_VERSION = \'1.4.12-download-diagnostics\''), 'transition service version missing');
...
must(pkg.qaChecks.includes('node qa/v143_playback_transition_audit_smoke.js'), 'v1.4.12 smoke should be in qaChecks');

console.log('PASS v1.4.12 playback transition audit smoke');
