...
must(sw.includes(`foxbear-shell-v${version}`) || sw.includes(`foxbear-shell-${version}`) || sw.includes(`foxbear-shell-v1.6.99-header-role-focus-integrity`), 'service worker cache should use v1.6.99 key');
must(runtime.includes('FoxBearPlaybackTransitionService.crossfadePair'), 'runtime health should require transition service');

must(service.includes('SERVICE_VERSION = \'1.6.99-header-role-focus-integrity\''), 'transition service version missing');
...
must(pkg.qaChecks.includes('node qa/v143_playback_transition_audit_smoke.js'), 'v1.6.99 smoke should be in qaChecks');
