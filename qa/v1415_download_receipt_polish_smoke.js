...
must(pkg.version === '1.6.74', 'package version should be current 1.6.74');
must(pkg.name === 'foxbear-mastering-studio', 'package name should be current v1-4-26');
must(html.includes('data-build="1.6.74"'), 'index build marker should be current');
must(html.includes('1.6.74-incident-admission-spark-retention-download-memory'), 'index should use current asset key');
must(sw.includes('foxbear-shell-v1.6.74-incident-admission-spark-retention-download-memory'), 'service worker cache should use current key');

must(service.includes('getDownloadActionReceipt'), 'download service should expose action receipt helper');
must(service.includes("version: '1.6.74'"), 'download receipt/diagnostics should report current version');
...
must(qaReport.includes('v1.6.74'), 'QA report should mention current version');
must(changelog.includes('v1.6.74'), 'changelog should mention current version');
must(handoff.includes('v1.6.74'), 'handoff should mention current version');
