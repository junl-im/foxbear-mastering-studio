...
must(pkg.version === '1.4.26', 'package version should be current 1.4.26');
must(pkg.name === 'foxbear-github-pro-v1-4-26', 'package name should be current v1-4-26');
must(html.includes('data-build="1.4.26"'), 'index build marker should be current');
must(html.includes('1.4.26-wake-lock-state-sync'), 'index should use current asset key');
must(sw.includes('foxbear-shell-v1.4.26-wake-lock-state-sync'), 'service worker cache should use current key');

must(service.includes('getDownloadActionReceipt'), 'download service should expose action receipt helper');
must(service.includes("version: '1.4.26'"), 'download receipt/diagnostics should report current version');
...
must(qaReport.includes('v1.4.26'), 'QA report should mention current version');
must(changelog.includes('v1.4.26'), 'changelog should mention current version');
must(handoff.includes('v1.4.26'), 'handoff should mention current version');
