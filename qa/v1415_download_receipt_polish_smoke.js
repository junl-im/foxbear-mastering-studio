...
must(pkg.version === '1.5.64', 'package version should be current 1.5.64');
must(pkg.name === 'foxbear-mastering-studio', 'package name should be current v1-4-26');
must(html.includes('data-build="1.5.64"'), 'index build marker should be current');
must(html.includes('1.5.64-incident-operations-health-self-diagnostics'), 'index should use current asset key');
must(sw.includes('foxbear-shell-v1.5.64-incident-operations-health-self-diagnostics'), 'service worker cache should use current key');

must(service.includes('getDownloadActionReceipt'), 'download service should expose action receipt helper');
must(service.includes("version: '1.5.64'"), 'download receipt/diagnostics should report current version');
...
must(qaReport.includes('v1.5.64'), 'QA report should mention current version');
must(changelog.includes('v1.5.64'), 'changelog should mention current version');
must(handoff.includes('v1.5.64'), 'handoff should mention current version');
