...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.11 download/share reliability smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(pkg.version === '1.4.11', 'package version should be 1.4.11');
assert(index.includes('data-build="1.4.11"'), 'index data-build should be 1.4.11');
assert(index.includes('1.4.11-download-share-reliability'), 'index assets should use v1.4.11 cache key');
assert(sw.includes('foxbear-shell-v1.4.11-download-share-reliability'), 'service worker cache should use v1.4.11 key');
...
console.log('PASS v1.4.11 download/share reliability smoke');
