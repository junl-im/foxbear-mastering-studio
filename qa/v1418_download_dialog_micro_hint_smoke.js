...
assert(index.includes('1.5.7-release-foundation'), 'index should use micro hint asset key');
assert(sw.includes('foxbear-shell-v1.5.7-release-foundation'), 'service worker should use micro hint cache key');

assert(service.includes('getDownloadDialogCompactHint'), 'download service should expose dialog micro hint helper');
assert(service.includes("version: '1.5.7'"), 'download helpers should report v1.5.7');
...
assert(pkg.qaChecks.includes('node qa/v1418_download_dialog_micro_hint_smoke.js'), 'package QA should include v1.5.7 micro hint smoke');
assert(matrix.includes('v1.4.26 Download dialog micro hint'), 'matrix should document v1.5.7 micro hint flow');
...
assert(readme.includes('Download dialog micro hint'), 'README should mention v1.5.7 scope');
