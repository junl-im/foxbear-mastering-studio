...
assert(pkg.version === '1.6.12', 'package version should be 1.6.12');
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match 1.6.12');
assert(index.includes('data-build="1.6.12"'), 'index build marker should be 1.6.12');
assert(index.includes('1.6.12-mastering-tone-loudness-fastpath'), 'index should use micro hint asset key');
assert(sw.includes('foxbear-shell-v1.6.12-mastering-tone-loudness-fastpath'), 'service worker should use micro hint cache key');

assert(service.includes('getDownloadDialogCompactHint'), 'download service should expose dialog micro hint helper');
assert(service.includes("version: '1.6.12'"), 'download helpers should report v1.6.12');
...
assert(dialog.includes("panel.append(close, title, name, warning, listLabel, list, selectedSummary, actions)"), 'v1.6.12 should mount the compact first-screen dialog stack');
assert(!dialog.includes('panel.append(close, title, name, envBox, flowCard'), 'verbose micro-hint cards must not be mounted by v1.6.12');
...
assert(pkg.qaChecks.includes('node qa/v1418_download_dialog_micro_hint_smoke.js'), 'package QA should include v1.6.12 micro hint smoke');
assert(matrix.includes('v1.4.26 Download dialog micro hint'), 'matrix should document v1.6.12 micro hint flow');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention final v1.6.12 pass count');
assert(changelog.includes('getDownloadDialogCompactHint'), 'changelog should mention micro hint helper');
assert(readme.includes('Download dialog micro hint'), 'README should mention v1.6.12 scope');
