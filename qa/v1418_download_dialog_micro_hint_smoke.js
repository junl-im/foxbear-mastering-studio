...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.24 download dialog micro hint smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(pkg.name === 'foxbear-github-pro-v1-4-24', 'package name should match 1.4.24');
assert(index.includes('data-build="1.4.24"'), 'index build marker should be 1.4.24');
assert(index.includes('1.4.24-bulk-import-hud'), 'index should use micro hint asset key');
assert(sw.includes('foxbear-shell-v1.4.24-bulk-import-hud'), 'service worker should use micro hint cache key');

assert(service.includes('getDownloadDialogCompactHint'), 'download service should expose dialog micro hint helper');
assert(service.includes("version: '1.4.24'"), 'download helpers should report v1.4.24');
...
assert(css.includes('v1.4.24 Download dialog micro hint'), 'CSS should document micro hint styles');
...
assert(pkg.qaChecks.includes('node qa/v1418_download_dialog_micro_hint_smoke.js'), 'package QA should include v1.4.24 micro hint smoke');
assert(matrix.includes('v1.4.24 Download dialog micro hint'), 'matrix should document v1.4.24 micro hint flow');
assert(qaReport.includes('144/144 PASS') || qaReport.includes('v1.4.24 final QA'), 'QA report should mention final v1.4.24 pass count');
assert(changelog.includes('getDownloadDialogCompactHint'), 'changelog should mention micro hint helper');
assert(readme.includes('Download dialog micro hint'), 'README should mention v1.4.24 scope');

console.log('PASS v1.4.24 download dialog micro hint smoke');
