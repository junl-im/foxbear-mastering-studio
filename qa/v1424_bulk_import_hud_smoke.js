...
assert(pkg.qaChecks.includes('node qa/v1424_bulk_import_hud_smoke.js'), 'package QA should include v1.7.4 HUD smoke');
...
assert(index.includes('data-build="1.7.4"'), 'index build marker should be v1.7.4');
...
assert(bulkHud.includes("const VIEW_VERSION = '1.7.4-reload-reentry-mode-chooser'"), 'HUD module should carry v1.7.4 version');
...
assert(index.includes('src/ui/bulk-import-hud-view.js?v=1.7.4-reload-reentry-mode-chooser'), 'index should load bulk import HUD module');

assert(index.includes('assets/css/bulk-import-hud.css?v=1.7.4-reload-reentry-mode-chooser'), 'index should load bulk import HUD CSS');
...
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should be updated for v1.7.4');
