...
assert(pkg.qaChecks.includes('node qa/v1424_bulk_import_hud_smoke.js'), 'package QA should include v1.4.26 HUD smoke');
...
assert(index.includes('data-build="1.4.26"'), 'index build marker should be v1.4.26');
...
assert(bulkHud.includes("const VIEW_VERSION = '1.4.26-wake-lock-state-sync'"), 'HUD module should carry v1.4.26 version');
...
assert(index.includes('src/ui/bulk-import-hud-view.js?v=1.4.26-wake-lock-state-sync'), 'index should load bulk import HUD module');

assert(index.includes('assets/css/bulk-import-hud.css?v=1.4.26-wake-lock-state-sync'), 'index should load bulk import HUD CSS');
...
assert(qaReport.includes('146/146 PASS') || qaReport.includes('v1.4.26 final QA'), 'QA report should be updated for v1.4.26');
