...
assert(pkg.qaChecks.includes('node qa/v1424_bulk_import_hud_smoke.js'), 'package QA should include v1.6.43 HUD smoke');
...
assert(index.includes('data-build="1.6.43"'), 'index build marker should be v1.6.43');
...
assert(bulkHud.includes("const VIEW_VERSION = '1.6.43-google-auth-trusted-types-csp-recovery'"), 'HUD module should carry v1.6.43 version');
...
assert(index.includes('src/ui/bulk-import-hud-view.js?v=1.6.43-google-auth-trusted-types-csp-recovery'), 'index should load bulk import HUD module');

assert(index.includes('assets/css/bulk-import-hud.css?v=1.6.43-google-auth-trusted-types-csp-recovery'), 'index should load bulk import HUD CSS');
...
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should be updated for v1.6.43');
