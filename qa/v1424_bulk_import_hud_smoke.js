...
const fail = message => { console.error(`FAIL v1.4.24 bulk import HUD smoke: ${message}`); process.exit(1); };
...
assert(pkg.qaChecks.includes('node qa/v1424_bulk_import_hud_smoke.js'), 'package QA should include v1.4.24 HUD smoke');
...
assert(index.includes('data-build="1.4.24"'), 'index build marker should be v1.4.24');
...
assert(bulkHud.includes("const VIEW_VERSION = '1.4.24-bulk-import-hud'"), 'HUD module should carry v1.4.24 version');
...
assert(index.includes('src/ui/bulk-import-hud-view.js?v=1.4.24-bulk-import-hud'), 'index should load bulk import HUD module');

assert(index.includes('assets/css/bulk-import-hud.css?v=1.4.24-bulk-import-hud'), 'index should load bulk import HUD CSS');
...
assert(qaReport.includes('146/146 PASS') || qaReport.includes('v1.4.24 final QA'), 'QA report should be updated for v1.4.24');
assert(changelog.includes('v1.4.24'), 'changelog should mention v1.4.24');

console.log('PASS v1.4.24 bulk import HUD smoke');
