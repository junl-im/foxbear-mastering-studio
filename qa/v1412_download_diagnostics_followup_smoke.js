...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.12 download diagnostics follow-up smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(matrix.includes('v1.4.12 Download diagnostics follow-up'), 'QA matrix should document v1.4.12 diagnostics checks');
assert(matrix.includes('Diagnostics copy'), 'QA matrix should include diagnostics copy scenario');

console.log('PASS v1.4.12 download diagnostics follow-up smoke');
