...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be updated for v1.5.38');
...
must(sw.includes('foxbear-shell-v1.5.38-preflight-worker-multitab-hardening'), 'service worker should use v1.5.38 cache key');
...
must(changelog.includes('v1.5.38') && changelog.includes('Dock FFT removal'), 'changelog should include v1.5.38 Dock FFT removal entry');
must(pkg.qaChecks.includes('node qa/v147_dock_fft_removal_stability_smoke.js'), 'v1.5.38 smoke should run in npm check');
