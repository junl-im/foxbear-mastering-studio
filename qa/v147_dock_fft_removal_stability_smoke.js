...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be updated for v1.6.7');
...
must(sw.includes('foxbear-shell-v1.6.7-incident-readiness-history-sync-performance-hud'), 'service worker should use v1.6.7 cache key');
...
must(changelog.includes('v1.6.7') && changelog.includes('Dock FFT removal'), 'changelog should include v1.6.7 Dock FFT removal entry');
must(pkg.qaChecks.includes('node qa/v147_dock_fft_removal_stability_smoke.js'), 'v1.6.7 smoke should run in npm check');
