...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be updated for v1.6.45');
...
must(sw.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'service worker should use v1.6.45 cache key');
...
must(changelog.includes('v1.6.45') && changelog.includes('Dock FFT removal'), 'changelog should include v1.6.45 Dock FFT removal entry');
must(pkg.qaChecks.includes('node qa/v147_dock_fft_removal_stability_smoke.js'), 'v1.6.45 smoke should run in npm check');
