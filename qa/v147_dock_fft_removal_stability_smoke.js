...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be updated for v1.5.15');
...
must(sw.includes('foxbear-shell-v1.5.15-e2e-runtime-classification'), 'service worker should use v1.5.15 cache key');
...
must(changelog.includes('v1.5.15') && changelog.includes('Dock FFT removal'), 'changelog should include v1.5.15 Dock FFT removal entry');
must(pkg.qaChecks.includes('node qa/v147_dock_fft_removal_stability_smoke.js'), 'v1.5.15 smoke should run in npm check');
