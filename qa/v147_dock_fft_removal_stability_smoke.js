...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.8 Dock FFT removal stability smoke: ${message}`);
    process.exit(1);
  }
};
...
must(pkg.name.includes('v1-4-8'), 'package name should be updated for v1.4.8');
...
must(sw.includes('foxbear-shell-v1.4.8-dock-spectrum-cleanup'), 'service worker should use v1.4.8 cache key');
...
must(mobileCss.includes('v1.4.8: align the floating settings gear'), 'PC settings gear alignment patch should be present');
...
must(changelog.includes('v1.4.8') && changelog.includes('Dock FFT removal'), 'changelog should include v1.4.8 Dock FFT removal entry');
must(pkg.qaChecks.includes('node qa/v147_dock_fft_removal_stability_smoke.js'), 'v1.4.8 smoke should run in npm check');

console.log('PASS v1.4.8 Dock FFT removal stability smoke');
