...
const BOOT_KEY = ['h=boot-sri-v1518','h=boot-sri-v156','h=boot-sri-v155','h=boot-sri-v154'].find(key => index.includes(key));
assert(index.includes(`src/boot/runtime-health.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'runtime health boot cache-bust key missing from index');
assert(index.includes(`src/boot/performance-diagnostics.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'performance diagnostics boot cache-bust key missing from index');
assert(index.includes(`src/app.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'app boot cache-bust key missing from index');
assert(sw.includes(`./src/boot/runtime-health.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'runtime health boot cache-bust key missing from service worker');
assert(sw.includes(`./src/boot/performance-diagnostics.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'performance diagnostics boot cache-bust key missing from service worker');
assert(sw.includes(`./src/app.js?v=1.5.18-ci-diagnostics-pwa-readiness&${BOOT_KEY}`), 'app boot cache-bust key missing from service worker');
assert(sw.includes("foxbear-shell-v1.5.4-boot-sri-recovery") || sw.includes("foxbear-shell-v1.5.5-update-safety") || sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") || sw.includes("foxbear-shell-v1.5.18-ci-diagnostics-pwa-readiness"), 'service worker cache generation was not bumped');
