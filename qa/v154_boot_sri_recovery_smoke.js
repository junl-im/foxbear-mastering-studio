...
const BOOT_KEY = ['h=boot-sri-v1543','h=boot-sri-v156','h=boot-sri-v155','h=boot-sri-v154'].find(key => index.includes(key));
assert(index.includes(`src/boot/runtime-health.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'runtime health boot cache-bust key missing from index');
assert(index.includes(`src/boot/performance-diagnostics.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'performance diagnostics boot cache-bust key missing from index');
assert(index.includes(`src/app.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'app boot cache-bust key missing from index');
assert(sw.includes(`./src/boot/runtime-health.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'runtime health boot cache-bust key missing from service worker');
assert(sw.includes(`./src/boot/performance-diagnostics.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'performance diagnostics boot cache-bust key missing from service worker');
assert(sw.includes(`./src/app.js?v=1.5.43-export-pipeline-integrity&${BOOT_KEY}`), 'app boot cache-bust key missing from service worker');
assert(sw.includes("foxbear-shell-v1.5.4-boot-sri-recovery") || sw.includes("foxbear-shell-v1.5.5-update-safety") || sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") || sw.includes("foxbear-shell-v1.5.43-export-pipeline-integrity"), 'service worker cache generation was not bumped');
