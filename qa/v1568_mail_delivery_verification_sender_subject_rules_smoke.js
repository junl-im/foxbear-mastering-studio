...
assert.strictEqual(pkg.foxbearRelease.assetVersion, '1.6.109-mastering-cooperative-pcm-responsiveness');
for (const token of [
...
for (const token of ['subject:', 'senderName:', 'acceptedCount:', 'smtpAcceptedAt:']) {
  assert(firebaseSource.includes(token), `delivery receipt missing ${token}`);
}
assert(!firebaseSource.includes("recipient: limitText(delivery.recipient || '', 180)"), 'public delivery fallback must not expose the operations recipient');
assert(!firebaseSource.includes("smtpResponse: limitText(delivery.smtpResponse || '', 300)"), 'public delivery fallback must not expose raw SMTP response text');
...
const manual = test.buildMail({ category: 'manual-test', severity: 'warning', fingerprint: 'manual-test-abc123', appVersion: '1.6.109' }, 'uid_report');
assert(manual.subject.startsWith('[AI마스터링 스튜디오][메일 테스트] 실제 발송 확인'));
assert.strictEqual(manual.type, 'manual-test');
const incident = test.buildMail({ category: 'mastering', severity: 'fatal', fingerprint: 'master', appVersion: '1.6.109' }, 'uid_report_12345');
assert(incident.subject.includes('[오류 신고] 긴급 · 마스터링 오류 · v1.6.109'));
