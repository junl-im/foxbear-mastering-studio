...
assert.strictEqual(pkg.foxbearRelease.assetVersion, '1.6.56-playback-blob-source-resilience');
for (const token of [
...
const manual = test.buildMail({ category: 'manual-test', severity: 'warning', fingerprint: 'manual-test-abc123', appVersion: '1.6.56' }, 'uid_report');
assert(manual.subject.startsWith('[AI마스터링 스튜디오][메일 테스트] 실제 발송 확인'));
assert.strictEqual(manual.type, 'manual-test');
const incident = test.buildMail({ category: 'mastering', severity: 'fatal', fingerprint: 'master', appVersion: '1.6.56' }, 'uid_report_12345');
assert(incident.subject.includes('[오류 신고] 긴급 · 마스터링 오류 · v1.6.56'));
