...
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.6.20 bulk control missing: ${id}`);
}
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.6.20 download panel contract missing');
