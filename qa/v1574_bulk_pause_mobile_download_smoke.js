...
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.5.96 bulk control missing: ${id}`);
}
...
assert(hud.includes("const VIEW_VERSION = '1.5.96-bulk-pause-skip-reorder-summary'"), 'HUD v1.5.96 contract missing');
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.5.96 download panel contract missing');
