...
assert.strictEqual(pkg.version, '1.6.4', 'package version should be v1.6.4');
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.6.4 bulk control missing: ${id}`);
}
...
assert(hud.includes("const VIEW_VERSION = '1.6.4-bulk-pause-skip-reorder-summary'"), 'HUD v1.6.4 contract missing');
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.6.4 download panel contract missing');
