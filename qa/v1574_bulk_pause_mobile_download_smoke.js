...
assert.strictEqual(pkg.version, '1.5.93', 'package version should be v1.5.93');
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.5.93 bulk control missing: ${id}`);
}
...
assert(hud.includes("const VIEW_VERSION = '1.5.93-bulk-pause-skip-reorder-summary'"), 'HUD v1.5.93 contract missing');
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.5.93 download panel contract missing');
