...
assert.strictEqual(pkg.version, '1.5.97', 'package version should be v1.5.97');
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.5.97 bulk control missing: ${id}`);
}
...
assert(hud.includes("const VIEW_VERSION = '1.5.97-bulk-pause-skip-reorder-summary'"), 'HUD v1.5.97 contract missing');
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.5.97 download panel contract missing');
