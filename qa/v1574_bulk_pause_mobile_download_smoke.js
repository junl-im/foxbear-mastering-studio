...
assert.strictEqual(pkg.version, '1.6.83', 'package version should be v1.6.83');
for (const id of ['bulkImportHudPause', 'bulkImportHudSkip', 'bulkImportHudSummary']) {
  assert(index.includes(`id="${id}"`), `v1.6.83 bulk control missing: ${id}`);
}
...
assert(dialog.includes('download-options-panel-v1574') && dialog.includes('formatPicker'), 'v1.6.83 download panel contract missing');
