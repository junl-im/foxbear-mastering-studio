...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.10 performance diagnostics smoke: ${message}`);
    process.exit(1);
  }
};
...
must(pkg.name.includes('v1-4-10'), 'package name should use v1.4.10');
...
must(sw.includes(`foxbear-shell-v${version}`), 'service worker should use v1.4.10 cache key');
...
must(pkg.qaChecks.includes('node qa/v149_performance_diagnostics_smoke.js'), 'package should run v1.4.10 smoke');
...
must(changelog.includes('v1.4.10') && changelog.includes('Performance diagnostics'), 'changelog should document v1.4.10 diagnostics');
must(handoff.includes('v1.4.10') && handoff.includes('FoxBearPerformanceDiagnostics'), 'handoff should mention diagnostics global');
must(notes.includes('v1.4.10') && notes.toLowerCase().includes('performance diagnostics'), 'project notes should mention diagnostics');
must(readme.includes('v1.4.10') && readme.includes('Ctrl/Command + Alt + P'), 'README should document diagnostics toggle');
must(matrix.includes('v1.4.10') && matrix.includes('Performance diagnostics'), 'QA matrix should cover v1.4.10 diagnostics');

console.log('PASS v1.4.10 performance diagnostics smoke');
