...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.18 performance diagnostics smoke: ${message}`);
    process.exit(1);
  }
};
...
must(pkg.name.includes('v1-4-18'), 'package name should use v1.4.18');
...
must(pkg.qaChecks.includes('node qa/v149_performance_diagnostics_smoke.js'), 'package should run v1.4.18 smoke');
...
must(changelog.includes('v1.4.18') && changelog.includes('Performance diagnostics'), 'changelog should document v1.4.18 diagnostics');
must(handoff.includes('v1.4.18') && handoff.includes('FoxBearPerformanceDiagnostics'), 'handoff should mention diagnostics global');
must(notes.includes('v1.4.18') && notes.toLowerCase().includes('performance diagnostics'), 'project notes should mention diagnostics');
must(readme.includes('v1.4.18') && readme.includes('Ctrl/Command + Alt + P'), 'README should document diagnostics toggle');
must(matrix.includes('v1.4.18') && matrix.includes('Performance diagnostics'), 'QA matrix should cover v1.4.18 diagnostics');

console.log('PASS v1.4.18 performance diagnostics smoke');
