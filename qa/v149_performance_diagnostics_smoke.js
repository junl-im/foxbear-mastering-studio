...
must(pkg.qaChecks.includes('node qa/v149_performance_diagnostics_smoke.js'), 'package should run v1.6.94 smoke');
...
must(changelog.includes('v1.6.94') && changelog.includes('Performance diagnostics'), 'changelog should document v1.6.94 diagnostics');
must(handoff.includes('v1.6.94') && handoff.includes('FoxBearPerformanceDiagnostics'), 'handoff should mention diagnostics global');
must(notes.toLowerCase().includes('performance diagnostics'), 'project notes should preserve performance diagnostics guidance');
must(readme.includes('v1.6.94') && readme.includes('Ctrl/Command + Alt + P'), 'README should document diagnostics toggle');
must(matrix.includes('v1.4.26') && matrix.includes('Performance diagnostics'), 'QA matrix should cover v1.6.94 diagnostics');
