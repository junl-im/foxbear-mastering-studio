...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.12 performance diagnostics smoke: ${message}`);
    process.exit(1);
  }
};
...
must(pkg.name.includes('v1-4-12'), 'package name should use v1.4.12');
...
must(sw.includes(`foxbear-shell-v${version}`), 'service worker should use v1.4.12 cache key');
...
must(pkg.qaChecks.includes('node qa/v149_performance_diagnostics_smoke.js'), 'package should run v1.4.12 smoke');
...
must(changelog.includes('v1.4.12') && changelog.includes('Performance diagnostics'), 'changelog should document v1.4.12 diagnostics');
must(handoff.includes('v1.4.12') && handoff.includes('FoxBearPerformanceDiagnostics'), 'handoff should mention diagnostics global');
must(notes.includes('v1.4.12') && notes.toLowerCase().includes('performance diagnostics'), 'project notes should mention diagnostics');
must(readme.includes('v1.4.12') && readme.includes('Ctrl/Command + Alt + P'), 'README should document diagnostics toggle');
must(matrix.includes('v1.4.12') && matrix.includes('Performance diagnostics'), 'QA matrix should cover v1.4.12 diagnostics');

console.log('PASS v1.4.12 performance diagnostics smoke');
