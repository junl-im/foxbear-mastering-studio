...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.14 performance diagnostics smoke: ${message}`);
    process.exit(1);
  }
};
...
must(pkg.name.includes('v1-4-14'), 'package name should use v1.4.14');
...
must(pkg.qaChecks.includes('node qa/v149_performance_diagnostics_smoke.js'), 'package should run v1.4.14 smoke');
...
must(changelog.includes('v1.4.14') && changelog.includes('Performance diagnostics'), 'changelog should document v1.4.14 diagnostics');
must(handoff.includes('v1.4.14') && handoff.includes('FoxBearPerformanceDiagnostics'), 'handoff should mention diagnostics global');
must(notes.includes('v1.4.14') && notes.toLowerCase().includes('performance diagnostics'), 'project notes should mention diagnostics');
must(readme.includes('v1.4.14') && readme.includes('Ctrl/Command + Alt + P'), 'README should document diagnostics toggle');
must(matrix.includes('v1.4.14') && matrix.includes('Performance diagnostics'), 'QA matrix should cover v1.4.14 diagnostics');

console.log('PASS v1.4.14 performance diagnostics smoke');
