...
const fallbackService = read('src/audio/master-finalizer-fallback-service.js');
...
assert(index.includes('src/audio/reference-profile-service.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'reference profile service not loaded in index');
...
assert(sw.includes('./src/audio/reference-profile-service.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'reference profile service not precached');
...
assert(gate.includes('1.6.112-engine-quality-regression'), 'quality gate service version missing');
...
assert(app.includes('const shortTermLufs = measureShortTermLufsStats(working)') || app.includes('const shortTermLufs = finalLoudness.shortTerm') || fallbackService.includes('shortTermLufs: finalLoudness.shortTerm'), 'fallback finalizer shortTermLufs telemetry missing');
