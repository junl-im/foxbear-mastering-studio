...
const version = '1.3.84-stage13-runtime-safety';
...
assert(index.indexOf('src/boot/runtime-health.js') > index.indexOf('src/security/site-guards.js'), 'runtime health should load after guard/module dependencies');
...
assert(sw.includes(`foxbear-shell-v1.3.84-stage13-runtime-safety`), 'service worker cache name should be stage13');
assert(cfg.includes(`const ASSET_VERSION = '${version}'`), 'runtime config should expose stage13 asset version');
...
assert(app.includes(`navigator.serviceWorker.register('./sw.js?v=${version}')`), 'service worker registration should use stage13 cache key');
...
assert(overwrite.includes('v1.3.84-stage13'), 'overwrite package default should be stage13');
...
assert(docs.includes('Stage13'), 'handoff docs should mention Stage13');
