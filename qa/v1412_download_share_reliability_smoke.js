...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.18 download/share reliability smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(index.includes('data-build="1.4.18"'), 'index data-build should be 1.4.18');
assert(index.includes('1.4.18-download-dialog-micro-hint'), 'index assets should use v1.4.18 cache key');
assert(sw.includes('foxbear-shell-v1.4.18-download-dialog-micro-hint'), 'service worker cache should use v1.4.18 key');
...
console.log('PASS v1.4.18 download/share reliability smoke');
