...
must(app.includes("const APP_VERSION = 'Pro v1.3.69'"), 'app version must be v1.3.69');
must(html.includes('data-build="1.3.69"'), 'index data-build must be v1.3.69');
...
must(html.includes('src/app.js?v=1.3.69-dock-action-runtime-fix'), 'app cache bust key missing');
must(sw.includes('foxbear-shell-v1.3.69-dock-action-target-fix'), 'SW cache key missing');
