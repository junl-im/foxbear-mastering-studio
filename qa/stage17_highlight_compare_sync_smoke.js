...
const assetVersion = '1.4.0-stage17-highlight-compare-sync';

assert(pkg.version === '1.4.0', 'Stage17 should keep official app version at 1.4.0');
assert(pkg.description.includes('Stage17') && pkg.description.includes('highlight compare'), 'package description should identify Stage17 highlight compare hotfix');
...
assert(compare.includes("mode === 'masterPreview' ? 0 : alignedStartSec"), 'original listen should seek to absolute highlight start while master preview starts at local zero');
...
assert(compare.includes('startSec: previewStart') && compare.includes('durationSec: previewDuration'), 'preview comparison rows should carry start and duration metadata');
