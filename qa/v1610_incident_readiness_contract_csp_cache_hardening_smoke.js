...
const sandbox = {
  ...
  document: {
    body: { dataset: { build: '1.6.109' }, appendChild() {} }, visibilityState: 'visible',
    getElementById: () => null,
  ...
  FoxBearBuildInfo: { productVersion: '1.6.109', assetVersion: '1.6.109-mastering-cooperative-pcm-responsiveness' }
};
const completeRemote = () => {
  ...
  return {
    ...
    service: { status: 'ready', productVersion: '1.6.109', functionsOrigin: origin },
    checks: {
      functions: { ok: true, status: 'ready', message: 'functions ok' },
      firestore: { ok: true, status: 'ready', message: 'firestore ok' },
      mailRouting: { ok: true, status: 'ready', message: 'routing ok' },
