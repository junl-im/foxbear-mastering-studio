...
const assetVersion = '1.6.56-playback-blob-source-resilience';

must(pkg.version === version, 'package version should be 1.6.56');
must(pkg.name === 'foxbear-mastering-studio', 'package name should use v1.6.56');
must(manifest.version === version, 'manifest version should be 1.6.56');
must(index.includes('data-build="1.6.56"'), 'index build marker should be v1.6.56');
must(index.includes(`src/boot/performance-diagnostics.js?v=${assetVersion}`), 'index should load diagnostics with new cache key');
must(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker should use v1.6.56 cache key');
...
must(perf.includes('const DIAGNOSTICS_VERSION = \'1.6.56-playback-blob-source-resilience\''), 'diagnostics version should be v1.6.56');
...
must(pkg.qaChecks.includes('node qa/v1410_performance_packaging_polish_smoke.js'), 'package should run v1.6.56 smoke');
must(changelog.includes('v1.6.56') && changelog.includes('Packaging'), 'changelog should document v1.6.56 packaging polish');
must(handoff.includes('v1.6.56') && handoff.includes('getSummary'), 'handoff should document diagnostics summary');
must(notes.includes('adaptive'), 'project notes should preserve adaptive diagnostics refresh guidance');
must(readme.includes('v1.6.56') && readme.includes('복사'), 'README should mention diagnostics copy action');
must(matrix.includes('v1.4.26') && matrix.includes('Performance diagnostics'), 'QA matrix should document v1.6.56');
