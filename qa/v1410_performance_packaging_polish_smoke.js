...
const version = '1.5.9';
const assetVersion = '1.5.9-version-display-cache-recovery';

must(pkg.version === version, 'package version should be 1.5.9');
must(pkg.name === 'foxbear-mastering-studio', 'package name should use v1.5.9');
must(manifest.version === version, 'manifest version should be 1.5.9');
must(index.includes('data-build="1.5.9"'), 'index build marker should be v1.5.9');
must(index.includes(`src/boot/performance-diagnostics.js?v=${assetVersion}`), 'index should load diagnostics with new cache key');
must(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker should use v1.5.9 cache key');
...
must(perf.includes('const DIAGNOSTICS_VERSION = \'1.5.9-version-display-cache-recovery\''), 'diagnostics version should be v1.5.9');
...
must(pkg.qaChecks.includes('node qa/v1410_performance_packaging_polish_smoke.js'), 'package should run v1.5.9 smoke');
must(changelog.includes('v1.5.9') && changelog.includes('Packaging'), 'changelog should document v1.5.9 packaging polish');
must(handoff.includes('v1.5.9') && handoff.includes('getSummary'), 'handoff should document diagnostics summary');
must(notes.includes('v1.5.9') && notes.includes('adaptive'), 'project notes should mention adaptive diagnostics refresh');
must(readme.includes('v1.5.9') && readme.includes('복사'), 'README should mention diagnostics copy action');
must(matrix.includes('v1.4.26') && matrix.includes('Performance diagnostics'), 'QA matrix should document v1.5.9');
