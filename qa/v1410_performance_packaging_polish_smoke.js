...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.20 performance packaging polish smoke: ${message}`);
    process.exit(1);
  }
};
...
const assetVersion = '1.4.20-bulk-import-guard';

must(pkg.version === version, 'package version should be 1.4.20');
must(pkg.name === 'foxbear-github-pro-v1-4-20', 'package name should use v1.4.20');
must(manifest.version === version, 'manifest version should be 1.4.20');
must(index.includes('data-build="1.4.20"'), 'index build marker should be v1.4.20');
must(index.includes(`src/boot/performance-diagnostics.js?v=${assetVersion}`), 'index should load diagnostics with new cache key');
must(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker should use v1.4.20 cache key');
...
must(perf.includes('const DIAGNOSTICS_VERSION = \'1.4.20-bulk-import-guard\''), 'diagnostics version should be v1.4.20');
...
must(pkg.qaChecks.includes('node qa/v1410_performance_packaging_polish_smoke.js'), 'package should run v1.4.20 smoke');
must(changelog.includes('v1.4.20') && changelog.includes('Packaging'), 'changelog should document v1.4.20 packaging polish');
must(handoff.includes('v1.4.20') && handoff.includes('getSummary'), 'handoff should document diagnostics summary');
must(notes.includes('v1.4.20') && notes.includes('adaptive'), 'project notes should mention adaptive diagnostics refresh');
must(readme.includes('v1.4.20') && readme.includes('복사'), 'README should mention diagnostics copy action');
must(matrix.includes('v1.4.20') && matrix.includes('Performance diagnostics'), 'QA matrix should document v1.4.20');

console.log('PASS v1.4.20 performance packaging polish smoke');
