...
const failures=[];
['index.html','external-browser.html','manifest.webmanifest','sw.js'].forEach(file=>{
...
const prepare = fs.existsSync(path.join(root,'tools','prepare-pages-site.sh'))
  ...
  : '';
['index.html','external-browser.html','manifest.webmanifest','sw.js','assets','src','vendor','touch "${SITE_DIR}/.nojekyll"','type l','-links +1'].forEach(s=>{
  if(!prepare.includes(s)) failures.push(`prepare script missing ${s}`);
});

if(!/required_files=\([^\n)]*external-browser\.html/.test(prepare)) failures.push('external-browser.html must be a required Pages artifact');
if(!prepare.includes('test -f "${SITE_DIR}/external-browser.html"')) failures.push('Pages artifact verification missing external-browser.html');
