...
must(html.includes('data-build="1.6.76"'), 'index build marker should be 1.6.76');
must(html.includes('1.6.76-download-viewport-runtime-fault-diagnostics'), 'index should use v1.6.76 asset key');
must(sw.includes('foxbear-shell-v1.6.76-download-viewport-runtime-fault-diagnostics'), 'service worker cache should use v1.6.76 key');
...
must(service.includes("version: '1.6.76'"), 'download helpers should report v1.6.76');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.76 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.76 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.76 browser matrix should exist');
must(qaReport.includes('v1.6.76'), 'QA report should mention v1.6.76');
must(changelog.includes('v1.6.76'), 'changelog should mention v1.6.76');
must(handoff.includes('v1.6.76'), 'handoff should mention v1.6.76');
