...
must(html.includes('data-build="1.6.25"'), 'index build marker should be 1.6.25');
must(html.includes('1.6.25-incident-recovery-timeout-abort-stress'), 'index should use v1.6.25 asset key');
...
must(service.includes("version: '1.6.25'"), 'download helpers should report v1.6.25');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.25 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.25 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.25 browser matrix should exist');
must(qaReport.includes('v1.6.25'), 'QA report should mention v1.6.25');
must(changelog.includes('v1.6.25'), 'changelog should mention v1.6.25');
must(handoff.includes('v1.6.25'), 'handoff should mention v1.6.25');
