...
must(html.includes('data-build="1.5.26"'), 'index build marker should be 1.5.26');
must(html.includes('1.5.26-engraved-command-header'), 'index should use v1.5.26 asset key');
must(sw.includes('foxbear-shell-v1.5.26-engraved-command-header'), 'service worker cache should use v1.5.26 key');
...
must(service.includes("version: '1.5.26'"), 'download helpers should report v1.5.26');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.5.26 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.5.26 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.5.26 browser matrix should exist');
must(qaReport.includes('v1.5.26'), 'QA report should mention v1.5.26');
must(changelog.includes('v1.5.26'), 'changelog should mention v1.5.26');
must(handoff.includes('v1.5.26'), 'handoff should mention v1.5.26');
