...
must(html.includes('data-build="1.6.91"'), 'index build marker should be 1.6.91');
must(html.includes('1.6.91-runtime-health-hidden-geometry-contract-recovery'), 'index should use v1.6.91 asset key');
...
must(service.includes("version: '1.6.91'"), 'download helpers should report v1.6.91');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.91 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.91 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.91 browser matrix should exist');
must(qaReport.includes('v1.6.91'), 'QA report should mention v1.6.91');
must(changelog.includes('v1.6.91'), 'changelog should mention v1.6.91');
must(handoff.includes('v1.6.91'), 'handoff should mention v1.6.91');
