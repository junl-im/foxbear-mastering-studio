...
must(html.includes('data-build="1.6.88"'), 'index build marker should be 1.6.88');
must(html.includes('1.6.88-mobile-header-grid-ownership-recovery'), 'index should use v1.6.88 asset key');
...
must(service.includes("version: '1.6.88'"), 'download helpers should report v1.6.88');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.88 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.88 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.88 browser matrix should exist');
must(qaReport.includes('v1.6.88'), 'QA report should mention v1.6.88');
must(changelog.includes('v1.6.88'), 'changelog should mention v1.6.88');
must(handoff.includes('v1.6.88'), 'handoff should mention v1.6.88');
