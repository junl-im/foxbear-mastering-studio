...
must(html.includes('data-build="1.6.108"'), 'index build marker should be 1.6.108');
must(html.includes('1.6.108-mastering-progress-visibility-hardening'), 'index should use v1.6.108 asset key');
must(sw.includes('foxbear-shell-v1.6.108-mastering-progress-visibility-hardening'), 'service worker cache should use v1.6.108 key');
...
must(service.includes("version: '1.6.108'"), 'download helpers should report v1.6.108');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.108 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.108 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.108 browser matrix should exist');
must(qaReport.includes('v1.6.108'), 'QA report should mention v1.6.108');
must(changelog.includes('v1.6.108'), 'changelog should mention v1.6.108');
must(handoff.includes('v1.6.108'), 'handoff should mention v1.6.108');
