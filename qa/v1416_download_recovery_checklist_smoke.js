...
must(html.includes('data-build="1.6.65"'), 'index build marker should be 1.6.65');
must(html.includes('1.6.65-firestore-write-fencing'), 'index should use v1.6.65 asset key');
...
must(service.includes("version: '1.6.65'"), 'download helpers should report v1.6.65');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.65 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.65 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.65 browser matrix should exist');
must(qaReport.includes('v1.6.65'), 'QA report should mention v1.6.65');
must(changelog.includes('v1.6.65'), 'changelog should mention v1.6.65');
must(handoff.includes('v1.6.65'), 'handoff should mention v1.6.65');
