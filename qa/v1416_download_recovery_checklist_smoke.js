...
must(html.includes('data-build="1.6.104"'), 'index build marker should be 1.6.104');
must(html.includes('1.6.104-boot-emergency-visit-privacy-hardening'), 'index should use v1.6.104 asset key');
must(sw.includes('foxbear-shell-v1.6.104-boot-emergency-visit-privacy-hardening'), 'service worker cache should use v1.6.104 key');
...
must(service.includes("version: '1.6.104'"), 'download helpers should report v1.6.104');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.104 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.104 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.104 browser matrix should exist');
must(qaReport.includes('v1.6.104'), 'QA report should mention v1.6.104');
must(changelog.includes('v1.6.104'), 'changelog should mention v1.6.104');
must(handoff.includes('v1.6.104'), 'handoff should mention v1.6.104');
