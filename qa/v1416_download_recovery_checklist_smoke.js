...
must(pkg.name === 'foxbear-github-pro-v1-4-23', 'package name should be v1-4-23');
must(html.includes('data-build="1.4.23"'), 'index build marker should be 1.4.23');
must(html.includes('1.4.23-audio-decode-memory-guard'), 'index should use v1.4.23 asset key');
must(sw.includes('foxbear-shell-v1.4.23-audio-decode-memory-guard'), 'service worker cache should use v1.4.23 key');
...
must(service.includes("version: '1.4.23'"), 'download helpers should report v1.4.23');
...
must(css.includes('v1.4.23 Download recovery checklist'), 'CSS should include v1.4.23 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.4.23 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.23.md')), 'v1.4.23 browser matrix should exist');
must(qaReport.includes('v1.4.23'), 'QA report should mention v1.4.23');
must(changelog.includes('v1.4.23'), 'changelog should mention v1.4.23');
must(handoff.includes('v1.4.23'), 'handoff should mention v1.4.23');
console.log('PASS v1.4.23 download recovery checklist smoke');
