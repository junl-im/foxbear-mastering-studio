...
must(html.includes('data-build="1.6.0"'), 'index build marker should be 1.6.0');
must(html.includes('1.6.0-incident-mail-pipeline-health'), 'index should use v1.6.0 asset key');
must(sw.includes('foxbear-shell-v1.6.0-incident-mail-pipeline-health'), 'service worker cache should use v1.6.0 key');
...
must(service.includes("version: '1.6.0'"), 'download helpers should report v1.6.0');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.6.0 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.6.0 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.6.0 browser matrix should exist');
must(qaReport.includes('v1.6.0'), 'QA report should mention v1.6.0');
must(changelog.includes('v1.6.0'), 'changelog should mention v1.6.0');
must(handoff.includes('v1.6.0'), 'handoff should mention v1.6.0');
