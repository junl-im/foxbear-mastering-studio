...
must(html.includes('data-build="1.5.59"'), 'index build marker should be 1.5.59');
must(html.includes('1.5.59-kakao-session-handoff-memory-diagnostics'), 'index should use v1.5.59 asset key');
must(sw.includes('foxbear-shell-v1.5.59-kakao-session-handoff-memory-diagnostics'), 'service worker cache should use v1.5.59 key');
...
must(service.includes("version: '1.5.59'"), 'download helpers should report v1.5.59');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.5.59 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.5.59 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.5.59 browser matrix should exist');
must(qaReport.includes('v1.5.59'), 'QA report should mention v1.5.59');
must(changelog.includes('v1.5.59'), 'changelog should mention v1.5.59');
must(handoff.includes('v1.5.59'), 'handoff should mention v1.5.59');
