...
must(html.includes('data-build="1.5.80"'), 'index build marker should be 1.5.80');
must(html.includes('1.5.80-mobile-return-media-focus-recovery'), 'index should use v1.5.80 asset key');
...
must(service.includes("version: '1.5.80'"), 'download helpers should report v1.5.80');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.5.80 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.5.80 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.5.80 browser matrix should exist');
must(qaReport.includes('v1.5.80'), 'QA report should mention v1.5.80');
must(changelog.includes('v1.5.80'), 'changelog should mention v1.5.80');
must(handoff.includes('v1.5.80'), 'handoff should mention v1.5.80');
