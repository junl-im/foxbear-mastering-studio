...
must(html.includes('1.5.20-idempotent-pwa-cache-warm'), 'index should use v1.5.20 asset key');
...
must(service.includes("version: '1.5.20'"), 'download helpers should report v1.5.20');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.5.20 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.5.20 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.5.20 browser matrix should exist');
must(qaReport.includes('v1.5.20'), 'QA report should mention v1.5.20');
must(changelog.includes('v1.5.20'), 'changelog should mention v1.5.20');
must(handoff.includes('v1.5.20'), 'handoff should mention v1.5.20');
