...
must(html.includes('data-build="1.7.4"'), 'index build marker should be 1.7.4');
must(html.includes('1.7.4-reload-reentry-mode-chooser'), 'index should use v1.7.4 asset key');
...
must(service.includes("version: '1.7.4'"), 'download helpers should report v1.7.4');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.7.4 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.7.4 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.7.4 browser matrix should exist');
must(qaReport.includes('v1.7.4'), 'QA report should mention v1.7.4');
must(changelog.includes('v1.7.4'), 'changelog should mention v1.7.4');
must(handoff.includes('v1.7.4'), 'handoff should mention v1.7.4');
