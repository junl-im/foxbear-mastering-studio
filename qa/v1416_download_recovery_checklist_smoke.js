...
must(html.includes('data-build="1.5.99"'), 'index build marker should be 1.5.99');
must(html.includes('1.5.99-incident-callable-mail-recovery'), 'index should use v1.5.99 asset key');
...
must(service.includes("version: '1.5.99'"), 'download helpers should report v1.5.99');
...
must(css.includes('Download recovery checklist'), 'CSS should include v1.5.99 checklist comment');
...
must(pkg.qaChecks.includes('node qa/v1416_download_recovery_checklist_smoke.js'), 'package qaChecks should include v1.5.99 smoke');
must(fs.existsSync(path.join(root, 'qa/BROWSER_BACK_QA_MATRIX_1.4.26.md')), 'v1.5.99 browser matrix should exist');
must(qaReport.includes('v1.5.99'), 'QA report should mention v1.5.99');
must(changelog.includes('v1.5.99'), 'changelog should mention v1.5.99');
must(handoff.includes('v1.5.99'), 'handoff should mention v1.5.99');
