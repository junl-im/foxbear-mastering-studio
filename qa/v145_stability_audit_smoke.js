...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.5 stability audit smoke: ${message}`);
    process.exit(1);
  }
};
...
must(html.includes('data-build="1.4.5"'), 'index build marker should be 1.4.5');
must(html.includes('1.4.5-stability-audit'), 'index should use v1.4.5 cache key');
must(sw.includes('foxbear-shell-v1.4.5-stability-audit'), 'service worker cache should use v1.4.5 key');
...
console.log('PASS v1.4.5 stability audit smoke');
