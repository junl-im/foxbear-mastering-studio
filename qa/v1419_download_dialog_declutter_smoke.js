...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.20 download dialog declutter smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(index.includes('1.4.20-bulk-import-guard'), 'index should use declutter asset key');
assert(sw.includes('foxbear-shell-v1.4.20-bulk-import-guard'), 'service worker should use v1.4.20 cache key');
...
assert(css.includes('v1.4.20 Download dialog first-screen declutter'), 'CSS should document declutter styles');
...
assert(pkg.qaChecks.includes('node qa/v1419_download_dialog_declutter_smoke.js'), 'package QA should include v1.4.20 smoke');
assert(matrix.includes('v1.4.20 Download dialog first-screen declutter'), 'matrix should document v1.4.20 scope');
...
console.log('PASS v1.4.20 download dialog declutter smoke');
