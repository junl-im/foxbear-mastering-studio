...
assert(index.includes('1.6.89-mobile-header-flex-ownership-browser-gate-recovery'), 'index should use declutter asset key');
assert(sw.includes('foxbear-shell-v1.6.89-mobile-header-flex-ownership-browser-gate-recovery'), 'service worker should use v1.6.89 cache key');
...
assert(pkg.qaChecks.includes('node qa/v1419_download_dialog_declutter_smoke.js'), 'package QA should include v1.6.89 smoke');
assert(matrix.includes('v1.4.26 Download dialog first-screen declutter'), 'matrix should document v1.6.89 scope');
