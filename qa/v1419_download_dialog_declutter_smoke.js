...
assert(index.includes('1.5.81-master-preview-cancellation-native-result-isolation'), 'index should use declutter asset key');
assert(sw.includes('foxbear-shell-v1.5.81-master-preview-cancellation-native-result-isolation'), 'service worker should use v1.5.81 cache key');
...
assert(pkg.qaChecks.includes('node qa/v1419_download_dialog_declutter_smoke.js'), 'package QA should include v1.5.81 smoke');
assert(matrix.includes('v1.4.26 Download dialog first-screen declutter'), 'matrix should document v1.5.81 scope');
