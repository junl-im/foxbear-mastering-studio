...
assert(pkg.version === version, 'package version should be 1.4.24');
assert(pkg.description.includes('v1.4.24') && pkg.description.includes('Spectrum'), 'package description should identify latest v1.4.24 line');
assert(manifest.version === version, 'manifest should carry version 1.4.24');
assert(html.includes('<title>FoxBear Mastering PRO v1.4.24</title>'), 'title should show v1.4.24');
assert(html.includes('data-build="1.4.24"'), 'body data-build should be 1.4.24');
assert(html.includes('버전 정보 v1.4.24'), 'visible version button should show v1.4.24');
assert(html.includes('FoxBear Mastering PRO v1.4.24'), 'program info should show v1.4.24');
assert(app.includes("const APP_VERSION = 'Pro v1.4.24'"), 'app version constant should be Pro v1.4.24');
...
assert(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker cache should use latest v1.4.24 line');
...
console.log('PASS v1.4.24 version release smoke');
