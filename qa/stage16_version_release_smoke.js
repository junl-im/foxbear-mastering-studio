...
const version = '1.4.0';
const assetVersion = '1.4.0-stage16-version-release';
...
assert(pkg.description.includes('v1.4.0') && pkg.description.includes('Stage16'), 'package description should identify Stage16 v1.4.0');
...
assert(config.includes(`const ASSET_VERSION = '${assetVersion}'`), 'runtime config asset version should be Stage16');
assert(health.includes(`const FALLBACK_VERSION = '${assetVersion}'`), 'runtime health fallback should be Stage16');
assert(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker cache should use v1.4.0 Stage16');
assert(app.includes(`navigator.serviceWorker.register('./sw.js?v=${assetVersion}')`), 'service worker registration should use Stage16 query');
['src/app.js', 'src/boot/runtime-health.js', 'assets/css/mobile-native.css', 'manifest.webmanifest'].forEach(asset => {
  assert(html.includes(`${asset}?v=${assetVersion}`), `${asset} should use Stage16 query in index.html`);
  assert(sw.includes(`./${asset}?v=${assetVersion}`), `${asset} should use Stage16 query in sw.js`);
});
assert(overwrite.includes('v1.4.0-stage16'), 'overwrite package default should be v1.4.0-stage16');
...
console.log('PASS Stage16 v1.4.0 version release smoke');
