...
assert(pkg.version === version, 'package version should be 1.4.23');
assert(pkg.description.includes('v1.4.23') && pkg.description.includes('Spectrum'), 'package description should identify latest v1.4.23 line');
assert(manifest.version === version, 'manifest should carry version 1.4.23');
assert(html.includes('<title>FoxBear Mastering PRO v1.4.23</title>'), 'title should show v1.4.23');
assert(html.includes('data-build="1.4.23"'), 'body data-build should be 1.4.23');
assert(html.includes('버전 정보 v1.4.23'), 'visible version button should show v1.4.23');
assert(html.includes('FoxBear Mastering PRO v1.4.23'), 'program info should show v1.4.23');
assert(app.includes("const APP_VERSION = 'Pro v1.4.23'"), 'app version constant should be Pro v1.4.23');
...
assert(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker cache should use latest v1.4.23 line');
...
console.log('PASS v1.4.23 version release smoke');
