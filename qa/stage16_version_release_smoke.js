...
const version = '1.4.8';
const assetVersion = '1.4.8-dock-spectrum-cleanup';

assert(pkg.name === 'foxbear-github-pro-v1-4-8', 'package name should be v1.4.8');
assert(pkg.version === version, 'package version should be 1.4.8');
assert(pkg.description.includes('v1.4.8') && pkg.description.includes('Spectrum'), 'package description should identify latest v1.4.8 line');
assert(manifest.version === version, 'manifest should carry version 1.4.8');
assert(html.includes('<title>FoxBear Mastering PRO v1.4.8</title>'), 'title should show v1.4.8');
assert(html.includes('data-build="1.4.8"'), 'body data-build should be 1.4.8');
assert(html.includes('버전 정보 v1.4.8'), 'visible version button should show v1.4.8');
assert(html.includes('FoxBear Mastering PRO v1.4.8'), 'program info should show v1.4.8');
assert(app.includes("const APP_VERSION = 'Pro v1.4.8'"), 'app version constant should be Pro v1.4.8');
...
assert(sw.includes(`foxbear-shell-v${assetVersion}`), 'service worker cache should use latest v1.4.8 line');
...
assert(overwrite.includes('v1.4.8'), 'overwrite package default should be latest v1.4.8 line');
...
console.log('PASS v1.4.8 version release smoke');
