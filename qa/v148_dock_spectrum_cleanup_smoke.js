...
const version = '1.6.41-admin-secret-pin-session';

must(pkg.version === '1.6.41', 'package version should be 1.6.41');
must(pkg.name === 'foxbear-mastering-studio', 'package name should be v1.6.41');
must(index.includes('data-build="1.6.41"'), 'index build marker should be 1.6.41');
must(app.includes("const APP_VERSION = 'Pro v1.6.41'"), 'app version should be Pro v1.6.41');
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.6.41 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker should use v1.6.41 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache v1.6.41 spectrum visualizer');
...
must(changelog.includes('v1.6.41') && changelog.includes('renderMini'), 'changelog should document v1.6.41 renderMini cleanup');
must(handoff.includes('v1.6.41') && handoff.includes('detail-only FFT'), 'handoff should mention detail-only FFT');
must(notes.includes('renderMini') && notes.includes('removed'), 'project notes should record renderMini removal');
must(readme.includes('v1.6.41') && readme.includes('detail-only'), 'README should summarize v1.6.41 detail-only FFT');
must(pkg.qaChecks.includes('node qa/v148_dock_spectrum_cleanup_smoke.js'), 'package should run v1.6.41 smoke');
