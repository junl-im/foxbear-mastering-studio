...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be v1.5.60');
...
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.5.60 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker should use v1.5.60 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache v1.5.60 spectrum visualizer');
...
must(changelog.includes('v1.5.60') && changelog.includes('renderMini'), 'changelog should document v1.5.60 renderMini cleanup');
must(handoff.includes('v1.5.60') && handoff.includes('detail-only FFT'), 'handoff should mention detail-only FFT');
must(notes.includes('renderMini') && notes.includes('removed'), 'project notes should record renderMini removal');
must(readme.includes('v1.5.60') && readme.includes('detail-only'), 'README should summarize v1.5.60 detail-only FFT');
must(pkg.qaChecks.includes('node qa/v148_dock_spectrum_cleanup_smoke.js'), 'package should run v1.5.60 smoke');
