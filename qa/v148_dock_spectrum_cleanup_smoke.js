...
must(pkg.name === 'foxbear-mastering-studio', 'package name should be v1.5.12');
must(index.includes('data-build="1.5.12"'), 'index build marker should be 1.5.12');
must(app.includes("const APP_VERSION = 'Pro v1.5.12'"), 'app version should be Pro v1.5.12');
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.5.12 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker should use v1.5.12 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache v1.5.12 spectrum visualizer');
...
must(changelog.includes('v1.5.12') && changelog.includes('renderMini'), 'changelog should document v1.5.12 renderMini cleanup');
must(handoff.includes('v1.5.12') && handoff.includes('detail-only FFT'), 'handoff should mention detail-only FFT');
must(notes.includes('renderMini') && notes.includes('removed'), 'project notes should record renderMini removal');
must(readme.includes('v1.5.12') && readme.includes('detail-only'), 'README should summarize v1.5.12 detail-only FFT');
must(pkg.qaChecks.includes('node qa/v148_dock_spectrum_cleanup_smoke.js'), 'package should run v1.5.12 smoke');
