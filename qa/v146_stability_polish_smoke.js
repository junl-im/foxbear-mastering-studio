...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.23 stability polish smoke: ${message}`);
    process.exit(1);
  }
};
...
must(app.includes("const APP_VERSION = 'Pro v1.4.23'"), 'app version should be Pro v1.4.23');
must(index.includes('data-build="1.4.23"'), 'index build marker should be 1.4.23');
must(index.includes('1.4.23-audio-decode-memory-guard'), 'index should use v1.4.23 cache key');
must(sw.includes('foxbear-shell-v1.4.23-audio-decode-memory-guard'), 'service worker cache should use v1.4.23 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.4.23-audio-decode-memory-guard'), 'service worker should precache v1.4.23 spectrum visualizer');
...
must(matrix.includes('v1.4.23') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.4.23 Dock FFT/back confirm focus');
must(changelog.includes('v1.4.23') && changelog.includes('stability'), 'changelog should include v1.4.23 stability entry');
must(handoff.includes('v1.4.23') && handoff.includes('stability'), 'handoff should include v1.4.23 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.4.23 polish smoke');

console.log('PASS v1.4.23 stability polish smoke');
