...
must(index.includes('1.6.59-readiness-corp-security-hardening'), 'index should use v1.6.59 cache key');
must(sw.includes('foxbear-shell-v1.6.59-readiness-corp-security-hardening'), 'service worker cache should use v1.6.59 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.59-readiness-corp-security-hardening'), 'service worker should precache v1.6.59 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.59 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.59') && changelog.includes('stability'), 'changelog should include v1.6.59 stability entry');
must(handoff.includes('v1.6.59') && handoff.includes('stability'), 'handoff should include v1.6.59 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.59 polish smoke');
