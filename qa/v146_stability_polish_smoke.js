...
must(index.includes('1.6.87-mobile-header-device-label-overlap-recovery'), 'index should use v1.6.87 cache key');
...
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.87-mobile-header-device-label-overlap-recovery'), 'service worker should precache v1.6.87 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.87 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.87') && changelog.includes('stability'), 'changelog should include v1.6.87 stability entry');
must(handoff.includes('v1.6.87') && handoff.includes('stability'), 'handoff should include v1.6.87 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.87 polish smoke');
