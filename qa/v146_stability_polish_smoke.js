...
must(index.includes('1.6.66-static-gate-hygiene-repair'), 'index should use v1.6.66 cache key');
must(sw.includes('foxbear-shell-v1.6.66-static-gate-hygiene-repair'), 'service worker cache should use v1.6.66 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.66-static-gate-hygiene-repair'), 'service worker should precache v1.6.66 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.66 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.66') && changelog.includes('stability'), 'changelog should include v1.6.66 stability entry');
must(handoff.includes('v1.6.66') && handoff.includes('stability'), 'handoff should include v1.6.66 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.66 polish smoke');
