...
must(pkg.version === '1.5.74', 'package version should be 1.5.74');
must(app.includes("const APP_VERSION = 'Pro v1.5.74'"), 'app version should be Pro v1.5.74');
must(index.includes('data-build="1.5.74"'), 'index build marker should be 1.5.74');
must(index.includes('1.5.74-bulk-pause-skip-reorder-mobile-download'), 'index should use v1.5.74 cache key');
must(sw.includes('foxbear-shell-v1.5.74-bulk-pause-skip-reorder-mobile-download'), 'service worker cache should use v1.5.74 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.5.74-bulk-pause-skip-reorder-mobile-download'), 'service worker should precache v1.5.74 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.5.74 Dock FFT/back confirm focus');
must(changelog.includes('v1.5.74') && changelog.includes('stability'), 'changelog should include v1.5.74 stability entry');
must(handoff.includes('v1.5.74') && handoff.includes('stability'), 'handoff should include v1.5.74 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.5.74 polish smoke');
