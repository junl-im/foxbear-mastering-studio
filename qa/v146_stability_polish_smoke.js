...
must(app.includes("const APP_VERSION = 'Pro v1.6.75'"), 'app version should be Pro v1.6.75');
must(index.includes('data-build="1.6.75"'), 'index build marker should be 1.6.75');
must(index.includes('1.6.75-download-progress-admission-fallback-closure'), 'index should use v1.6.75 cache key');
must(sw.includes('foxbear-shell-v1.6.75-download-progress-admission-fallback-closure'), 'service worker cache should use v1.6.75 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.75-download-progress-admission-fallback-closure'), 'service worker should precache v1.6.75 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.75 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.75') && changelog.includes('stability'), 'changelog should include v1.6.75 stability entry');
must(handoff.includes('v1.6.75') && handoff.includes('stability'), 'handoff should include v1.6.75 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.75 polish smoke');
