...
must(pkg.version === '1.5.67', 'package version should be 1.5.67');
must(app.includes("const APP_VERSION = 'Pro v1.5.67'"), 'app version should be Pro v1.5.67');
must(index.includes('data-build="1.5.67"'), 'index build marker should be 1.5.67');
must(index.includes('1.5.67-incident-admin-audit-webhook-failover-index-health'), 'index should use v1.5.67 cache key');
must(sw.includes('foxbear-shell-v1.5.67-incident-admin-audit-webhook-failover-index-health'), 'service worker cache should use v1.5.67 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.5.67-incident-admin-audit-webhook-failover-index-health'), 'service worker should precache v1.5.67 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.5.67 Dock FFT/back confirm focus');
must(changelog.includes('v1.5.67') && changelog.includes('stability'), 'changelog should include v1.5.67 stability entry');
must(handoff.includes('v1.5.67') && handoff.includes('stability'), 'handoff should include v1.5.67 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.5.67 polish smoke');
