...
must(index.includes('1.5.91-cancellable-audio-pipeline-performance-guards'), 'index should use v1.5.91 cache key');
...
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.5.91-cancellable-audio-pipeline-performance-guards'), 'service worker should precache v1.5.91 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.5.91 Dock FFT/back confirm focus');
must(changelog.includes('v1.5.91') && changelog.includes('stability'), 'changelog should include v1.5.91 stability entry');
must(handoff.includes('v1.5.91') && handoff.includes('stability'), 'handoff should include v1.5.91 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.5.91 polish smoke');
