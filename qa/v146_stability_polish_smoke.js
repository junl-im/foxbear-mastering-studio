...
must(index.includes('1.6.65-firestore-write-fencing'), 'index should use v1.6.65 cache key');
must(sw.includes('foxbear-shell-v1.6.65-firestore-write-fencing'), 'service worker cache should use v1.6.65 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.65-firestore-write-fencing'), 'service worker should precache v1.6.65 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.65 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.65') && changelog.includes('stability'), 'changelog should include v1.6.65 stability entry');
must(handoff.includes('v1.6.65') && handoff.includes('stability'), 'handoff should include v1.6.65 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.65 polish smoke');
