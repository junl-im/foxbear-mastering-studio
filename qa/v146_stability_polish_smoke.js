...
must(pkg.version === '1.6.45', 'package version should be 1.6.45');
must(app.includes("const APP_VERSION = 'Pro v1.6.45'"), 'app version should be Pro v1.6.45');
must(index.includes('data-build="1.6.45"'), 'index build marker should be 1.6.45');
must(index.includes('1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'index should use v1.6.45 cache key');
must(sw.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'service worker cache should use v1.6.45 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'service worker should precache v1.6.45 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.6.45 Dock FFT/back confirm focus');
must(changelog.includes('v1.6.45') && changelog.includes('stability'), 'changelog should include v1.6.45 stability entry');
must(handoff.includes('v1.6.45') && handoff.includes('stability'), 'handoff should include v1.6.45 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.6.45 polish smoke');
