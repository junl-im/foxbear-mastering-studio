...
must(index.includes('1.5.58-kakao-mastering-runtime-recovery'), 'index should use v1.5.58 cache key');
must(sw.includes('foxbear-shell-v1.5.58-kakao-mastering-runtime-recovery'), 'service worker cache should use v1.5.58 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.5.58-kakao-mastering-runtime-recovery'), 'service worker should precache v1.5.58 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.5.58 Dock FFT/back confirm focus');
must(changelog.includes('v1.5.58') && changelog.includes('stability'), 'changelog should include v1.5.58 stability entry');
must(handoff.includes('v1.5.58') && handoff.includes('stability'), 'handoff should include v1.5.58 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.5.58 polish smoke');
