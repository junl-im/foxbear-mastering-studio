...
must(pkg.version === '1.5.82', 'package version should be 1.5.82');
must(app.includes("const APP_VERSION = 'Pro v1.5.82'"), 'app version should be Pro v1.5.82');
must(index.includes('data-build="1.5.82"'), 'index build marker should be 1.5.82');
must(index.includes('1.5.82-mastering-cancel-playback-resume-recovery'), 'index should use v1.5.82 cache key');
must(sw.includes('foxbear-shell-v1.5.82-mastering-cancel-playback-resume-recovery'), 'service worker cache should use v1.5.82 key');
must(sw.includes('./src/ui/spectrum-visualizer.js?v=1.5.82-mastering-cancel-playback-resume-recovery'), 'service worker should precache v1.5.82 spectrum visualizer');
...
must(matrix.includes('v1.4.26') && matrix.includes('confirm') && matrix.includes('Dock mini FFT'), 'QA matrix should mention v1.5.82 Dock FFT/back confirm focus');
must(changelog.includes('v1.5.82') && changelog.includes('stability'), 'changelog should include v1.5.82 stability entry');
must(handoff.includes('v1.5.82') && handoff.includes('stability'), 'handoff should include v1.5.82 stability entry');
must(pkg.qaChecks.includes('node qa/v146_stability_polish_smoke.js'), 'package should run v1.5.82 polish smoke');
