...
async function runLowCopyWorkerRuntime() {
  ...
  let timeoutHandle;
  const timeoutPromise = new Promise((_, reject) => { timeoutHandle = setTimeout(() => reject(new Error('low-copy worker timeout')), 3000); });
...
async function main() {
  ...
  assert(pkg.qaChecks.includes('node qa/v1543_export_pipeline_integrity_smoke.js'), 'v1.6.102 QA is not registered');
