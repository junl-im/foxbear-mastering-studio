...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.7.1');
assert(index.includes('data-build="1.7.1"'), 'index build should be v1.7.1');
assert(index.includes('1.7.1-reference-match-2-phase1'), 'index should use v1.7.1 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.7.1-reference-match-2-phase1'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.7.1'"), 'download diagnostics/flow should report v1.7.1');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.7.1 action row');
