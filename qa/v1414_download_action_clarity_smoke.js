...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.5.16');
assert(index.includes('data-build="1.5.16"'), 'index build should be v1.5.16');
assert(index.includes('1.5.16-e2e-server-pipe-deadlock-fix'), 'index should use v1.5.16 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.5.16-e2e-server-pipe-deadlock-fix'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.5.16'"), 'download diagnostics/flow should report v1.5.16');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.5.16 action row');
