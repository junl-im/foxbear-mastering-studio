...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.5.53');
assert(index.includes('data-build="1.5.53"'), 'index build should be v1.5.53');
assert(index.includes('1.5.53-engine-recovery-performance-diagnostics'), 'index should use v1.5.53 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.5.53-engine-recovery-performance-diagnostics'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.5.53'"), 'download diagnostics/flow should report v1.5.53');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.5.53 action row');
