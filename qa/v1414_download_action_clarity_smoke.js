...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.7.2');
assert(index.includes('data-build="1.7.2"'), 'index build should be v1.7.2');
assert(index.includes('1.7.2-expert-workspace-default-entry'), 'index should use v1.7.2 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.7.2-expert-workspace-default-entry'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.7.2'"), 'download diagnostics/flow should report v1.7.2');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.7.2 action row');
