...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.22');
assert(index.includes('data-build="1.6.22"'), 'index build should be v1.6.22');
assert(index.includes('1.6.22-incident-recovery-coalescing-time-decay'), 'index should use v1.6.22 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.22-incident-recovery-coalescing-time-decay'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.22'"), 'download diagnostics/flow should report v1.6.22');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.22 action row');
