...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.5.51');
assert(index.includes('data-build="1.5.51"'), 'index build should be v1.5.51');
assert(index.includes('1.5.51-ci-runtime-contract-hardening'), 'index should use v1.5.51 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.5.51-ci-runtime-contract-hardening'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.5.51'"), 'download diagnostics/flow should report v1.5.51');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.5.51 action row');
