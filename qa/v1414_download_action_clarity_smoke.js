...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.45');
assert(index.includes('data-build="1.6.45"'), 'index build should be v1.6.45');
assert(index.includes('1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'index should use v1.6.45 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.45'"), 'download diagnostics/flow should report v1.6.45');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.45 action row');
