...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.61');
assert(index.includes('data-build="1.6.61"'), 'index build should be v1.6.61');
assert(index.includes('1.6.61-human-readable-download-filenames'), 'index should use v1.6.61 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.61-human-readable-download-filenames'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.61'"), 'download diagnostics/flow should report v1.6.61');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.61 action row');
