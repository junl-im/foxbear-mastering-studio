...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.63');
assert(index.includes('data-build="1.6.63"'), 'index build should be v1.6.63');
assert(index.includes('1.6.63-download-filename-review-hardening'), 'index should use v1.6.63 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.63-download-filename-review-hardening'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.63'"), 'download diagnostics/flow should report v1.6.63');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.63 action row');
