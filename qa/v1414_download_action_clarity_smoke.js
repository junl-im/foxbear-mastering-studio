...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.14 download action clarity smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(pkg.name === 'foxbear-github-pro-v1-4-14', 'package name should match v1.4.14');
assert(index.includes('data-build="1.4.14"'), 'index build should be v1.4.14');
assert(index.includes('1.4.14-download-action-clarity'), 'index should use v1.4.14 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.4.14-download-action-clarity'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.4.14'"), 'download diagnostics/flow should report v1.4.14');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.4.14 action row');
...
assert(matrix.includes('v1.4.14 Download action clarity'), 'QA matrix should document action clarity');
...
console.log('PASS v1.4.14 download action clarity smoke');
