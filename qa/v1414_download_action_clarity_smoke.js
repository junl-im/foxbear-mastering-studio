...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.39');
assert(index.includes('data-build="1.6.39"'), 'index build should be v1.6.39');
assert(index.includes('1.6.39-ui-shell-partial-script-probe-isolation'), 'index should use v1.6.39 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.39-ui-shell-partial-script-probe-isolation'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.39'"), 'download diagnostics/flow should report v1.6.39');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.39 action row');
