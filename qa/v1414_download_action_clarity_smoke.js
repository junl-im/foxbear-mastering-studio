...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.6.85');
assert(index.includes('data-build="1.6.85"'), 'index build should be v1.6.85');
assert(index.includes('1.6.85-browser-sentinel-ui-mode-header-recovery'), 'index should use v1.6.85 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.6.85-browser-sentinel-ui-mode-header-recovery'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.6.85'"), 'download diagnostics/flow should report v1.6.85');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.6.85 action row');
