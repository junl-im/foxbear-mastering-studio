...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.7.0');
assert(index.includes('data-build="1.7.0"'), 'index build should be v1.7.0');
assert(index.includes('1.7.0-adaptive-mastering-decision-phase1'), 'index should use v1.7.0 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.7.0-adaptive-mastering-decision-phase1'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.7.0'"), 'download diagnostics/flow should report v1.7.0');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.7.0 action row');
