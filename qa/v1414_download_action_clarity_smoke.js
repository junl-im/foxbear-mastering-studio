...
assert(pkg.name === 'foxbear-mastering-studio', 'package name should match v1.5.93');
assert(index.includes('data-build="1.5.93"'), 'index build should be v1.5.93');
assert(index.includes('1.5.93-external-engine-transfer-admin-export-openai-readiness'), 'index should use v1.5.93 action clarity cache key');
assert(sw.includes('foxbear-shell-v1.5.93-external-engine-transfer-admin-export-openai-readiness'), 'service worker should use action clarity cache key');

assert(service.includes("version: '1.5.93'"), 'download diagnostics/flow should report v1.5.93');
...
assert(css.includes('.download-options-actions-v1414'), 'CSS should style v1.5.93 action row');
