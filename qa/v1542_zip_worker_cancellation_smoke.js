...
async function main() {
  ...
  assert(index.includes('id="exportProgressCancel"') && index.includes('src/download/zip-export-service.js?v=1.6.80-ai-mastering-expert-workspace'), 'ZIP cancel UI/service asset missing');
  ...
  assert(sw.includes("'./src/workers/zip-encoder.worker.js'") && sw.includes("'./src/workers/zip-encoder.worker.js?v=1.6.80-ai-mastering-expert-workspace'"), 'versioned ZIP worker is not cached by the service worker');
  assert(pkg.qaChecks.includes('node --check src/download/zip-export-service.js') && pkg.qaChecks.includes('node qa/v1542_zip_worker_cancellation_smoke.js'), 'v1.6.80 QA is not registered');
