...
async function main() {
  ...
  assert(index.includes('id="exportProgressCancel"') && index.includes('src/download/zip-export-service.js?v=1.5.61-worker-mail-delivery-recovery'), 'ZIP cancel UI/service asset missing');
  ...
  assert(sw.includes("'./src/workers/zip-encoder.worker.js'") && sw.includes("'./src/workers/zip-encoder.worker.js?v=1.5.61-worker-mail-delivery-recovery'"), 'versioned ZIP worker is not cached by the service worker');
  assert(pkg.qaChecks.includes('node --check src/download/zip-export-service.js') && pkg.qaChecks.includes('node qa/v1542_zip_worker_cancellation_smoke.js'), 'v1.5.61 QA is not registered');
