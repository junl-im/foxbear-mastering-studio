...
assert(html.includes('aria-hidden="true" data-required="false" hidden>'), 'startup chooser must be hidden in initial HTML');
...
assert(serviceSource.includes("setAttribute('data-ui-mode-pref', restored || MODES.EXPERT)"));
...
assert.strictEqual(snapshot.chooserOpen, false, 'first visit must enter the full expert workspace directly');
assert.strictEqual(snapshot.chooserRequired, false, 'startup chooser must not require a choice');
assert.strictEqual(controller.closeChooser(), true, 'hidden optional chooser can remain closed');
...
delete fakeWindow.__FOXBEAR_PENDING_UI_MODE__;
