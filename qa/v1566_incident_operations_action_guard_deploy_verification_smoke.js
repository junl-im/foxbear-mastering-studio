...
assert.strictEqual(meta.assetVersion, '1.6.71-pwa-share-lease-handoff-deploy-policy-e2e');
...
]) assert(functionsSource.includes(token), `v1.6.71 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.71 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.71 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.71 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.71 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.71 Firestore rule missing ${token}`);
