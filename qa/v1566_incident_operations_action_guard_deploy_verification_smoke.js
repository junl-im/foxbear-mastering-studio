...
assert.strictEqual(meta.assetVersion, '1.6.102-admin-lazyload-sw-hygiene-hardening');
...
]) assert(functionsSource.includes(token), `v1.6.102 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.102 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.102 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.102 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.102 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.102 Firestore rule missing ${token}`);
