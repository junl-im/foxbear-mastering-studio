...
assert.strictEqual(meta.assetVersion, '1.6.49-download-variant-cache-reuse');
...
]) assert(functionsSource.includes(token), `v1.6.49 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.49 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.49 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.49 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.49 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.49 Firestore rule missing ${token}`);
