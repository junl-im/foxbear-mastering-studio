...
assert.strictEqual(meta.assetVersion, '1.7.4-reload-reentry-mode-chooser');
...
]) assert(functionsSource.includes(token), `v1.7.4 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.7.4 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.7.4 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.7.4 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.7.4 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.7.4 Firestore rule missing ${token}`);
