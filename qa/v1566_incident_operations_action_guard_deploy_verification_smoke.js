...
]) assert(functionsSource.includes(token), `v1.6.75 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.75 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.75 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.75 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.75 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.75 Firestore rule missing ${token}`);
