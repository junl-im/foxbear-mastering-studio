...
assert.strictEqual(meta.assetVersion, '1.5.92-python-bytecode-ci-hygiene-node24-cache');
...
]) assert(functionsSource.includes(token), `v1.5.92 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.5.92 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.5.92 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.5.92 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.5.92 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.5.92 Firestore rule missing ${token}`);
