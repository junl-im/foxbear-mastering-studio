...
assert.strictEqual(meta.assetVersion, '1.6.10-incident-readiness-contract-csp-cache-hardening');
...
]) assert(functionsSource.includes(token), `v1.6.10 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.10 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.10 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.10 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.10 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.10 Firestore rule missing ${token}`);
