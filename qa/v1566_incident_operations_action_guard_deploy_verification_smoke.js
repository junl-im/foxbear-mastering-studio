...
assert.strictEqual(meta.assetVersion, '1.6.80-ai-mastering-expert-workspace');
...
]) assert(functionsSource.includes(token), `v1.6.80 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.80 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.80 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.80 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.80 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.80 Firestore rule missing ${token}`);
