...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.7 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.7 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.7 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.7 Firestore rule missing ${token}`);
