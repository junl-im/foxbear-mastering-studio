...
assert.strictEqual(meta.assetVersion, '1.6.65-firestore-write-fencing');
...
]) assert(functionsSource.includes(token), `v1.6.65 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.65 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.65 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.65 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.65 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.65 Firestore rule missing ${token}`);
