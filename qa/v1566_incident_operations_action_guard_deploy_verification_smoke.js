...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.5.68 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.5.68 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.5.68 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.5.68 Firestore rule missing ${token}`);
