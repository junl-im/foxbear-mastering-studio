...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.4 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.4 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.4 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.4 Firestore rule missing ${token}`);
