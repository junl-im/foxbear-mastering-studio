...
]) assert(functionsSource.includes(token), `v1.6.37 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.37 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.37 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.37 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.37 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.37 Firestore rule missing ${token}`);
