...
assert.strictEqual(meta.assetVersion, '1.6.108-mastering-progress-visibility-hardening');
...
]) assert(functionsSource.includes(token), `v1.6.108 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.108 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.108 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.108 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.108 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.108 Firestore rule missing ${token}`);
