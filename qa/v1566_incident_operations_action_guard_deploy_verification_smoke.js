...
assert.strictEqual(meta.assetVersion, '1.6.91-runtime-health-hidden-geometry-contract-recovery');
...
]) assert(functionsSource.includes(token), `v1.6.91 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.91 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.91 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.91 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.91 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.91 Firestore rule missing ${token}`);
