...
assert.strictEqual(meta.assetVersion, '1.7.2-expert-workspace-default-entry');
...
]) assert(functionsSource.includes(token), `v1.7.2 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.7.2 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.7.2 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.7.2 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.7.2 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.7.2 Firestore rule missing ${token}`);
