...
assert.strictEqual(meta.assetVersion, '1.6.42-spark-google-admin-auth');
...
]) assert(functionsSource.includes(token), `v1.6.42 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.6.42 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.6.42 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.6.42 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.6.42 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.6.42 Firestore rule missing ${token}`);
