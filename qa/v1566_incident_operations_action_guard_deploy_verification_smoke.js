...
assert.strictEqual(meta.assetVersion, '1.5.90-browser-retry-integrity-metadata-aware-scope');
...
]) assert(functionsSource.includes(token), `v1.5.90 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.5.90 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.5.90 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.5.90 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.5.90 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.5.90 Firestore rule missing ${token}`);
