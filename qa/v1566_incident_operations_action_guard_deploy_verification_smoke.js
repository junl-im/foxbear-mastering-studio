...
assert.strictEqual(meta.assetVersion, '1.5.93-external-engine-transfer-admin-export-openai-readiness');
...
]) assert(functionsSource.includes(token), `v1.5.93 function contract missing ${token}`);
...
for (const token of [
  ...
  'recommendedActions',
  'setDoc'
]) assert(firebaseSource.includes(token), `v1.5.93 Firebase bridge contract missing ${token}`);
...
for (const id of [
  ...
  assert(indexSource.includes(`id="${id}"`), `v1.5.93 HTML missing ${id}`);
  assert(appSource.includes(`'${id}'`), `v1.5.93 app element cache missing ${id}`);
}
...
]) assert(monitorSource.includes(token), `v1.5.93 monitor contract missing ${token}`);
...
]) assert(rules.includes(token), `v1.5.93 Firestore rule missing ${token}`);
