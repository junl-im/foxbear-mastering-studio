...
must(pkg.version==='1.6.78','package version');
must(html.includes('data-build="1.6.78"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.78'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.78-release-generation-assist-url-playback-invalidation'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.78-release-generation-assist-url-playback-invalidation'),'apple icon cache key');
