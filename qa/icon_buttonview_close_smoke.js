...
must(pkg.version==='1.5.52','package version');
must(html.includes('data-build="1.5.52"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.52'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.52-ci-parallel-release-gate'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.52-ci-parallel-release-gate'),'apple icon cache key');
