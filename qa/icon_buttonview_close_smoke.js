...
must(pkg.version==='1.6.63','package version');
must(html.includes('data-build="1.6.63"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.63'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.63-download-filename-review-hardening'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.63-download-filename-review-hardening'),'apple icon cache key');
