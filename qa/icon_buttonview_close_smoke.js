...
must(pkg.version==='1.6.20','package version');
must(html.includes('data-build="1.6.20"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.20'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.20-incident-background-sync-network-decay'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.20-incident-background-sync-network-decay'),'apple icon cache key');
