...
must(pkg.version==='1.5.54','package version');
must(html.includes('data-build="1.5.54"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.54'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.54-quality-recovery-profiles-browser-qa'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.54-quality-recovery-profiles-browser-qa'),'apple icon cache key');
