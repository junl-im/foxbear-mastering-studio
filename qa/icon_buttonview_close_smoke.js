...
must(pkg.version==='1.4.26','package version');
must(html.includes('data-build="1.4.26"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.4.26'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.4.26-wake-lock-state-sync'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.4.26-wake-lock-state-sync'),'apple icon cache key');
