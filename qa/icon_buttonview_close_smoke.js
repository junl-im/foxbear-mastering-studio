...
must(pkg.version==='1.6.50','package version');
must(html.includes('data-build="1.6.50"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.50'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.50-kakao-centered-entry-notice'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.50-kakao-centered-entry-notice'),'apple icon cache key');
