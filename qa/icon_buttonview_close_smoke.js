...
must(pkg.version==='1.6.97','package version');
must(html.includes('data-build="1.6.97"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.97'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.97-boot-payload-delivery-privacy-hardening'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.97-boot-payload-delivery-privacy-hardening'),'apple icon cache key');
