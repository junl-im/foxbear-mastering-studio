...
must(pkg.version==='1.5.57','package version');
must(html.includes('data-build="1.5.57"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.57'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.57-modal-close-consistency'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.57-modal-close-consistency'),'apple icon cache key');
