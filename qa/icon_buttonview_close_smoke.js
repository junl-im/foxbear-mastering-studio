...
must(pkg.version==='1.7.2','package version');
must(html.includes('data-build="1.7.2"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.7.2'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.7.2-expert-workspace-default-entry'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.7.2-expert-workspace-default-entry'),'apple icon cache key');
