...
must(pkg.version==='1.5.24','package version');
must(html.includes('data-build="1.5.24"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.24'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.24-e2e-responsive-preview-control'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.24-e2e-responsive-preview-control'),'apple icon cache key');
