...
must(pkg.version==='1.6.25','package version');
must(html.includes('data-build="1.6.25"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.25'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.25-incident-recovery-timeout-abort-stress'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.25-incident-recovery-timeout-abort-stress'),'apple icon cache key');
