...
must(pkg.version==='1.5.43','package version');
must(html.includes('data-build="1.5.43"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.43'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.43-export-pipeline-integrity'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.43-export-pipeline-integrity'),'apple icon cache key');
