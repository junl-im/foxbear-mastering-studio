...
must(pkg.version==='1.6.9','package version');
must(html.includes('data-build="1.6.9"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.9'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.9-incident-readiness-history-recovery-copy-events'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.9-incident-readiness-history-recovery-copy-events'),'apple icon cache key');
