...
must(pkg.version === '1.3.80', 'package version should be 1.3.80');
must(html.includes('data-build="1.3.80"'), 'index build should be v1.3.80');
must(app.includes("const APP_VERSION = 'Pro v1.3.80'"), 'app version should be Pro v1.3.80');
must(app.includes("const SHARED_DSP_PROFILE_VERSION = 'v1.3.80-icons-buttonview-close'"), 'DSP slug should be v1.3.80');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.3.80-icons-buttonview-close'), '512 icon link should use new icon');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.3.80-icons-buttonview-close'), 'apple touch icon should use generated icon');
...
must(app.includes("['pointerdown', 'mousedown', 'click', 'touchend']"), 'feature dialog fallback must cover pointer/mouse/click/touch close paths');
must(css.includes('v1.3.80 Icon Refresh / Button View Close Repair'), 'CSS close repair section missing');
must(css.includes('#featureDialogClose') && css.includes('z-index: 24760'), 'close button z-index/pointer fix missing');
