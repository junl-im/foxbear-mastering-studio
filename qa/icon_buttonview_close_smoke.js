...
must(pkg.version==='1.6.2','package version');
must(html.includes('data-build="1.6.2"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.2'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.2-nonblocking-health-status-design-polish'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.2-nonblocking-health-status-design-polish'),'apple icon cache key');
