...
must(pkg.version==='1.5.99','package version');
must(html.includes('data-build="1.5.99"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.5.99'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.5.99-incident-callable-mail-recovery'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.5.99-incident-callable-mail-recovery'),'apple icon cache key');
