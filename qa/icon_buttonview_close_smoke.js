...
must(pkg.version==='1.6.100','package version');
must(html.includes('data-build="1.6.100"'),'build');
must(app.includes("const APP_VERSION = 'Pro v1.6.100'"),'app version');
must(html.includes('assets/icons/foxbear-icon-512.png?v=1.6.100-sw-stereo-ci-cleanup-hardening'),'512 icon cache key');
must(html.includes('assets/icons/apple-touch-icon.png?v=1.6.100-sw-stereo-ci-cleanup-hardening'),'apple icon cache key');
