...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.11 stability audit smoke: ${message}`);
    process.exit(1);
  }
};
...
must(html.includes('data-build="1.4.11"'), 'index build marker should be 1.4.11');
must(html.includes('1.4.11-download-share-reliability'), 'index should use v1.4.11 cache key');
...
console.log('PASS v1.4.11 stability audit smoke');
