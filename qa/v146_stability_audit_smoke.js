...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.12 stability audit smoke: ${message}`);
    process.exit(1);
  }
};
...
must(html.includes('data-build="1.4.12"'), 'index build marker should be 1.4.12');
must(html.includes('1.4.12-download-diagnostics'), 'index should use v1.4.12 cache key');
...
console.log('PASS v1.4.12 stability audit smoke');
