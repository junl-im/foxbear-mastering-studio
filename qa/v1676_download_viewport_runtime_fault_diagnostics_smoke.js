...
assert.strictEqual(pkg.version, '1.6.90', 'package version must be v1.6.90');
...
const sw = read('sw.js');
assert(index.includes('src/boot/runtime-fault-counters.js?v=1.6.90-engine-control-overlay-isolation-header-contract-recovery'), 'runtime fault counters must load from index');
assert(sw.includes('./src/boot/runtime-fault-counters.js?v=1.6.90-engine-control-overlay-isolation-header-contract-recovery'), 'runtime fault counters must be precached');
...
assert(pkg.qaChecks.includes('node qa/v1676_download_viewport_runtime_fault_diagnostics_smoke.js'), 'v1.6.90 smoke must be registered');
