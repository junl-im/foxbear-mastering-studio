...
must(app.includes("const APP_VERSION = 'Pro v1.6.34'"),'version');
must(html.includes('data-build="1.6.34"'),'build');
must(pkg.version==='1.6.34','package version');
must(sw.includes('foxbear-shell-v1.6.34-history-hard-stall-sw-activity-lifecycle'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.34-history-hard-stall-sw-activity-lifecycle'),'dock controller loaded');
