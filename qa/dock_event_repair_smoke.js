...
must(app.includes("const APP_VERSION = 'Pro v1.6.87'"),'version');
must(html.includes('data-build="1.6.87"'),'build');
must(pkg.version==='1.6.87','package version');
must(sw.includes('foxbear-shell-v1.6.87-mobile-header-device-label-overlap-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.87-mobile-header-device-label-overlap-recovery'),'dock controller loaded');
