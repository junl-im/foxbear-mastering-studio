...
must(app.includes("const APP_VERSION = 'Pro v1.5.92'"),'version');
must(html.includes('data-build="1.5.92"'),'build');
must(pkg.version==='1.5.92','package version');
must(sw.includes('foxbear-shell-v1.5.92-python-bytecode-ci-hygiene-node24-cache'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.92-python-bytecode-ci-hygiene-node24-cache'),'dock controller loaded');
