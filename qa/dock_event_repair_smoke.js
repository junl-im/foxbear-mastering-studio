...
must(app.includes("const APP_VERSION = 'Pro v1.5.74'"),'version');
must(html.includes('data-build="1.5.74"'),'build');
must(pkg.version==='1.5.74','package version');
must(sw.includes('foxbear-shell-v1.5.74-bulk-pause-skip-reorder-mobile-download'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.74-bulk-pause-skip-reorder-mobile-download'),'dock controller loaded');
