...
must(app.includes("const APP_VERSION = 'Pro v1.6.59'"),'version');
must(html.includes('data-build="1.6.59"'),'build');
must(pkg.version==='1.6.59','package version');
must(sw.includes('foxbear-shell-v1.6.59-readiness-corp-security-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.59-readiness-corp-security-hardening'),'dock controller loaded');
