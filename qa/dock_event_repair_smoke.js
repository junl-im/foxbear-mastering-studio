...
must(app.includes("const APP_VERSION = 'Pro v1.7.4'"),'version');
must(html.includes('data-build="1.7.4"'),'build');
must(pkg.version==='1.7.4','package version');
must(sw.includes('foxbear-shell-v1.7.4-reload-reentry-mode-chooser'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.7.4-reload-reentry-mode-chooser'),'dock controller loaded');
