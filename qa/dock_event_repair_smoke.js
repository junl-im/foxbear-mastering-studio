...
must(app.includes("const APP_VERSION = 'Pro v1.6.22'"),'version');
must(html.includes('data-build="1.6.22"'),'build');
must(pkg.version==='1.6.22','package version');
must(sw.includes('foxbear-shell-v1.6.22-incident-recovery-coalescing-time-decay'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.22-incident-recovery-coalescing-time-decay'),'dock controller loaded');
