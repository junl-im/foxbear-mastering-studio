...
must(app.includes("const APP_VERSION = 'Pro v1.4.12'"),'version');
must(html.includes('data-build="1.4.12"'),'build');
must(pkg.version==='1.4.12','package version');
must(sw.includes('foxbear-shell-v1.4.12-download-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.4.12-download-diagnostics'),'dock controller loaded');
