...
must(app.includes("const APP_VERSION = 'Pro v1.5.16'"),'version');
must(html.includes('data-build="1.5.16"'),'build');
must(pkg.version==='1.5.16','package version');
must(sw.includes('foxbear-shell-v1.5.16-e2e-server-pipe-deadlock-fix'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.16-e2e-server-pipe-deadlock-fix'),'dock controller loaded');
