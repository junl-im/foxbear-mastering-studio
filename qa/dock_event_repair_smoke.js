...
must(app.includes("const APP_VERSION = 'Pro v1.5.94'"),'version');
must(html.includes('data-build="1.5.94"'),'build');
must(pkg.version==='1.5.94','package version');
must(sw.includes('foxbear-shell-v1.5.94-aiff-fallback-worker-diagnostics-reporting-contract'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.94-aiff-fallback-worker-diagnostics-reporting-contract'),'dock controller loaded');
