...
must(app.includes("const APP_VERSION = 'Pro v1.5.59'"),'version');
must(html.includes('data-build="1.5.59"'),'build');
must(pkg.version==='1.5.59','package version');
must(sw.includes('foxbear-shell-v1.5.59-kakao-session-handoff-memory-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.59-kakao-session-handoff-memory-diagnostics'),'dock controller loaded');
