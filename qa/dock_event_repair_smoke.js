...
must(app.includes("const APP_VERSION = 'Pro v1.5.41'"),'version');
must(html.includes('data-build="1.5.41"'),'build');
must(pkg.version==='1.5.41','package version');
must(sw.includes('foxbear-shell-v1.5.41-export-eta-download-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.41-export-eta-download-recovery'),'dock controller loaded');
