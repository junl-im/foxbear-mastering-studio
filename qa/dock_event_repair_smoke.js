...
must(app.includes("const APP_VERSION = 'Pro v1.7.2'"),'version');
must(html.includes('data-build="1.7.2"'),'build');
must(pkg.version==='1.7.2','package version');
must(sw.includes('foxbear-shell-v1.7.2-expert-workspace-default-entry'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.7.2-expert-workspace-default-entry'),'dock controller loaded');
