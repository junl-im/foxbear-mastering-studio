...
must(app.includes("const APP_VERSION = 'Pro v1.6.45'"),'version');
must(html.includes('data-build="1.6.45"'),'build');
must(pkg.version==='1.6.45','package version');
must(sw.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.45-windows-release-gate-spark-hosting-no-app-check'),'dock controller loaded');
