...
must(app.includes("const APP_VERSION = 'Pro v1.5.67'"),'version');
must(html.includes('data-build="1.5.67"'),'build');
must(pkg.version==='1.5.67','package version');
must(sw.includes('foxbear-shell-v1.5.67-incident-admin-audit-webhook-failover-index-health'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.67-incident-admin-audit-webhook-failover-index-health'),'dock controller loaded');
