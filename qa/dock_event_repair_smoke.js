...
must(app.includes("const APP_VERSION = 'Pro v1.5.65'"),'version');
must(html.includes('data-build="1.5.65"'),'build');
must(pkg.version==='1.5.65','package version');
must(sw.includes('foxbear-shell-v1.5.65-incident-recovery-control-alert-history'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.65-incident-recovery-control-alert-history'),'dock controller loaded');
