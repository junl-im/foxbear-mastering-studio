...
must(app.includes("const APP_VERSION = 'Pro v1.6.37'"),'version');
must(html.includes('data-build="1.6.37"'),'build');
must(pkg.version==='1.6.37','package version');
must(sw.includes('foxbear-shell-v1.6.37-ui-shell-cross-generation-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.37-ui-shell-cross-generation-recovery'),'dock controller loaded');
