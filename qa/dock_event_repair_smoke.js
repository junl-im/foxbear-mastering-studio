...
must(app.includes("const APP_VERSION = 'Pro v1.4.26'"),'version');
must(html.includes('data-build="1.4.26"'),'build');
must(pkg.version==='1.4.26','package version');
must(sw.includes('foxbear-shell-v1.4.26-wake-lock-state-sync'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.4.26-wake-lock-state-sync'),'dock controller loaded');
