...
must(app.includes("const APP_VERSION = 'Pro v1.5.22'"),'version');
must(html.includes('data-build="1.5.22"'),'build');
must(pkg.version==='1.5.22','package version');
must(sw.includes('foxbear-shell-v1.5.22-header-preview-routing-polish'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.22-header-preview-routing-polish'),'dock controller loaded');
