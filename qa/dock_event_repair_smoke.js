...
must(app.includes("const APP_VERSION = 'Pro v1.5.52'"),'version');
must(html.includes('data-build="1.5.52"'),'build');
must(pkg.version==='1.5.52','package version');
must(sw.includes('foxbear-shell-v1.5.52-ci-parallel-release-gate'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.52-ci-parallel-release-gate'),'dock controller loaded');
