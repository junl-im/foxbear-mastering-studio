...
must(app.includes("const APP_VERSION = 'Pro v1.5.97'"),'version');
must(html.includes('data-build="1.5.97"'),'build');
must(pkg.version==='1.5.97','package version');
must(sw.includes('foxbear-shell-v1.5.97-worker-recovery-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.97-worker-recovery-diagnostics'),'dock controller loaded');
