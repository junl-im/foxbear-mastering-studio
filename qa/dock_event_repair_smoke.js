...
must(app.includes("const APP_VERSION = 'Pro v1.5.23'"),'version');
must(html.includes('data-build="1.5.23"'),'build');
must(pkg.version==='1.5.23','package version');
must(sw.includes('foxbear-shell-v1.5.23-e2e-preview-readiness'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.23-e2e-preview-readiness'),'dock controller loaded');
