...
must(app.includes("const APP_VERSION = 'Pro v1.6.75'"),'version');
must(html.includes('data-build="1.6.75"'),'build');
must(pkg.version==='1.6.75','package version');
must(sw.includes('foxbear-shell-v1.6.75-download-progress-admission-fallback-closure'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.75-download-progress-admission-fallback-closure'),'dock controller loaded');
