...
must(app.includes("const APP_VERSION = 'Pro v1.5.69'"),'version');
must(html.includes('data-build="1.5.69"'),'build');
must(pkg.version==='1.5.69','package version');
must(sw.includes('foxbear-shell-v1.5.69-mail-receipt-confirmation-history-branded-template'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.69-mail-receipt-confirmation-history-branded-template'),'dock controller loaded');
