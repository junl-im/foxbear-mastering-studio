...
must(app.includes("const APP_VERSION = 'Pro v1.5.25'"),'version');
must(html.includes('data-build="1.5.25"'),'build');
must(pkg.version==='1.5.25','package version');
must(sw.includes('foxbear-shell-v1.5.25-e2e-preview-stability'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.25-e2e-preview-stability'),'dock controller loaded');
