...
must(app.includes("const APP_VERSION = 'Pro v1.6.20'"),'version');
must(html.includes('data-build="1.6.20"'),'build');
must(pkg.version==='1.6.20','package version');
must(sw.includes('foxbear-shell-v1.6.20-incident-background-sync-network-decay'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.20-incident-background-sync-network-decay'),'dock controller loaded');
