...
must(app.includes("const APP_VERSION = 'Pro v1.5.30'"),'version');
must(html.includes('data-build="1.5.30"'),'build');
must(pkg.version==='1.5.30','package version');
must(sw.includes('foxbear-shell-v1.5.30-inapp-playback-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.30-inapp-playback-recovery'),'dock controller loaded');
