...
must(app.includes("const APP_VERSION = 'Pro v1.6.88'"),'version');
must(html.includes('data-build="1.6.88"'),'build');
must(pkg.version==='1.6.88','package version');
must(sw.includes('foxbear-shell-v1.6.88-mobile-header-grid-ownership-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.88-mobile-header-grid-ownership-recovery'),'dock controller loaded');
