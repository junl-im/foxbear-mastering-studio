...
must(app.includes("const APP_VERSION = 'Pro v1.6.49'"),'version');
must(html.includes('data-build="1.6.49"'),'build');
must(pkg.version==='1.6.49','package version');
must(sw.includes('foxbear-shell-v1.6.49-download-variant-cache-reuse'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.49-download-variant-cache-reuse'),'dock controller loaded');
