...
must(app.includes("const APP_VERSION = 'Pro v1.5.98'"),'version');
must(html.includes('data-build="1.5.98"'),'build');
must(pkg.version==='1.5.98','package version');
must(sw.includes('foxbear-shell-v1.5.98-worker-retry-health-levels'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.98-worker-retry-health-levels'),'dock controller loaded');
