...
must(app.includes("const APP_VERSION = 'Pro v1.5.38'"),'version');
must(html.includes('data-build="1.5.38"'),'build');
must(pkg.version==='1.5.38','package version');
must(sw.includes('foxbear-shell-v1.5.38-preflight-worker-multitab-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.38-preflight-worker-multitab-hardening'),'dock controller loaded');
