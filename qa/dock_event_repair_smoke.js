...
must(app.includes("const APP_VERSION = 'Pro v1.5.24'"),'version');
must(html.includes('data-build="1.5.24"'),'build');
must(pkg.version==='1.5.24','package version');
must(sw.includes('foxbear-shell-v1.5.24-e2e-responsive-preview-control'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.24-e2e-responsive-preview-control'),'dock controller loaded');
