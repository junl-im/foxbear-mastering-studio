...
must(app.includes("const APP_VERSION = 'Pro v1.6.90'"),'version');
must(html.includes('data-build="1.6.90"'),'build');
must(pkg.version==='1.6.90','package version');
must(sw.includes('foxbear-shell-v1.6.90-engine-control-overlay-isolation-header-contract-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.90-engine-control-overlay-isolation-header-contract-recovery'),'dock controller loaded');
