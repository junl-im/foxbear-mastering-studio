...
must(app.includes("const APP_VERSION = 'Pro v1.5.81'"),'version');
must(html.includes('data-build="1.5.81"'),'build');
must(pkg.version==='1.5.81','package version');
must(sw.includes('foxbear-shell-v1.5.81-master-preview-cancellation-native-result-isolation'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.81-master-preview-cancellation-native-result-isolation'),'dock controller loaded');
