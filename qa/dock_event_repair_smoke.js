...
must(app.includes("const APP_VERSION = 'Pro v1.5.56'"),'version');
must(html.includes('data-build="1.5.56"'),'build');
must(pkg.version==='1.5.56','package version');
must(sw.includes('foxbear-shell-v1.5.56-incident-operations-app-check'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.56-incident-operations-app-check'),'dock controller loaded');
