...
must(app.includes("const APP_VERSION = 'Pro v1.5.50'"),'version');
must(html.includes('data-build="1.5.50"'),'build');
must(pkg.version==='1.5.50','package version');
must(sw.includes('foxbear-shell-v1.5.50-pwa-current-cache-test-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.50-pwa-current-cache-test-recovery'),'dock controller loaded');
