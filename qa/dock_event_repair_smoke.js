...
must(app.includes("const APP_VERSION = 'Pro v1.4.23'"),'version');
must(html.includes('data-build="1.4.23"'),'build');
must(pkg.version==='1.4.23','package version');
must(sw.includes('foxbear-shell-v1.4.23-audio-decode-memory-guard'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.4.23-audio-decode-memory-guard'),'dock controller loaded');
