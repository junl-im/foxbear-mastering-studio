...
must(app.includes("const APP_VERSION = 'Pro v1.6.71'"),'version');
must(html.includes('data-build="1.6.71"'),'build');
must(pkg.version==='1.6.71','package version');
must(sw.includes('foxbear-shell-v1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'),'dock controller loaded');
