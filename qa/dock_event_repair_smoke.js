...
must(app.includes("const APP_VERSION = 'Pro v1.5.66'"),'version');
must(html.includes('data-build="1.5.66"'),'build');
must(pkg.version==='1.5.66','package version');
must(sw.includes('foxbear-shell-v1.5.66-incident-operations-action-guard-deploy-verification'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.66-incident-operations-action-guard-deploy-verification'),'dock controller loaded');
