...
must(app.includes("const APP_VERSION = 'Pro v1.5.27'"),'version');
must(html.includes('data-build="1.5.27"'),'build');
must(pkg.version==='1.5.27','package version');
must(sw.includes('foxbear-shell-v1.5.27-device-glyph-sri-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.27-device-glyph-sri-hardening'),'dock controller loaded');
