...
must(app.includes("const APP_VERSION = 'Pro v1.5.99'"),'version');
must(html.includes('data-build="1.5.99"'),'build');
must(pkg.version==='1.5.99','package version');
must(sw.includes('foxbear-shell-v1.5.99-incident-callable-mail-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.99-incident-callable-mail-recovery'),'dock controller loaded');
