...
must(app.includes("const APP_VERSION = 'Pro v1.6.46'"),'version');
must(html.includes('data-build="1.6.46"'),'build');
must(pkg.version==='1.6.46','package version');
must(sw.includes('foxbear-shell-v1.6.46-google-auth-same-origin-network-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.46-google-auth-same-origin-network-recovery'),'dock controller loaded');
