...
must(app.includes("const APP_VERSION = 'Pro v1.6.1'"),'version');
must(html.includes('data-build="1.6.1"'),'build');
must(pkg.version==='1.6.1','package version');
must(sw.includes('foxbear-shell-v1.6.1-transient-performance-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.1-transient-performance-diagnostics'),'dock controller loaded');
