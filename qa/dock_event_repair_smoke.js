...
must(app.includes("const APP_VERSION = 'Pro v1.6.47'"),'version');
must(html.includes('data-build="1.6.47"'),'build');
must(pkg.version==='1.6.47','package version');
must(sw.includes('foxbear-shell-v1.6.47-external-host-admin-auth-opaque-error-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.47-external-host-admin-auth-opaque-error-recovery'),'dock controller loaded');
