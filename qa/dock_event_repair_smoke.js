...
must(app.includes("const APP_VERSION = 'Pro v1.6.2'"),'version');
must(html.includes('data-build="1.6.2"'),'build');
must(pkg.version==='1.6.2','package version');
must(sw.includes('foxbear-shell-v1.6.2-nonblocking-health-status-design-polish'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.2-nonblocking-health-status-design-polish'),'dock controller loaded');
