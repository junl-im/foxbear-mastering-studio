...
must(app.includes("const APP_VERSION = 'Pro v1.6.25'"),'version');
must(html.includes('data-build="1.6.25"'),'build');
must(pkg.version==='1.6.25','package version');
must(sw.includes('foxbear-shell-v1.6.25-incident-recovery-timeout-abort-stress'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.25-incident-recovery-timeout-abort-stress'),'dock controller loaded');
