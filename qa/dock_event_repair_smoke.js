...
must(app.includes("const APP_VERSION = 'Pro v1.5.26'"),'version');
must(html.includes('data-build="1.5.26"'),'build');
must(pkg.version==='1.5.26','package version');
must(sw.includes('foxbear-shell-v1.5.26-engraved-command-header'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.26-engraved-command-header'),'dock controller loaded');
