...
must(pkg.version === '1.3.73', 'package version should be v1.3.73');
must(sw.includes('foxbear-shell-v1.3.73-dock-event-repair'), 'SW cache key should be v1.3.73');
must(html.includes('src/app.js?v=1.3.73-dock-event-repair'), 'app cache-bust key should be v1.3.73');
...
must(app.includes('const track = activateMainTrackFromDock(resolveMainActiveTrackForDock());\n    if (!track) {\n        showToast(\'비교할 음원을 먼저 불러와주세요.\');'), 'waveform popup should use active Dock/main track with feedback');
must(dockCss.includes('v1.3.73 Dock event repair'), 'dock css should include event repair section');
