...
must(app.includes("const APP_VERSION = 'Pro v1.5.64'"),'version');
must(html.includes('data-build="1.5.64"'),'build');
must(pkg.version==='1.5.64','package version');
must(sw.includes('foxbear-shell-v1.5.64-incident-operations-health-self-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.64-incident-operations-health-self-diagnostics'),'dock controller loaded');
