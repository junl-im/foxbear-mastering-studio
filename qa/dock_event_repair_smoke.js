...
must(app.includes("const APP_VERSION = 'Pro v1.6.41'"),'version');
must(html.includes('data-build="1.6.41"'),'build');
must(pkg.version==='1.6.41','package version');
must(sw.includes('foxbear-shell-v1.6.41-admin-secret-pin-session'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.41-admin-secret-pin-session'),'dock controller loaded');
