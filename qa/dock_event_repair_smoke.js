...
must(app.includes("const APP_VERSION = 'Pro v1.5.53'"),'version');
must(html.includes('data-build="1.5.53"'),'build');
must(pkg.version==='1.5.53','package version');
must(sw.includes('foxbear-shell-v1.5.53-engine-recovery-performance-diagnostics'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.53-engine-recovery-performance-diagnostics'),'dock controller loaded');
