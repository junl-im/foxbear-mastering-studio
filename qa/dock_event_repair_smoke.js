...
must(app.includes("const APP_VERSION = 'Pro v1.6.100'"),'version');
must(html.includes('data-build="1.6.100"'),'build');
must(pkg.version==='1.6.100','package version');
must(sw.includes('foxbear-shell-v1.6.100-sw-stereo-ci-cleanup-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.100-sw-stereo-ci-cleanup-hardening'),'dock controller loaded');
