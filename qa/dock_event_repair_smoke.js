...
must(app.includes("const APP_VERSION = 'Pro v1.6.102'"),'version');
must(html.includes('data-build="1.6.102"'),'build');
must(pkg.version==='1.6.102','package version');
must(sw.includes('foxbear-shell-v1.6.102-admin-lazyload-sw-hygiene-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.102-admin-lazyload-sw-hygiene-hardening'),'dock controller loaded');
