...
must(app.includes("const APP_VERSION = 'Pro v1.5.12'"),'version');
must(html.includes('data-build="1.5.12"'),'build');
must(pkg.version==='1.5.12','package version');
must(sw.includes('foxbear-shell-v1.5.12-ci-runtime-readiness'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.12-ci-runtime-readiness'),'dock controller loaded');
