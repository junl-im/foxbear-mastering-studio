...
must(app.includes("const APP_VERSION = 'Pro v1.6.89'"),'version');
must(html.includes('data-build="1.6.89"'),'build');
must(pkg.version==='1.6.89','package version');
must(sw.includes('foxbear-shell-v1.6.89-mobile-header-flex-ownership-browser-gate-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.89-mobile-header-flex-ownership-browser-gate-recovery'),'dock controller loaded');
