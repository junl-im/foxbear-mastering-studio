...
must(app.includes("const APP_VERSION = 'Pro v1.4.10'"),'version');
must(html.includes('data-build="1.4.10"'),'build');
must(pkg.version==='1.4.10','package version');
must(sw.includes('foxbear-shell-v1.4.10-perf-polish'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.4.10-perf-polish'),'dock controller loaded');
