...
must(app.includes("const APP_VERSION = 'Pro v1.6.111'"),'version');
must(html.includes('data-build="1.6.111"'),'build');
must(pkg.version==='1.6.111','package version');
must(sw.includes('foxbear-shell-v1.6.111-ui-mode-session-contract-hardening'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.111-ui-mode-session-contract-hardening'),'dock controller loaded');
