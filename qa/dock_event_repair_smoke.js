...
must(app.includes("const APP_VERSION = 'Pro v1.5.32'"),'version');
must(html.includes('data-build="1.5.32"'),'build');
must(pkg.version==='1.5.32','package version');
must(sw.includes('foxbear-shell-v1.5.32-kakao-external-browser-local-flow'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.32-kakao-external-browser-local-flow'),'dock controller loaded');
