...
must(app.includes("const APP_VERSION = 'Pro v1.5.84'"),'version');
must(html.includes('data-build="1.5.84"'),'build');
must(pkg.version==='1.5.84','package version');
must(sw.includes('foxbear-shell-v1.5.84-trusted-types-browser-gate-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.84-trusted-types-browser-gate-recovery'),'dock controller loaded');
