...
must(app.includes("const APP_VERSION = 'Pro v1.5.90'"),'version');
must(html.includes('data-build="1.5.90"'),'build');
must(pkg.version==='1.5.90','package version');
must(sw.includes('foxbear-shell-v1.5.90-browser-retry-integrity-metadata-aware-scope'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.90-browser-retry-integrity-metadata-aware-scope'),'dock controller loaded');
