...
must(app.includes("const APP_VERSION = 'Pro v1.5.91'"),'version');
must(html.includes('data-build="1.5.91"'),'build');
must(pkg.version==='1.5.91','package version');
must(sw.includes('foxbear-shell-v1.5.91-cancellable-audio-pipeline-performance-guards'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.91-cancellable-audio-pipeline-performance-guards'),'dock controller loaded');
