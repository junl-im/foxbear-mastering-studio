...
must(app.includes("const APP_VERSION = 'Pro v1.5.20'"),'version');
must(html.includes('data-build="1.5.20"'),'build');
must(pkg.version==='1.5.20','package version');
must(sw.includes('foxbear-shell-v1.5.20-idempotent-pwa-cache-warm'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.20-idempotent-pwa-cache-warm'),'dock controller loaded');
