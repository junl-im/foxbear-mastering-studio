...
must(app.includes("const APP_VERSION = 'Pro v1.5.60'"),'version');
must(html.includes('data-build="1.5.60"'),'build');
must(pkg.version==='1.5.60','package version');
must(sw.includes('foxbear-shell-v1.5.60-kakao-inapp-entry-memory-governor'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.60-kakao-inapp-entry-memory-governor'),'dock controller loaded');
