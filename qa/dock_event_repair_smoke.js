...
must(app.includes("const APP_VERSION = 'Pro v1.5.73'"),'version');
must(html.includes('data-build="1.5.73"'),'build');
must(pkg.version==='1.5.73','package version');
must(sw.includes('foxbear-shell-v1.5.73-bulk-control-eta-result-filter-ui'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.73-bulk-control-eta-result-filter-ui'),'dock controller loaded');
