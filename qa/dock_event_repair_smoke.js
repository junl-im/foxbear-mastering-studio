...
must(app.includes("const APP_VERSION = 'Pro v1.6.7'"),'version');
must(html.includes('data-build="1.6.7"'),'build');
must(pkg.version==='1.6.7','package version');
must(sw.includes('foxbear-shell-v1.6.7-incident-readiness-history-sync-performance-hud'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.7-incident-readiness-history-sync-performance-hud'),'dock controller loaded');
