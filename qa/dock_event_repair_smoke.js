...
must(app.includes("const APP_VERSION = 'Pro v1.4.14'"),'version');
must(html.includes('data-build="1.4.14"'),'build');
must(pkg.version==='1.4.14','package version');
must(sw.includes('foxbear-shell-v1.4.14-download-action-clarity'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.4.14-download-action-clarity'),'dock controller loaded');
