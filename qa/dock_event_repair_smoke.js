...
must(app.includes("const APP_VERSION = 'Pro v1.6.95'"),'version');
must(html.includes('data-build="1.6.95"'),'build');
must(pkg.version==='1.6.95','package version');
must(sw.includes('foxbear-shell-v1.6.95-release-artifact-safety'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.95-release-artifact-safety'),'dock controller loaded');
