...
must(app.includes("const APP_VERSION = 'Pro v1.6.101'"),'version');
must(html.includes('data-build="1.6.101"'),'build');
must(pkg.version==='1.6.101','package version');
must(sw.includes('foxbear-shell-v1.6.101-admin-lazyload-repo-cleanup'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.101-admin-lazyload-repo-cleanup'),'dock controller loaded');
