...
must(app.includes("const APP_VERSION = 'Pro v1.6.9'"),'version');
must(html.includes('data-build="1.6.9"'),'build');
must(pkg.version==='1.6.9','package version');
must(sw.includes('foxbear-shell-v1.6.9-incident-readiness-history-recovery-copy-events'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.6.9-incident-readiness-history-recovery-copy-events'),'dock controller loaded');
