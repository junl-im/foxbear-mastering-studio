...
must(app.includes("const APP_VERSION = 'Pro v1.5.49'"),'version');
must(html.includes('data-build="1.5.49"'),'build');
must(pkg.version==='1.5.49','package version');
must(sw.includes('foxbear-shell-v1.5.49-asset-generation-route-recovery'),'sw cache');
must(html.includes('src/ui/dock-controller.js?v=1.5.49-asset-generation-route-recovery'),'dock controller loaded');
