...
(async () => {
  ...
  assert.strictEqual(publicFirst.service.serviceSchemaVersion, 8);
  assert.strictEqual(publicFirst.service.smtpReadinessRequiresAdmin, true);
  assert.strictEqual(publicFirst.checks.mailRouting.status, 'restricted');
  ...
  assert.strictEqual(adminFirst.checks.mailRouting.status, 'warning');
  assert.strictEqual(adminFirst.checks.mailRouting.fallbackActive, true);
  assert.strictEqual(adminFirst.checks.mailRouting.recipientSource, 'fallback');
  assert.strictEqual(adminFirst.checks.mailRouting.senderSource, 'fallback');
  assert(!JSON.stringify(adminFirst.checks.mailRouting).includes('@'), 'mail routing readiness must not expose addresses');
