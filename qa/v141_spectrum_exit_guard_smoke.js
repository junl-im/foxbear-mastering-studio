...
must(pkg.qaChecks.includes('node qa/v141_spectrum_exit_guard_smoke.js'), 'package should run v1.5.65 smoke');
...
must(handoff.includes('v1.5.65') && handoff.includes('Spectrum'), 'handoff should mention v1.5.65 spectrum update');
must(notes.includes('Exit Guard'), 'project notes should preserve the Exit Guard invariant');
must(changelog.includes('v1.5.65') && changelog.includes('Spectrum'), 'changelog should include v1.5.65 entry');
