...
async function main() {
    ...
    assert.strictEqual(policy.version, '1.6.43');
    ...
    assert(handoff.includes('- Product version: `1.6.43`'));
    assert(handoff.includes(`- Configured static/regression target: ${pkg.qaChecks.length} checks.`));
    assert(desktop.startsWith('# GitHub Desktop Handoff - v1.6.43'));
    for (const heading of ['## 1. 작업한 내역', '## 2. 다운로드 파일 2종', '## 3. 다음 예정 내역']) assert(deliveryRules.includes(heading));
