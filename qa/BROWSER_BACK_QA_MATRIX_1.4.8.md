# Browser / PWA QA Matrix - v1.4.18 Dock Spectrum Cleanup

v1.4.18 finishes the Dock FFT removal by deleting the leftover mini-spectrum API/runtime requirement. The persistent Dock should stay focused on playback, waveform, source switching, and export actions. The FFT spectrum remains available only in the detail analysis panel.

| Environment | Refresh / close expectation | Back expectation | v1.4.18 checks |
| --- | --- | --- | --- |
...
Compatibility note: older v1.4.6/v1.4.7 QA smoke still phrases this area as "Dock mini FFT"; in v1.4.18 the expected result is that Dock mini FFT remains removed while Back confirm behavior still works.
