...
assert(!changelog.includes('## v1.6.104 carry-forward anchors') && !changelog.includes('## v1.6.104 Carry-forward Documentation Anchors'), 'carry-forward anchor sections should not return');
...
assert(runner.includes('resolvePlaywrightCli') && runner.includes("'@playwright/test/cli'") && !runner.includes("spawnSync('npx'"), 'browser runner must use pinned local Playwright CLI through the lazy resolver');
...
assert((pkg.qaChecks || []).includes('node qa/v157_release_foundation_smoke.js'), 'v1.6.104 smoke missing from qaChecks');
