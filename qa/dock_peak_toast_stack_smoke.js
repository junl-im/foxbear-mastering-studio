...
expect(index.includes('v1.3.73'), 'index should show v1.3.73');
expect(index.includes('1.3.73-dock-event-repair'), 'index asset query should use v1.3.73 cache key');
expect(pkg.version === '1.3.73', 'package version should be 1.3.73');
expect(app.includes("const APP_VERSION = 'Pro v1.3.73'"), 'app version constant should be v1.3.73');
...
expect(/function onBottomWaveformButtonClick\(event\)\s*{\s*event\?\.preventDefault\?\.\(\);\s*event\?\.stopPropagation\?\.\(\);\s*openWaveformCompareDialog\(\);\s*}/s.test(app), 'Dock waveform click should always open compare popup');
...
expect(dockCss.includes('Dock event repair'), 'dock css should include v1.3.73 section');
