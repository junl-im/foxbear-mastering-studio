...
must(pkg.version === '1.6.112', 'package version should be 1.6.112');
must(pkg.name === 'foxbear-mastering-studio', 'package name should use v1-4-26');
must(pkg.qaChecks.includes('node qa/v1425_exit_guard_fallback_smoke.js'), 'package should run v1.6.112 smoke');
must(index.includes('data-build="1.6.112"'), 'index build marker should be 1.6.112');
must(index.includes('src/security/site-guards.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'site guard cache key should be updated');
must(index.includes('assets/css/studio.css?v=1.6.112-mastering-lifecycle-race-hardening'), 'studio CSS cache key should be updated');
must(app.includes("const APP_VERSION = 'Pro v1.6.112'"), 'app version should be Pro v1.6.112');
must(sw.includes('foxbear-shell-v1.6.112-mastering-lifecycle-race-hardening'), 'service worker cache should use v1.6.112 key');
...
must(changelog.includes('v1.6.112') && changelog.includes('Exit Guard'), 'changelog should mention v1.6.112 Exit Guard');
must(handoff.includes('v1.6.112') && handoff.includes('뒤로가기'), 'handoff should mention v1.6.112 back navigation');
must(notes.includes('fallback'), 'project notes should preserve exit fallback guidance');
must(matrix.includes('v1.4.26') && matrix.includes('exit fallback'), 'matrix should cover v1.6.112 exit fallback');
