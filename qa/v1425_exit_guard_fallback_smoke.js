...
must(pkg.qaChecks.includes('node qa/v1425_exit_guard_fallback_smoke.js'), 'package should run v1.6.83 smoke');
must(index.includes('data-build="1.6.83"'), 'index build marker should be 1.6.83');
must(index.includes('src/security/site-guards.js?v=1.6.83-browser-ui-mode-fixture-source-hygiene-recovery'), 'site guard cache key should be updated');
must(index.includes('assets/css/studio.css?v=1.6.83-browser-ui-mode-fixture-source-hygiene-recovery'), 'studio CSS cache key should be updated');
...
must(changelog.includes('v1.6.83') && changelog.includes('Exit Guard'), 'changelog should mention v1.6.83 Exit Guard');
must(handoff.includes('v1.6.83') && handoff.includes('뒤로가기'), 'handoff should mention v1.6.83 back navigation');
must(notes.includes('fallback'), 'project notes should preserve exit fallback guidance');
must(matrix.includes('v1.4.26') && matrix.includes('exit fallback'), 'matrix should cover v1.6.83 exit fallback');
