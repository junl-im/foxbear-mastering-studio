...
must(pkg.qaChecks.includes('node qa/v1425_exit_guard_fallback_smoke.js'), 'package should run v1.5.66 smoke');
must(index.includes('data-build="1.5.66"'), 'index build marker should be 1.5.66');
must(index.includes('src/security/site-guards.js?v=1.5.66-incident-operations-action-guard-deploy-verification'), 'site guard cache key should be updated');
must(index.includes('assets/css/studio.css?v=1.5.66-incident-operations-action-guard-deploy-verification'), 'studio CSS cache key should be updated');
...
must(changelog.includes('v1.5.66') && changelog.includes('Exit Guard'), 'changelog should mention v1.5.66 Exit Guard');
must(handoff.includes('v1.5.66') && handoff.includes('뒤로가기'), 'handoff should mention v1.5.66 back navigation');
must(notes.includes('fallback'), 'project notes should preserve exit fallback guidance');
must(matrix.includes('v1.4.26') && matrix.includes('exit fallback'), 'matrix should cover v1.5.66 exit fallback');
