...
must(pkg.qaChecks.includes('node qa/v1425_exit_guard_fallback_smoke.js'), 'package should run v1.5.11 smoke');
must(index.includes('data-build="1.5.11"'), 'index build marker should be 1.5.11');
must(index.includes('src/security/site-guards.js?v=1.5.11-audio-context-ci-stability'), 'site guard cache key should be updated');
must(index.includes('assets/css/studio.css?v=1.5.11-audio-context-ci-stability'), 'studio CSS cache key should be updated');
must(app.includes("const APP_VERSION = 'Pro v1.5.11'"), 'app version should be Pro v1.5.11');
must(sw.includes('foxbear-shell-v1.5.11-audio-context-ci-stability'), 'service worker cache should use v1.5.11 key');
...
must(changelog.includes('v1.5.11') && changelog.includes('Exit Guard'), 'changelog should mention v1.5.11 Exit Guard');
must(handoff.includes('v1.5.11') && handoff.includes('뒤로가기'), 'handoff should mention v1.5.11 back navigation');
must(notes.includes('v1.5.11') && notes.includes('fallback'), 'project notes should mention v1.5.11 fallback');
must(matrix.includes('v1.4.26') && matrix.includes('exit fallback'), 'matrix should cover v1.5.11 exit fallback');
