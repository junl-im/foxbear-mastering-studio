...
must(pkg.version === '1.7.0', 'package version should be 1.7.0');
must(pkg.name === 'foxbear-mastering-studio', 'package name should use v1-4-26');
must(pkg.qaChecks.includes('node qa/v1425_exit_guard_fallback_smoke.js'), 'package should run v1.7.0 smoke');
must(index.includes('data-build="1.7.0"'), 'index build marker should be 1.7.0');
must(index.includes('src/security/site-guards.js?v=1.7.0-adaptive-mastering-decision-phase1'), 'site guard cache key should be updated');
must(index.includes('assets/css/studio.css?v=1.7.0-adaptive-mastering-decision-phase1'), 'studio CSS cache key should be updated');
must(app.includes("const APP_VERSION = 'Pro v1.7.0'"), 'app version should be Pro v1.7.0');
must(sw.includes('foxbear-shell-v1.7.0-adaptive-mastering-decision-phase1'), 'service worker cache should use v1.7.0 key');
...
must(changelog.includes('v1.7.0') && changelog.includes('Exit Guard'), 'changelog should mention v1.7.0 Exit Guard');
must(handoff.includes('v1.7.0') && handoff.includes('뒤로가기'), 'handoff should mention v1.7.0 back navigation');
must(notes.includes('fallback'), 'project notes should preserve exit fallback guidance');
must(matrix.includes('v1.4.26') && matrix.includes('exit fallback'), 'matrix should cover v1.7.0 exit fallback');
