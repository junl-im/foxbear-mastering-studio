...
assert(index.includes('data-build="1.5.14"'), 'index build should be 1.5.14');
assert(index.includes('1.5.14-github-desktop-handoff-preflight'), 'asset cache key should use v1.5.14 flow polish');
assert(sw.includes('foxbear-shell-v1.5.14-github-desktop-handoff-preflight'), 'service worker cache should use v1.5.14 flow polish');
...
assert(service.includes("version: '1.5.14'"), 'download flow/diagnostics should report v1.5.14');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.14 flow polish');
