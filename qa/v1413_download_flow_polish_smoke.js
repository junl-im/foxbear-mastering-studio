...
assert(index.includes('data-build="1.5.16"'), 'index build should be 1.5.16');
assert(index.includes('1.5.16-e2e-server-pipe-deadlock-fix'), 'asset cache key should use v1.5.16 flow polish');
assert(sw.includes('foxbear-shell-v1.5.16-e2e-server-pipe-deadlock-fix'), 'service worker cache should use v1.5.16 flow polish');
...
assert(service.includes("version: '1.5.16'"), 'download flow/diagnostics should report v1.5.16');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.16 flow polish');
