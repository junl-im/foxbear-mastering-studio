...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.20 download flow polish smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(index.includes('data-build="1.4.20"'), 'index build should be 1.4.20');
assert(index.includes('1.4.20-bulk-import-guard'), 'asset cache key should use v1.4.20 flow polish');
assert(sw.includes('foxbear-shell-v1.4.20-bulk-import-guard'), 'service worker cache should use v1.4.20 flow polish');
...
assert(service.includes("version: '1.4.20'"), 'download flow/diagnostics should report v1.4.20');
...
assert(matrix.includes('v1.4.20 Download flow polish'), 'QA matrix should document v1.4.20 flow polish');
assert(matrix.includes('Advanced actions are hidden behind'), 'QA matrix should include advanced action collapse scenario');

console.log('PASS v1.4.20 download flow polish smoke');
