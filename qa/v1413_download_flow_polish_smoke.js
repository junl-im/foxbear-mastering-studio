...
assert(index.includes('data-build="1.5.53"'), 'index build should be 1.5.53');
assert(index.includes('1.5.53-engine-recovery-performance-diagnostics'), 'asset cache key should use v1.5.53 flow polish');
assert(sw.includes('foxbear-shell-v1.5.53-engine-recovery-performance-diagnostics'), 'service worker cache should use v1.5.53 flow polish');
...
assert(service.includes("version: '1.5.53'"), 'download flow/diagnostics should report v1.5.53');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.53 flow polish');
