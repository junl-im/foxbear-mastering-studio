...
assert(index.includes('data-build="1.5.20"'), 'index build should be 1.5.20');
assert(index.includes('1.5.20-idempotent-pwa-cache-warm'), 'asset cache key should use v1.5.20 flow polish');
assert(sw.includes('foxbear-shell-v1.5.20-idempotent-pwa-cache-warm'), 'service worker cache should use v1.5.20 flow polish');
...
assert(service.includes("version: '1.5.20'"), 'download flow/diagnostics should report v1.5.20');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.20 flow polish');
