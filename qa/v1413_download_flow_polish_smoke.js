...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.24 download flow polish smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(index.includes('data-build="1.4.24"'), 'index build should be 1.4.24');
assert(index.includes('1.4.24-bulk-import-hud'), 'asset cache key should use v1.4.24 flow polish');
assert(sw.includes('foxbear-shell-v1.4.24-bulk-import-hud'), 'service worker cache should use v1.4.24 flow polish');
...
assert(service.includes("version: '1.4.24'"), 'download flow/diagnostics should report v1.4.24');
...
assert(matrix.includes('v1.4.24 Download flow polish'), 'QA matrix should document v1.4.24 flow polish');
assert(matrix.includes('Advanced actions are hidden behind'), 'QA matrix should include advanced action collapse scenario');

console.log('PASS v1.4.24 download flow polish smoke');
