...
assert(index.includes('data-build="1.5.99"'), 'index build should be 1.5.99');
assert(index.includes('1.5.99-incident-callable-mail-recovery'), 'asset cache key should use v1.5.99 flow polish');
assert(sw.includes('foxbear-shell-v1.5.99-incident-callable-mail-recovery'), 'service worker cache should use v1.5.99 flow polish');
...
assert(service.includes("version: '1.5.99'"), 'download flow/diagnostics should report v1.5.99');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.99 flow polish');
