...
assert(index.includes('data-build="1.6.87"'), 'index build should be 1.6.87');
assert(index.includes('1.6.87-mobile-header-device-label-overlap-recovery'), 'asset cache key should use v1.6.87 flow polish');
assert(sw.includes('foxbear-shell-v1.6.87-mobile-header-device-label-overlap-recovery'), 'service worker cache should use v1.6.87 flow polish');
...
assert(service.includes("version: '1.6.87'"), 'download flow/diagnostics should report v1.6.87');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.6.87 flow polish');
