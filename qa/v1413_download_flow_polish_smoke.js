...
assert(index.includes('data-build="1.5.98"'), 'index build should be 1.5.98');
assert(index.includes('1.5.98-worker-retry-health-levels'), 'asset cache key should use v1.5.98 flow polish');
assert(sw.includes('foxbear-shell-v1.5.98-worker-retry-health-levels'), 'service worker cache should use v1.5.98 flow polish');
...
assert(service.includes("version: '1.5.98'"), 'download flow/diagnostics should report v1.5.98');
...
assert(matrix.includes('v1.4.26 Download flow polish'), 'QA matrix should document v1.5.98 flow polish');
