...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.7.2' },
  foxbearRelease: {
    buildId: 'old-build',
    assetVersion: '1.7.2-old-build',
    cacheName: 'foxbear-shell-v1.7.2-old-build',
...
afterPackage.version = '1.7.2';
afterPackage.description = 'new release';
afterPackage.scripts['package:verify:release'] = 'node verify v1.7.2';
afterPackage.foxbearRelease = {
  buildId: 'new-build',
  assetVersion: '1.7.2-new-build',
  cacheName: 'foxbear-shell-v1.7.2-new-build',
  bootRevision: 'boot-sri-v172',
  updateSafetyRevision: 'update-safety-v172',
  serviceWorkerRevision: 'sw-v172'
};
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.7.2-old-build';\nrun();\n", "const version = '1.7.2-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.7.2-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.7.2-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.7.2-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.7.2-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
