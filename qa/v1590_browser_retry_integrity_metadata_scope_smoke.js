...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.6.103' },
  foxbearRelease: {
    buildId: 'old-build',
...
afterPackage.scripts['package:verify:release'] = 'node verify v1.6.103';
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.6.103-old-build';\nrun();\n", "const version = '1.6.103-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.103-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.103-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.103-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.103-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
