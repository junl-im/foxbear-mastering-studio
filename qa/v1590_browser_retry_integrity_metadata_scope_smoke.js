...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.6.108' },
  foxbearRelease: {
    buildId: 'old-build',
    assetVersion: '1.6.108-old-build',
    cacheName: 'foxbear-shell-v1.6.108-old-build',
...
afterPackage.version = '1.6.108';
afterPackage.description = 'new release';
afterPackage.scripts['package:verify:release'] = 'node verify v1.6.108';
afterPackage.foxbearRelease = {
  buildId: 'new-build',
  assetVersion: '1.6.108-new-build',
  cacheName: 'foxbear-shell-v1.6.108-new-build',
  bootRevision: 'boot-sri-v16108',
  updateSafetyRevision: 'update-safety-v16108',
  serviceWorkerRevision: 'sw-v16108'
};
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.6.108-old-build';\nrun();\n", "const version = '1.6.108-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.108-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.108-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.108-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.108-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
