...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.6.49' },
  foxbearRelease: {
    buildId: 'old-build',
    assetVersion: '1.6.49-old-build',
    cacheName: 'foxbear-shell-v1.6.49-old-build',
...
afterPackage.version = '1.6.49';
afterPackage.description = 'new release';
afterPackage.scripts['package:verify:release'] = 'node verify v1.6.49';
afterPackage.foxbearRelease = {
  buildId: 'new-build',
  assetVersion: '1.6.49-new-build',
  cacheName: 'foxbear-shell-v1.6.49-new-build',
  bootRevision: 'boot-sri-v1649-download-cache',
  updateSafetyRevision: 'update-safety-v1649-download-cache',
  serviceWorkerRevision: 'sw-v1649-download-cache'
};
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.6.49-old-build';\nrun();\n", "const version = '1.6.49-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.49-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.49-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.49-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.49-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
