...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.6.10' },
  foxbearRelease: {
    buildId: 'old-build',
    assetVersion: '1.6.10-old-build',
    cacheName: 'foxbear-shell-v1.6.10-old-build',
...
afterPackage.version = '1.6.10';
afterPackage.description = 'new release';
afterPackage.scripts['package:verify:release'] = 'node verify v1.6.10';
afterPackage.foxbearRelease = {
  buildId: 'new-build',
  assetVersion: '1.6.10-new-build',
  cacheName: 'foxbear-shell-v1.6.10-new-build',
  bootRevision: 'boot-sri-v1610',
  updateSafetyRevision: 'update-safety-v1610',
  serviceWorkerRevision: 'sw-v1610'
};
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.6.10-old-build';\nrun();\n", "const version = '1.6.10-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.10-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.10-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.6.10-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.6.10-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
