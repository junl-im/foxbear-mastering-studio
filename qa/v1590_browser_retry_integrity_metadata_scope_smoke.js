...
const beforePackage = {
  ...
  scripts: { check: 'node qa/run_all_checks.js', 'package:verify:release': 'node verify v1.5.98' },
  foxbearRelease: {
    buildId: 'old-build',
    assetVersion: '1.5.98-old-build',
    cacheName: 'foxbear-shell-v1.5.98-old-build',
...
afterPackage.version = '1.5.98';
afterPackage.description = 'new release';
afterPackage.scripts['package:verify:release'] = 'node verify v1.5.98';
afterPackage.foxbearRelease = {
  buildId: 'new-build',
  assetVersion: '1.5.98-new-build',
  cacheName: 'foxbear-shell-v1.5.98-new-build',
  bootRevision: 'boot-sri-v1598',
  updateSafetyRevision: 'update-safety-v1598',
  serviceWorkerRevision: 'sw-v1598'
};
...
assert.strictEqual(isReleaseMetadataOnlyChange('src/app.js', "const version = '1.5.98-old-build';\nrun();\n", "const version = '1.5.98-new-build';\nrun();\n", beforeMetadata, afterMetadata), true);
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.5.98-old-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.5.98-old-build" integrity="sha384-OLD"></script>\n');
...
fs.writeFileSync(path.join(gitDir, 'src/app.js'), "const build = '1.5.98-new-build';\n");
fs.writeFileSync(path.join(gitDir, 'index.html'), '<script src="src/app.js?v=1.5.98-new-build" integrity="sha384-NEW"></script>\n');
assert.strictEqual(git(['add', '.']).status, 0);
