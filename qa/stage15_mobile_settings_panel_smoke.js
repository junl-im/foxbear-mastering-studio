...
function assert(condition, message) {
  if (!condition) {
    console.error(`FAIL stage15_mobile_settings_panel_smoke: ${message}`);
    process.exit(1);
  }
}
...
assert(css.includes('Stage15: Quick panel is now a compact Settings panel'), 'mobile CSS should include Stage15 settings layer');
...
assert(index.includes('1.3.84-stage15-settings-panel'), 'index asset query should use Stage15 cache key');
assert(sw.includes('foxbear-shell-v1.3.84-stage15-settings-panel'), 'service worker cache should use Stage15 cache key');
assert(pkg.qaChecks.includes('node qa/stage15_mobile_settings_panel_smoke.js'), 'package QA should include Stage15 settings smoke');

console.log('PASS stage15 mobile settings panel smoke');
