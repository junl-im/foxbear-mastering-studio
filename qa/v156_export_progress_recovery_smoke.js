...
assert(index.includes('src/download/export-progress-view.js?v=1.4.26-wake-lock-state-sync&h=export-progress-v156'), 'export progress view not loaded with v1.5.6 cache key');
...
assert(sw.includes('./src/download/export-progress-view.js?v=1.4.26-wake-lock-state-sync&h=export-progress-v156'), 'export progress view not precached');
assert(index.includes('assets/css/export.css?v=1.4.26-wake-lock-state-sync&h=export-progress-v156'), 'export CSS cache key missing');
assert(sw.includes('./assets/css/export.css?v=1.4.26-wake-lock-state-sync&h=export-progress-v156'), 'export CSS cache key missing from SW');
...
assert(index.includes('h=boot-sri-v156') && sw.includes('h=boot-sri-v156'), 'boot-critical v1.5.6 cache key missing');
assert(index.includes('h=update-safety-v156') && sw.includes('h=update-safety-v156'), 'update safety v1.5.6 cache key missing');
assert(updateSafety.includes("const EXPECTED_BOOT_KEY = 'boot-sri-v156'"), 'Update Safety expected boot key not bumped');
assert(sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") && sw.includes("foxbear-shell-v1.5.5-update-safety"), 'service worker cache generation/legacy missing');
assert(app.includes("./sw.js?v=1.4.26-wake-lock-state-sync&h=sw-v156"), 'app should register service worker with v1.5.6 key');
...
assert(changelog.startsWith('# v1.5.6 - Export Progress Recovery'), 'CHANGELOG missing v1.5.6 top entry');
assert(qaReport.includes('178/178 PASS') && qaReport.includes('v1.5.6 coverage'), 'QA report missing v1.5.6 PASS/coverage');
