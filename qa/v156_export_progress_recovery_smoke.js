...
assert(index.includes('src/download/export-progress-view.js?v=1.6.82-overlay-focus-ancestor-hardening&h=export-progress-v156'), 'export progress view not loaded with v1.5.6 cache key');
...
assert(sw.includes('./src/download/export-progress-view.js?v=1.6.82-overlay-focus-ancestor-hardening&h=export-progress-v156'), 'export progress view not precached');
assert(index.includes('assets/css/export.css?v=1.6.82-overlay-focus-ancestor-hardening&h=export-progress-v156'), 'export CSS cache key missing');
assert(sw.includes('./assets/css/export.css?v=1.6.82-overlay-focus-ancestor-hardening&h=export-progress-v156'), 'export CSS cache key missing from SW');
...
assert(['boot-sri-v156','boot-sri-v1682'].some(key => index.includes(`h=${key}`) && sw.includes(`h=${key}`)), 'boot-critical v1.5.6+ cache key missing');
assert(['update-safety-v156','update-safety-v1682'].some(key => index.includes(`h=${key}`) && sw.includes(`h=${key}`)), 'update safety v1.5.6+ cache key missing');
assert(updateSafety.includes('const EXPECTED_BOOT_KEY = BUILD_INFO.bootRevision ||') && updateSafety.includes('boot-sri-v1682'), 'Update Safety expected boot key should be build-info driven');
assert((sw.includes("foxbear-shell-v1.5.6-export-progress-recovery") || sw.includes("foxbear-shell-v1.6.82-overlay-focus-ancestor-hardening")) && sw.includes("foxbear-shell-v1.5.5-update-safety"), 'service worker cache generation/legacy missing');
assert(['sw-v156','sw-v1682'].some(key => app.includes(`./sw.js?v=1.6.82-overlay-focus-ancestor-hardening&h=${key}`)), 'app should register service worker with v1.5.6+ key');
