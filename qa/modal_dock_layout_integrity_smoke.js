...
must(app.includes("hardSetModalState(dialog, false, 'feature-dialog-open')"), 'feature dialog hard close missing');
must(app.includes("hardSetModalState(el.previewDialog, false, 'preview-dialog-open')"), 'preview dialog hard close missing');
must(app.includes("hardSetModalState(el.previewDialog, true, 'preview-dialog-open')"), 'preview dialog hard open missing');
must(css.includes('v1.3.83 PC Dock / Modal Hard Fix'), 'studio CSS v1.3.83 section missing');
must(dockCss.includes('v1.3.83 PC Dock / Modal Hard Fix'), 'dock CSS v1.3.83 section missing');
must(css.includes('grid-template-columns: minmax(140px, 1fr) minmax(260px, max-content)'), 'dock top line must keep title left and genre/compare right');
must(html.includes('id="bottomPreviewPlayBtn"'), 'PC dock left meta should now be an external play button');
must(css.includes('grid-template-columns: clamp(118px, 13vw, 164px) minmax(0, 1fr)'), 'PC dock two-column layout should be restored with left play column');
...
must(!html.includes('PATCH_NOTES_v1.3.83'), 'no per-version patch note should be referenced');
