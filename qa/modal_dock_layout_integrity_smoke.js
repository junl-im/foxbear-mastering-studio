...
must(css.includes('v1.3.81 Modal / Dock Layout Integrity Audit'), 'studio CSS v1.3.81 section missing');
must(dockCss.includes('v1.3.81 Modal / Dock Layout Integrity Audit'), 'dock CSS v1.3.81 section missing');
must(css.includes('grid-template-columns: minmax(0, 1fr) auto'), 'dock top line must keep title left and genre/compare right');
must(css.includes('.bottom-preview-meta {') && css.includes('display: none !important;'), 'legacy two-column dock meta should be hidden');
...
must(!html.includes('PATCH_NOTES_v1.3.81'), 'no per-version patch note should be referenced');
