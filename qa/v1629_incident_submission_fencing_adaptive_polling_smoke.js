...
async function main() {
    ...
    assert.strictEqual(identity.version, '1.6.113');
    ...
    assert.strictEqual(controls.version, '1.6.113');
    ...
    assert(rules.includes('allow create: if false;'), 'incident submission fencing must be enforced server-side');
    assert(!rules.includes('function validIncidentCreate(reportId)'));
    assert(functionsSource.includes('incidentSubmissionKey(incident)'), 'server canonical submission key fence missing');
