...
must(app.includes("const APP_VERSION = 'Pro v1.3.68'"), 'app version should be v1.3.68');
must(html.includes('data-build="1.3.68"'), 'index build should be v1.3.68');
...
must(app.includes('await masterTrack(track, false, { notifyBlocked: true, forceIfIdle: true, source: \'dock\' })'), 'dock master button must force explicit runtime path');
