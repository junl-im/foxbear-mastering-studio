...
must(index.includes('1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'), 'index asset key should be v1.6.71 cache key');
must(sw.includes('foxbear-shell-v1.6.71-pwa-share-lease-handoff-deploy-policy-e2e'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.6.71');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.6.71 smoke should be in package qaChecks');
