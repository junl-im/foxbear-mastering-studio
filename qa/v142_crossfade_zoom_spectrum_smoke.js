...
must(index.includes('1.5.91-cancellable-audio-pipeline-performance-guards'), 'index asset key should be v1.5.91 cache key');
must(sw.includes('foxbear-shell-v1.5.91-cancellable-audio-pipeline-performance-guards'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.5.91');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.5.91 smoke should be in package qaChecks');
