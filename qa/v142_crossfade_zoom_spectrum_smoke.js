...
must(index.includes('1.4.23-audio-decode-memory-guard'), 'index asset key should be v1.4.23 cache key');
must(sw.includes('foxbear-shell-v1.4.23-audio-decode-memory-guard'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.4.23');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.4.23 smoke should be in package qaChecks');

console.log('PASS v1.4.23 crossfade zoom spectrum smoke');
