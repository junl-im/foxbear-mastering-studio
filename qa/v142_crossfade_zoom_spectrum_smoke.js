...
must(index.includes('1.4.12-download-diagnostics'), 'index asset key should be v1.4.12 cache key');
must(sw.includes('foxbear-shell-v1.4.12-download-diagnostics'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.4.12');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.4.12 smoke should be in package qaChecks');

console.log('PASS v1.4.12 crossfade zoom spectrum smoke');
