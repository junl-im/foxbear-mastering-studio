...
must(index.includes('1.5.97-worker-recovery-diagnostics'), 'index asset key should be v1.5.97 cache key');
must(sw.includes('foxbear-shell-v1.5.97-worker-recovery-diagnostics'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.5.97');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.5.97 smoke should be in package qaChecks');
