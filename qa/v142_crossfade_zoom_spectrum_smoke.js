...
must(index.includes('1.5.32-kakao-external-browser-local-flow'), 'index asset key should be v1.5.32 cache key');
must(sw.includes('foxbear-shell-v1.5.32-kakao-external-browser-local-flow'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.5.32');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.5.32 smoke should be in package qaChecks');
