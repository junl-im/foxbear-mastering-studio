...
must(index.includes('1.4.26-wake-lock-state-sync'), 'index asset key should be v1.4.26 cache key');
must(sw.includes('foxbear-shell-v1.4.26-wake-lock-state-sync'), 'service worker cache should be bumped');
...
must(!index.includes('id="bottomPreviewSpectrum"'), 'Dock mini spectrum host should be removed in v1.4.26');
...
must(pkg.qaChecks.includes('node qa/v142_crossfade_zoom_spectrum_smoke.js'), 'v1.4.26 smoke should be in package qaChecks');
