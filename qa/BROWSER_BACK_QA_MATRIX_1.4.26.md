...
| --- | --- | --- |
| Wake Lock setting | Settings panel opened while no playback/import/mastering is active | `화면켜짐유지` shows `OFF` and no `화면유지 ON` toast appears automatically. |
| Wake Lock auto mode | Playback/import/mastering is active while user setting is OFF | Wake Lock may run as temporary `AUTO`, settings badge shows `AUTO`, and automatic acquisition remains silent. |
| Wake Lock manual ON | User taps `화면켜짐유지` | Setting persists as `ON`, manual toast may appear once, and `FoxBearWakeLockController.getSnapshot().userEnabled` is true. |
| Wake Lock manual OFF | User taps `화면켜짐유지` while user setting is ON | Setting persists as `OFF`, active sentinel is released, and the off toast appears once. |
| Wake Lock unsupported/failure | Browser rejects Wake Lock request | Manual desired setting reverts to `OFF`, error is recorded in diagnostics, and setting is not left falsely ON. |
| Diagnostics | `?perf=1` or console snapshot is collected | `wakeLock` snapshot includes `active`, `mode`, `settingLabel`, `userEnabled`, and last error/request counters. |
