...
assert(pkg.includes('Stage26 unified waveform controls') || pkg.includes('Stage27 common waveform control service') || pkg.includes('Stage26 unified waveform controls') || pkg.includes('Stage25 compare controls rehome') || pkg.includes('Stage23 playback orchestration'), 'package description should identify current Stage23+ line');
assert(pkg.includes('node qa/stage23_playback_orchestration_smoke.js'), 'package should run Stage23 smoke');
assert(overwrite.includes('v1.4.0-stage27'), 'overwrite package default should be Stage23');
