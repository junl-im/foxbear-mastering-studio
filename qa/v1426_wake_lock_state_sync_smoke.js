...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.4.26 smoke');
must(index.includes('data-build="1.4.26"'), 'index build marker should be 1.4.26');
must(index.includes('1.4.26-wake-lock-state-sync'), 'index cache key should use v1.4.26 wake key');
must(sw.includes('foxbear-shell-v1.4.26-wake-lock-state-sync'), 'service worker cache should use v1.4.26 wake key');
...
must(runtime.includes('1.4.26-wake-lock-state-sync'), 'runtime health fallback version should be v1.4.26');
...
must(report.includes('148/148 PASS') || report.includes('v1.4.26'), 'QA report should mention v1.4.26 or 148/148 PASS');
must(changelog.includes('v1.4.26') && changelog.includes('Wake Lock'), 'changelog should mention v1.4.26 Wake Lock');
