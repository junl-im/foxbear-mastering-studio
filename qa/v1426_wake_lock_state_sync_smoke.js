...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.6.70 smoke');
must(index.includes('data-build="1.6.70"'), 'index build marker should be 1.6.70');
must(index.includes('1.6.70-share-retry-policy-drift-ci-efficiency'), 'index cache key should use v1.6.70 wake key');
must(sw.includes('foxbear-shell-v1.6.70-share-retry-policy-drift-ci-efficiency'), 'service worker cache should use v1.6.70 wake key');
...
must(runtime.includes('1.6.70-share-retry-policy-drift-ci-efficiency'), 'runtime health fallback version should be v1.6.70');
...
must(report.includes('148/148 PASS') || report.includes('v1.6.70'), 'QA report should mention v1.6.70 or 148/148 PASS');
must(changelog.includes('v1.6.70') && changelog.includes('Wake Lock'), 'changelog should mention v1.6.70 Wake Lock');
