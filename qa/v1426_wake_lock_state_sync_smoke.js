...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.60 smoke');
must(index.includes('data-build="1.5.60"'), 'index build marker should be 1.5.60');
must(index.includes('1.5.60-kakao-inapp-entry-memory-governor'), 'index cache key should use v1.5.60 wake key');
must(sw.includes('foxbear-shell-v1.5.60-kakao-inapp-entry-memory-governor'), 'service worker cache should use v1.5.60 wake key');
...
must(runtime.includes('1.5.60-kakao-inapp-entry-memory-governor'), 'runtime health fallback version should be v1.5.60');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.60'), 'QA report should mention v1.5.60 or 148/148 PASS');
must(changelog.includes('v1.5.60') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.60 Wake Lock');
