...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.62 smoke');
must(index.includes('data-build="1.5.62"'), 'index build marker should be 1.5.62');
must(index.includes('1.5.62-incident-delivery-watchdog-package-gate'), 'index cache key should use v1.5.62 wake key');
must(sw.includes('foxbear-shell-v1.5.62-incident-delivery-watchdog-package-gate'), 'service worker cache should use v1.5.62 wake key');
...
must(runtime.includes('1.5.62-incident-delivery-watchdog-package-gate'), 'runtime health fallback version should be v1.5.62');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.62'), 'QA report should mention v1.5.62 or 148/148 PASS');
must(changelog.includes('v1.5.62') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.62 Wake Lock');
