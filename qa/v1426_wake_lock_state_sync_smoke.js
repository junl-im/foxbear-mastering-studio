...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.41 smoke');
must(index.includes('data-build="1.5.41"'), 'index build marker should be 1.5.41');
must(index.includes('1.5.41-export-eta-download-recovery'), 'index cache key should use v1.5.41 wake key');
must(sw.includes('foxbear-shell-v1.5.41-export-eta-download-recovery'), 'service worker cache should use v1.5.41 wake key');
...
must(runtime.includes('1.5.41-export-eta-download-recovery'), 'runtime health fallback version should be v1.5.41');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.41'), 'QA report should mention v1.5.41 or 148/148 PASS');
must(changelog.includes('v1.5.41') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.41 Wake Lock');
