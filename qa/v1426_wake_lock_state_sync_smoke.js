...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.6.45 smoke');
must(index.includes('data-build="1.6.45"'), 'index build marker should be 1.6.45');
must(index.includes('1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'index cache key should use v1.6.45 wake key');
must(sw.includes('foxbear-shell-v1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'service worker cache should use v1.6.45 wake key');
...
must(runtime.includes('1.6.45-windows-release-gate-spark-hosting-no-app-check'), 'runtime health fallback version should be v1.6.45');
...
must(report.includes('148/148 PASS') || report.includes('v1.6.45'), 'QA report should mention v1.6.45 or 148/148 PASS');
must(changelog.includes('v1.6.45') && changelog.includes('Wake Lock'), 'changelog should mention v1.6.45 Wake Lock');
