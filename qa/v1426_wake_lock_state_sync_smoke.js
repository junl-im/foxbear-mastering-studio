...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.15 smoke');
must(index.includes('data-build="1.5.15"'), 'index build marker should be 1.5.15');
must(index.includes('1.5.15-e2e-runtime-classification'), 'index cache key should use v1.5.15 wake key');
must(sw.includes('foxbear-shell-v1.5.15-e2e-runtime-classification'), 'service worker cache should use v1.5.15 wake key');
...
must(runtime.includes('1.5.15-e2e-runtime-classification'), 'runtime health fallback version should be v1.5.15');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.15'), 'QA report should mention v1.5.15 or 148/148 PASS');
must(changelog.includes('v1.5.15') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.15 Wake Lock');
