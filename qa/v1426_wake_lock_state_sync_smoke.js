...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.7.0 smoke');
must(index.includes('data-build="1.7.0"'), 'index build marker should be 1.7.0');
must(index.includes('1.7.0-adaptive-mastering-decision-phase1'), 'index cache key should use v1.7.0 wake key');
must(sw.includes('foxbear-shell-v1.7.0-adaptive-mastering-decision-phase1'), 'service worker cache should use v1.7.0 wake key');
...
must(runtime.includes('1.7.0-adaptive-mastering-decision-phase1'), 'runtime health fallback version should be v1.7.0');
...
must(report.includes('148/148 PASS') || report.includes('v1.7.0'), 'QA report should mention v1.7.0 or 148/148 PASS');
must(changelog.includes('v1.7.0') && changelog.includes('Wake Lock'), 'changelog should mention v1.7.0 Wake Lock');
