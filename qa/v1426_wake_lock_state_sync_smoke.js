...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.25 smoke');
must(index.includes('data-build="1.5.25"'), 'index build marker should be 1.5.25');
must(index.includes('1.5.25-e2e-preview-stability'), 'index cache key should use v1.5.25 wake key');
must(sw.includes('foxbear-shell-v1.5.25-e2e-preview-stability'), 'service worker cache should use v1.5.25 wake key');
...
must(runtime.includes('1.5.25-e2e-preview-stability'), 'runtime health fallback version should be v1.5.25');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.25'), 'QA report should mention v1.5.25 or 148/148 PASS');
must(changelog.includes('v1.5.25') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.25 Wake Lock');
