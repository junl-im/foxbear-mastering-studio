...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.69 smoke');
must(index.includes('data-build="1.5.69"'), 'index build marker should be 1.5.69');
must(index.includes('1.5.69-mail-receipt-confirmation-history-branded-template'), 'index cache key should use v1.5.69 wake key');
must(sw.includes('foxbear-shell-v1.5.69-mail-receipt-confirmation-history-branded-template'), 'service worker cache should use v1.5.69 wake key');
...
must(runtime.includes('1.5.69-mail-receipt-confirmation-history-branded-template'), 'runtime health fallback version should be v1.5.69');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.69'), 'QA report should mention v1.5.69 or 148/148 PASS');
must(changelog.includes('v1.5.69') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.69 Wake Lock');
