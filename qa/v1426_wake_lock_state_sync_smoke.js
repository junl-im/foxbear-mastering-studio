...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.6.92 smoke');
must(index.includes('data-build="1.6.92"'), 'index build marker should be 1.6.92');
must(index.includes('1.6.92-spectrum-panel-mount-lifecycle-recovery'), 'index cache key should use v1.6.92 wake key');
must(sw.includes('foxbear-shell-v1.6.92-spectrum-panel-mount-lifecycle-recovery'), 'service worker cache should use v1.6.92 wake key');
...
must(runtime.includes('1.6.92-spectrum-panel-mount-lifecycle-recovery'), 'runtime health fallback version should be v1.6.92');
...
must(report.includes('148/148 PASS') || report.includes('v1.6.92'), 'QA report should mention v1.6.92 or 148/148 PASS');
must(changelog.includes('v1.6.92') && changelog.includes('Wake Lock'), 'changelog should mention v1.6.92 Wake Lock');
