...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.6.78 smoke');
must(index.includes('data-build="1.6.78"'), 'index build marker should be 1.6.78');
must(index.includes('1.6.78-release-generation-assist-url-playback-invalidation'), 'index cache key should use v1.6.78 wake key');
must(sw.includes('foxbear-shell-v1.6.78-release-generation-assist-url-playback-invalidation'), 'service worker cache should use v1.6.78 wake key');
...
must(runtime.includes('1.6.78-release-generation-assist-url-playback-invalidation'), 'runtime health fallback version should be v1.6.78');
...
must(report.includes('148/148 PASS') || report.includes('v1.6.78'), 'QA report should mention v1.6.78 or 148/148 PASS');
must(changelog.includes('v1.6.78') && changelog.includes('Wake Lock'), 'changelog should mention v1.6.78 Wake Lock');
