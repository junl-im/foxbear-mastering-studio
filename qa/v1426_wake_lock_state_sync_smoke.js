...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.45 smoke');
must(index.includes('data-build="1.5.45"'), 'index build marker should be 1.5.45');
must(index.includes('1.5.45-export-queue-recovery'), 'index cache key should use v1.5.45 wake key');
must(sw.includes('foxbear-shell-v1.5.45-export-queue-recovery'), 'service worker cache should use v1.5.45 wake key');
...
must(runtime.includes('1.5.45-export-queue-recovery'), 'runtime health fallback version should be v1.5.45');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.45'), 'QA report should mention v1.5.45 or 148/148 PASS');
must(changelog.includes('v1.5.45') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.45 Wake Lock');
