...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.23 smoke');
must(index.includes('data-build="1.5.23"'), 'index build marker should be 1.5.23');
must(index.includes('1.5.23-e2e-preview-readiness'), 'index cache key should use v1.5.23 wake key');
must(sw.includes('foxbear-shell-v1.5.23-e2e-preview-readiness'), 'service worker cache should use v1.5.23 wake key');
...
must(runtime.includes('1.5.23-e2e-preview-readiness'), 'runtime health fallback version should be v1.5.23');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.23'), 'QA report should mention v1.5.23 or 148/148 PASS');
must(changelog.includes('v1.5.23') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.23 Wake Lock');
