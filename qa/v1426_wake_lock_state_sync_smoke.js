...
must(pkg.qaChecks.includes('node qa/v1426_wake_lock_state_sync_smoke.js'), 'package QA should run v1.5.66 smoke');
must(index.includes('data-build="1.5.66"'), 'index build marker should be 1.5.66');
must(index.includes('1.5.66-incident-operations-action-guard-deploy-verification'), 'index cache key should use v1.5.66 wake key');
must(sw.includes('foxbear-shell-v1.5.66-incident-operations-action-guard-deploy-verification'), 'service worker cache should use v1.5.66 wake key');
...
must(runtime.includes('1.5.66-incident-operations-action-guard-deploy-verification'), 'runtime health fallback version should be v1.5.66');
...
must(report.includes('148/148 PASS') || report.includes('v1.5.66'), 'QA report should mention v1.5.66 or 148/148 PASS');
must(changelog.includes('v1.5.66') && changelog.includes('Wake Lock'), 'changelog should mention v1.5.66 Wake Lock');
