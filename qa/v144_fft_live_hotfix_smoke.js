...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.6 FFT live hotfix smoke: ${message}`);
    process.exit(1);
  }
};
...
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.4.6 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache spectrum visualizer with v1.4.6 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker cache should use v1.4.6 key');
must(runtime.includes('FoxBearSpectrumVisualizer.renderMini'), 'runtime health should require mini spectrum API');

must(spectrum.includes("VISUALIZER_VERSION = '1.4.6-stability-polish'"), 'spectrum visualizer version should be v1.4.6 hotfix');
...
must(pkg.qaChecks.includes('node qa/v144_fft_live_hotfix_smoke.js'), 'v1.4.6 smoke should run in npm check');

console.log('PASS v1.4.6 FFT live hotfix smoke');
