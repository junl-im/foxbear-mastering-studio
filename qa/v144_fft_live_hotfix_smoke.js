...
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.6.50 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache spectrum visualizer with v1.6.50 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker cache should use v1.6.50 key');
must(runtime.includes('FoxBearSpectrumVisualizer.renderPanel'), 'runtime health should require detail spectrum API');

must(spectrum.includes("VISUALIZER_VERSION = '1.6.50-kakao-centered-entry-notice'"), 'spectrum visualizer version should be v1.6.50 hotfix');
...
must(!app.includes('function renderBottomMiniSpectrum'), 'Dock mini spectrum renderer should be removed in v1.6.50');
must(!app.includes('bottomPreviewSpectrum'), 'Dock mini spectrum ref should be removed in v1.6.50');
must(pkg.qaChecks.includes('node qa/v144_fft_live_hotfix_smoke.js'), 'v1.6.50 smoke should run in npm check');
