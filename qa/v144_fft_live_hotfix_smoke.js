...
const must = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.7 FFT live hotfix smoke: ${message}`);
    process.exit(1);
  }
};
...
must(index.includes(`src/ui/spectrum-visualizer.js?v=${version}`), 'index should load spectrum visualizer with v1.4.7 cache key');
must(sw.includes(`./src/ui/spectrum-visualizer.js?v=${version}`), 'service worker should precache spectrum visualizer with v1.4.7 key');
must(sw.includes(`foxbear-shell-v${version}`), 'service worker cache should use v1.4.7 key');
must(runtime.includes('FoxBearSpectrumVisualizer.renderPanel'), 'runtime health should require detail spectrum API');

must(spectrum.includes("VISUALIZER_VERSION = '1.4.7-dock-fft-removal'"), 'spectrum visualizer version should be v1.4.7 hotfix');
...
must(!app.includes('function renderBottomMiniSpectrum'), 'Dock mini spectrum renderer should be removed in v1.4.7');
must(!app.includes('bottomPreviewSpectrum'), 'Dock mini spectrum ref should be removed in v1.4.7');
must(pkg.qaChecks.includes('node qa/v144_fft_live_hotfix_smoke.js'), 'v1.4.7 smoke should run in npm check');

console.log('PASS v1.4.7 FFT live hotfix smoke');
