# FoxBear v1.4.18 Browser / Download QA Matrix

목표: v1.4.18 다운로드 영수증/상태 안내가 실제 저장 흐름을 방해하지 않고, 카카오/모바일 fallback 이해도를 높이는지 확인한다.
...
## Cumulative regression anchors retained in v1.4.18
...
- v1.4.18 stability / confirm focus: Back confirm debounce should avoid stacked confirm dialogs.
...
- v1.4.18 Download flow polish: recommended flow card should show the current browser path.
...
- v1.4.18 Download action clarity: visible buttons should expose `data-download-action` metadata.
- v1.4.18 Download receipt polish: the receipt/status card should explain the action result and next steps.
