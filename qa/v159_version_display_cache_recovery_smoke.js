...
assert(pkg.qaChecks.includes('node qa/v159_version_display_cache_recovery_smoke.js'), 'v1.5.13 smoke missing from package QA');
...
assert(status.includes('FoxBearReleasePresentation') && status.includes('1.5.13-handoff-package-integrity'), 'STATUS current release/invariant missing');
assert(changelog.includes(`# v${historicalVersion} - Version Display and Cache Recovery`), 'CHANGELOG v1.5.13 history missing');
assert(readme.includes(`v${historicalVersion} Version Display and Cache Recovery`), 'README v1.5.13 history missing');
assert(handoff.includes(`v${historicalVersion} Version Display and Cache Recovery`), 'HANDOFF v1.5.13 history missing');
assert(qaReport.includes('185/185 PASS') && qaReport.includes(`## v${historicalVersion} coverage`), 'QA report missing v1.5.13 result/coverage');
