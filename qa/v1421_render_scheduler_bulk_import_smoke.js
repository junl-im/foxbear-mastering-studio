...
assert(pkg.name === 'foxbear-github-pro-v1-4-26', 'package name should be v1-4-26');
assert(index.includes('data-build="1.4.26"'), 'index build marker should be 1.4.26');
assert(config.includes("ASSET_VERSION = '1.4.26-wake-lock-state-sync'"), 'runtime asset key should be v1.4.26');
assert(sw.includes('foxbear-shell-v1.4.26-wake-lock-state-sync'), 'service worker cache should use v1.4.26 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.4.26-wake-lock-state-sync'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.4.26-wake-lock-state-sync'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.4.26-wake-lock-state-sync'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.4.26-wake-lock-state-sync'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.4.26 scope');
assert(qaReport.includes('144/144 PASS') || qaReport.includes('v1.4.26 final QA'), 'QA report should mention v1.4.26 final QA');
assert(changelog.includes('v1.4.26'), 'changelog should mention v1.4.26');
