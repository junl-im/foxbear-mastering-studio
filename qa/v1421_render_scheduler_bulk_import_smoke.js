...
assert(config.includes("ASSET_VERSION = '1.6.112-mastering-lifecycle-race-hardening'"), 'runtime asset key should be v1.6.112');
assert(sw.includes('foxbear-shell-v1.6.112-mastering-lifecycle-race-hardening'), 'service worker cache should use v1.6.112 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.6.112-mastering-lifecycle-race-hardening'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.6.112 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.6.112 final QA');
assert(changelog.includes('v1.6.112'), 'changelog should mention v1.6.112');
