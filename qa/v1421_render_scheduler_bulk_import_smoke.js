...
assert(config.includes("ASSET_VERSION = '1.5.69-mail-receipt-confirmation-history-branded-template'"), 'runtime asset key should be v1.5.69');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.69-mail-receipt-confirmation-history-branded-template'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.69-mail-receipt-confirmation-history-branded-template'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.69-mail-receipt-confirmation-history-branded-template'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.69-mail-receipt-confirmation-history-branded-template'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.69 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.69 final QA');
