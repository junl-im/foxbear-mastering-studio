...
assert(pkg.version === '1.6.39', 'package version should be 1.6.39');
assert(pkg.name === 'foxbear-mastering-studio', 'package name should be v1-4-26');
assert(index.includes('data-build="1.6.39"'), 'index build marker should be 1.6.39');
assert(config.includes("ASSET_VERSION = '1.6.39-ui-shell-partial-script-probe-isolation'"), 'runtime asset key should be v1.6.39');
assert(sw.includes('foxbear-shell-v1.6.39-ui-shell-partial-script-probe-isolation'), 'service worker cache should use v1.6.39 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.6.39-ui-shell-partial-script-probe-isolation'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.6.39-ui-shell-partial-script-probe-isolation'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.6.39-ui-shell-partial-script-probe-isolation'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.6.39-ui-shell-partial-script-probe-isolation'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.6.39 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.6.39 final QA');
assert(changelog.includes('v1.6.39'), 'changelog should mention v1.6.39');
