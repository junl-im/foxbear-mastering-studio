...
assert(config.includes("ASSET_VERSION = '1.5.79-preview-download-ownership-recovery'"), 'runtime asset key should be v1.5.79');
assert(sw.includes('foxbear-shell-v1.5.79-preview-download-ownership-recovery'), 'service worker cache should use v1.5.79 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.79-preview-download-ownership-recovery'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.79-preview-download-ownership-recovery'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.79-preview-download-ownership-recovery'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.79-preview-download-ownership-recovery'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.79 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.79 final QA');
assert(changelog.includes('v1.5.79'), 'changelog should mention v1.5.79');
