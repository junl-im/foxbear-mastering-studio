...
assert(config.includes("ASSET_VERSION = '1.5.32-kakao-external-browser-local-flow'"), 'runtime asset key should be v1.5.32');
assert(sw.includes('foxbear-shell-v1.5.32-kakao-external-browser-local-flow'), 'service worker cache should use v1.5.32 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.32-kakao-external-browser-local-flow'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.32-kakao-external-browser-local-flow'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.32-kakao-external-browser-local-flow'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.32-kakao-external-browser-local-flow'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.32 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.32 final QA');
assert(changelog.includes('v1.5.32'), 'changelog should mention v1.5.32');
