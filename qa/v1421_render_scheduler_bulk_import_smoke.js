...
assert(pkg.version === '1.5.53', 'package version should be 1.5.53');
assert(pkg.name === 'foxbear-mastering-studio', 'package name should be v1-4-26');
assert(index.includes('data-build="1.5.53"'), 'index build marker should be 1.5.53');
assert(config.includes("ASSET_VERSION = '1.5.53-engine-recovery-performance-diagnostics'"), 'runtime asset key should be v1.5.53');
assert(sw.includes('foxbear-shell-v1.5.53-engine-recovery-performance-diagnostics'), 'service worker cache should use v1.5.53 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.53-engine-recovery-performance-diagnostics'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.53-engine-recovery-performance-diagnostics'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.53-engine-recovery-performance-diagnostics'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.53-engine-recovery-performance-diagnostics'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.53 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.53 final QA');
assert(changelog.includes('v1.5.53'), 'changelog should mention v1.5.53');
