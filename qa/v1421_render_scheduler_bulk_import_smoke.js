...
assert(pkg.version === '1.5.21', 'package version should be 1.5.21');
assert(pkg.name === 'foxbear-mastering-studio', 'package name should be v1-4-26');
assert(index.includes('data-build="1.5.21"'), 'index build marker should be 1.5.21');
assert(config.includes("ASSET_VERSION = '1.5.21-history-csp-console-contract-fix'"), 'runtime asset key should be v1.5.21');
assert(sw.includes('foxbear-shell-v1.5.21-history-csp-console-contract-fix'), 'service worker cache should use v1.5.21 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.21-history-csp-console-contract-fix'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.21-history-csp-console-contract-fix'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.21-history-csp-console-contract-fix'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.21-history-csp-console-contract-fix'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.21 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.21 final QA');
assert(changelog.includes('v1.5.21'), 'changelog should mention v1.5.21');
