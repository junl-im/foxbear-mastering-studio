...
assert(config.includes("ASSET_VERSION = '1.5.73-bulk-control-eta-result-filter-ui'"), 'runtime asset key should be v1.5.73');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.73-bulk-control-eta-result-filter-ui'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.73-bulk-control-eta-result-filter-ui'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.73-bulk-control-eta-result-filter-ui'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.73-bulk-control-eta-result-filter-ui'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.73 scope');
assert(/\b(\d+)\/\1 PASS\b/.test(qaReport), 'QA report should mention v1.5.73 final QA');
