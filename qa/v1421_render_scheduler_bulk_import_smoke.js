...
assert(pkg.version === '1.5.12', 'package version should be 1.5.12');
assert(pkg.name === 'foxbear-mastering-studio', 'package name should be v1-4-26');
assert(index.includes('data-build="1.5.12"'), 'index build marker should be 1.5.12');
assert(config.includes("ASSET_VERSION = '1.5.12-ci-runtime-readiness'"), 'runtime asset key should be v1.5.12');
assert(sw.includes('foxbear-shell-v1.5.12-ci-runtime-readiness'), 'service worker cache should use v1.5.12 key');
...
assert(index.includes('src/boot/render-scheduler.js?v=1.5.12-ci-runtime-readiness'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.5.12-ci-runtime-readiness'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.5.12-ci-runtime-readiness'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.5.12-ci-runtime-readiness'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.26 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.5.12 scope');
assert(qaReport.includes('144/144 PASS') || qaReport.includes('v1.5.12 final QA'), 'QA report should mention v1.5.12 final QA');
assert(changelog.includes('v1.5.12'), 'changelog should mention v1.5.12');
