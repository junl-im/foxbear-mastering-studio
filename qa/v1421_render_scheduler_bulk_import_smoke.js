...
const assert = (condition, message) => {
  if (!condition) {
    console.error(`FAIL v1.4.23 render scheduler bulk import smoke: ${message}`);
    process.exit(1);
  }
};
...
assert(index.includes('src/boot/render-scheduler.js?v=1.4.23-audio-decode-memory-guard'), 'index should load render scheduler service');
assert(index.includes('src/audio/audio-decode-service.js?v=1.4.23-audio-decode-memory-guard'), 'index should load audio decode service');
assert(sw.includes('./src/boot/render-scheduler.js?v=1.4.23-audio-decode-memory-guard'), 'service worker should precache render scheduler');
assert(sw.includes('./src/audio/audio-decode-service.js?v=1.4.23-audio-decode-memory-guard'), 'service worker should precache audio decode service');
...
assert(matrix.includes('v1.4.23 Render Scheduler + Bulk Import UI Throttle'), 'matrix should document v1.4.23 scope');
...
console.log('PASS v1.4.23 carry-forward: render scheduler, silent wake lock, cache, decode, crossfade smoke');
