...
const sandbox = {
  ...
  FoxBearBuildInfo: { productVersion: '1.6.88', assetVersion: '1.6.88-mobile-header-grid-ownership-recovery' }
};
...
assert.strictEqual(metadata.productVersion, '1.6.88');
...
serviceStatus.handler({ auth: { uid: 'guest-1' }, app: null }).then(result => {
  assert.strictEqual(result.productVersion, '1.6.88');
