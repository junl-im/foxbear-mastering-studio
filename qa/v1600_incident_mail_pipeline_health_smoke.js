...
const sandbox = {
  ...
  FoxBearBuildInfo: { productVersion: '1.6.73', assetVersion: '1.6.73-csp-memory-admission-runtime-config' }
};
...
assert.strictEqual(metadata.productVersion, '1.6.73');
...
serviceStatus.handler({ auth: { uid: 'guest-1' }, app: null }).then(result => {
  assert.strictEqual(result.productVersion, '1.6.73');
