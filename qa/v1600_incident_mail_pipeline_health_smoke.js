...
const sandbox = {
  ...
  FoxBearBuildInfo: { productVersion: '1.6.101', assetVersion: '1.6.101-admin-lazyload-repo-cleanup' }
};
...
assert.strictEqual(metadata.productVersion, '1.6.101');
...
serviceStatus.handler({ auth: { uid: 'guest-1' }, app: null }).then(result => {
  assert.strictEqual(result.productVersion, '1.6.101');
