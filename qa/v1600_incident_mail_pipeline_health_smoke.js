...
const sandbox = {
  ...
  FoxBearBuildInfo: { productVersion: '1.6.2', assetVersion: '1.6.2-nonblocking-health-status-design-polish' }
};
...
assert.strictEqual(metadata.productVersion, '1.6.2');
...
serviceStatus.handler({ auth: { uid: 'guest-1' }, app: null }).then(result => {
  assert.strictEqual(result.productVersion, '1.6.2');
