...
const sandbox = {
  ...
  FoxBearBuildInfo: { productVersion: '1.6.100', assetVersion: '1.6.100-sw-stereo-ci-cleanup-hardening' }
};
...
assert.strictEqual(metadata.productVersion, '1.6.100');
...
serviceStatus.handler({ auth: { uid: 'guest-1' }, app: null }).then(result => {
  assert.strictEqual(result.productVersion, '1.6.100');
